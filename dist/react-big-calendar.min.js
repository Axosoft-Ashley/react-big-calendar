!(function(e, t) {
  'object' == typeof exports && 'undefined' != typeof module
    ? t(exports, require('react'), require('react-dom'))
    : 'function' == typeof define && define.amd
    ? define(['exports', 'react', 'react-dom'], t)
    : t(((e = e || self).ReactBigCalendar = {}), e.React, e.ReactDOM)
})(this, function(e, t, n) {
  'use strict'
  var r = 'default' in t ? t.default : t,
    o = 'default' in n ? n.default : n
  function a(e) {
    return e.children
  }
  function i() {
    return (i =
      Object.assign ||
      function(e) {
        for (var t = 1; t < arguments.length; t++) {
          var n = arguments[t]
          for (var r in n)
            Object.prototype.hasOwnProperty.call(n, r) && (e[r] = n[r])
        }
        return e
      }).apply(this, arguments)
  }
  function s(e, t) {
    if (null == e) return {}
    var n,
      r,
      o = {},
      a = Object.keys(e)
    for (r = 0; r < a.length; r++)
      (n = a[r]), t.indexOf(n) >= 0 || (o[n] = e[n])
    return o
  }
  function c(e, t) {
    ;(e.prototype = Object.create(t.prototype)),
      (e.prototype.constructor = e),
      (e.__proto__ = t)
  }
  function l(e) {
    return e &&
      e.__esModule &&
      Object.prototype.hasOwnProperty.call(e, 'default')
      ? e.default
      : e
  }
  function u(e, t) {
    return e((t = { exports: {} }), t.exports), t.exports
  }
  var f = u(function(e, t) {
    Object.defineProperty(t, '__esModule', { value: !0 })
    var n = 'function' == typeof Symbol && Symbol.for,
      r = n ? Symbol.for('react.element') : 60103,
      o = n ? Symbol.for('react.portal') : 60106,
      a = n ? Symbol.for('react.fragment') : 60107,
      i = n ? Symbol.for('react.strict_mode') : 60108,
      s = n ? Symbol.for('react.profiler') : 60114,
      c = n ? Symbol.for('react.provider') : 60109,
      l = n ? Symbol.for('react.context') : 60110,
      u = n ? Symbol.for('react.async_mode') : 60111,
      f = n ? Symbol.for('react.concurrent_mode') : 60111,
      d = n ? Symbol.for('react.forward_ref') : 60112,
      p = n ? Symbol.for('react.suspense') : 60113,
      v = n ? Symbol.for('react.memo') : 60115,
      h = n ? Symbol.for('react.lazy') : 60116
    function m(e) {
      if ('object' == typeof e && null !== e) {
        var t = e.$$typeof
        switch (t) {
          case r:
            switch ((e = e.type)) {
              case u:
              case f:
              case a:
              case s:
              case i:
              case p:
                return e
              default:
                switch ((e = e && e.$$typeof)) {
                  case l:
                  case d:
                  case c:
                    return e
                  default:
                    return t
                }
            }
          case h:
          case v:
          case o:
            return t
        }
      }
    }
    function g(e) {
      return m(e) === f
    }
    ;(t.typeOf = m),
      (t.AsyncMode = u),
      (t.ConcurrentMode = f),
      (t.ContextConsumer = l),
      (t.ContextProvider = c),
      (t.Element = r),
      (t.ForwardRef = d),
      (t.Fragment = a),
      (t.Lazy = h),
      (t.Memo = v),
      (t.Portal = o),
      (t.Profiler = s),
      (t.StrictMode = i),
      (t.Suspense = p),
      (t.isValidElementType = function(e) {
        return (
          'string' == typeof e ||
          'function' == typeof e ||
          e === a ||
          e === f ||
          e === s ||
          e === i ||
          e === p ||
          ('object' == typeof e &&
            null !== e &&
            (e.$$typeof === h ||
              e.$$typeof === v ||
              e.$$typeof === c ||
              e.$$typeof === l ||
              e.$$typeof === d))
        )
      }),
      (t.isAsyncMode = function(e) {
        return g(e) || m(e) === u
      }),
      (t.isConcurrentMode = g),
      (t.isContextConsumer = function(e) {
        return m(e) === l
      }),
      (t.isContextProvider = function(e) {
        return m(e) === c
      }),
      (t.isElement = function(e) {
        return 'object' == typeof e && null !== e && e.$$typeof === r
      }),
      (t.isForwardRef = function(e) {
        return m(e) === d
      }),
      (t.isFragment = function(e) {
        return m(e) === a
      }),
      (t.isLazy = function(e) {
        return m(e) === h
      }),
      (t.isMemo = function(e) {
        return m(e) === v
      }),
      (t.isPortal = function(e) {
        return m(e) === o
      }),
      (t.isProfiler = function(e) {
        return m(e) === s
      }),
      (t.isStrictMode = function(e) {
        return m(e) === i
      }),
      (t.isSuspense = function(e) {
        return m(e) === p
      })
  })
  l(f)
  f.typeOf,
    f.AsyncMode,
    f.ConcurrentMode,
    f.ContextConsumer,
    f.ContextProvider,
    f.Element,
    f.ForwardRef,
    f.Fragment,
    f.Lazy,
    f.Memo,
    f.Portal,
    f.Profiler,
    f.StrictMode,
    f.Suspense,
    f.isValidElementType,
    f.isAsyncMode,
    f.isConcurrentMode,
    f.isContextConsumer,
    f.isContextProvider,
    f.isElement,
    f.isForwardRef,
    f.isFragment,
    f.isLazy,
    f.isMemo,
    f.isPortal,
    f.isProfiler,
    f.isStrictMode,
    f.isSuspense
  var d = u(function(e, t) {})
  l(d)
  d.typeOf,
    d.AsyncMode,
    d.ConcurrentMode,
    d.ContextConsumer,
    d.ContextProvider,
    d.Element,
    d.ForwardRef,
    d.Fragment,
    d.Lazy,
    d.Memo,
    d.Portal,
    d.Profiler,
    d.StrictMode,
    d.Suspense,
    d.isValidElementType,
    d.isAsyncMode,
    d.isConcurrentMode,
    d.isContextConsumer,
    d.isContextProvider,
    d.isElement,
    d.isForwardRef,
    d.isFragment,
    d.isLazy,
    d.isMemo,
    d.isPortal,
    d.isProfiler,
    d.isStrictMode,
    d.isSuspense,
    u(function(e) {
      e.exports = f
    })
  var p = Object.getOwnPropertySymbols,
    v = Object.prototype.hasOwnProperty,
    h = Object.prototype.propertyIsEnumerable
  ;(function() {
    try {
      if (!Object.assign) return !1
      var e = new String('abc')
      if (((e[5] = 'de'), '5' === Object.getOwnPropertyNames(e)[0])) return !1
      for (var t = {}, n = 0; n < 10; n++) t['_' + String.fromCharCode(n)] = n
      if (
        '0123456789' !==
        Object.getOwnPropertyNames(t)
          .map(function(e) {
            return t[e]
          })
          .join('')
      )
        return !1
      var r = {}
      return (
        'abcdefghijklmnopqrst'.split('').forEach(function(e) {
          r[e] = e
        }),
        'abcdefghijklmnopqrst' === Object.keys(Object.assign({}, r)).join('')
      )
    } catch (e) {
      return !1
    }
  })() && Object.assign
  var m = 'SECRET_DO_NOT_PASS_THIS_OR_YOU_WILL_BE_FIRED'
  Function.call.bind(Object.prototype.hasOwnProperty)
  function g() {}
  function y() {}
  y.resetWarningCache = g
  var b = u(function(e) {
    e.exports = (function() {
      function e(e, t, n, r, o, a) {
        if (a !== m) {
          var i = new Error(
            'Calling PropTypes validators directly is not supported by the `prop-types` package. Use PropTypes.checkPropTypes() to call them. Read more at http://fb.me/use-check-prop-types'
          )
          throw ((i.name = 'Invariant Violation'), i)
        }
      }
      function t() {
        return e
      }
      e.isRequired = e
      var n = {
        array: e,
        bool: e,
        func: e,
        number: e,
        object: e,
        string: e,
        symbol: e,
        any: e,
        arrayOf: t,
        element: e,
        elementType: e,
        instanceOf: t,
        node: e,
        objectOf: t,
        oneOf: t,
        oneOfType: t,
        shape: t,
        exact: t,
        checkPropTypes: y,
        resetWarningCache: g,
      }
      return (n.PropTypes = n), n
    })()
  })
  function w() {
    return (w =
      Object.assign ||
      function(e) {
        for (var t = 1; t < arguments.length; t++) {
          var n = arguments[t]
          for (var r in n)
            Object.prototype.hasOwnProperty.call(n, r) && (e[r] = n[r])
        }
        return e
      }).apply(this, arguments)
  }
  var E = function(e, t, n, r, o, a, i, s) {
      if (!e) {
        var c
        if (void 0 === t)
          c = new Error(
            'Minified exception occurred; use the non-minified dev environment for the full error message and additional helpful warnings.'
          )
        else {
          var l = [n, r, o, a, i, s],
            u = 0
          ;(c = new Error(
            t.replace(/%s/g, function() {
              return l[u++]
            })
          )).name = 'Invariant Violation'
        }
        throw ((c.framesToPop = 1), c)
      }
    },
    _ = function() {}
  function O(e, t) {
    return void 0 !== e[t]
  }
  function M(e) {
    return 'default' + e.charAt(0).toUpperCase() + e.substr(1)
  }
  function S(e) {
    var t,
      n,
      r = ''
    if (e)
      if ('object' == typeof e)
        if (e.push)
          for (t = 0; t < e.length; t++)
            e[t] && (n = S(e[t])) && (r && (r += ' '), (r += n))
        else for (t in e) e[t] && (n = S(t)) && (r && (r += ' '), (r += n))
      else 'boolean' == typeof e || e.call || (r && (r += ' '), (r += e))
    return r
  }
  function D() {
    for (var e, t = 0, n = ''; t < arguments.length; )
      (e = S(arguments[t++])) && (n && (n += ' '), (n += e))
    return n
  }
  var x = { PREVIOUS: 'PREV', NEXT: 'NEXT', TODAY: 'TODAY', DATE: 'DATE' },
    j = {
      MONTH: 'month',
      WEEK: 'week',
      WORK_WEEK: 'work_week',
      DAY: 'day',
      AGENDA: 'agenda',
    },
    T = Object.keys(j).map(function(e) {
      return j[e]
    })
  b.oneOfType([b.string, b.func]),
    b.any,
    b.func,
    b.oneOfType([
      b.arrayOf(b.oneOf(T)),
      b.objectOf(function(e, t) {
        if (-1 !== T.indexOf(t) && 'boolean' == typeof e[t]) return null
        for (
          var n = arguments.length, r = new Array(n > 2 ? n - 2 : 0), o = 2;
          o < n;
          o++
        )
          r[o - 2] = arguments[o]
        return b.elementType.apply(b, [e, t].concat(r))
      }),
    ]),
    b.oneOfType([b.oneOf(['overlap', 'no-overlap']), b.func])
  function k(e, t) {
    e && e.apply(null, [].concat(t))
  }
  var P = b.oneOfType([b.string, b.func])
  var N = function(e) {
    var t = this
    'function' != typeof e.format && E(!1),
      'function' != typeof e.firstOfWeek && E(!1),
      (this.propType = e.propType || P),
      (this.startOfWeek = e.firstOfWeek),
      (this.formats = e.formats),
      (this.format = function() {
        for (var n = arguments.length, r = new Array(n), o = 0; o < n; o++)
          r[o] = arguments[o]
        return function(e, t, n, r, o) {
          var a = 'function' == typeof r ? r(n, o, e) : t.call(e, n, r, o)
          return null != a && 'string' != typeof a && E(!1), a
        }.apply(void 0, [t, e.format].concat(r))
      })
  }
  function A(e, t, n, r) {
    var o = i({}, e.formats, n)
    return i({}, e, {
      messages: r,
      startOfWeek: function() {
        return e.startOfWeek(t)
      },
      format: function(n, r) {
        return e.format(n, o[r] || r, t)
      },
    })
  }
  var R = {
    date: 'Date',
    time: 'Time',
    event: 'Event',
    allDay: 'All Day',
    week: 'Week',
    work_week: 'Work Week',
    day: 'Day',
    month: 'Month',
    previous: 'Back',
    next: 'Next',
    yesterday: 'Yesterday',
    tomorrow: 'Tomorrow',
    today: 'Today',
    agenda: 'Agenda',
    noEventsInRange: 'There are no events in this range.',
    showMore: function(e) {
      return '+' + e + ' more'
    },
  }
  function C(e) {
    if (void 0 === e)
      throw new ReferenceError(
        "this hasn't been initialised - super() hasn't been called"
      )
    return e
  }
  var L = 'milliseconds',
    F = 'seconds',
    z = 'minutes',
    I = 'hours',
    W = 'day',
    H = 'week',
    V = 'month',
    U = 'year',
    K = 'decade',
    B = 'century'
  function Y(e, t, n) {
    switch (((e = new Date(e)), n)) {
      case L:
        return oe(e, oe(e) + t)
      case F:
        return ae(e, ae(e) + t)
      case z:
        return ie(e, ie(e) + t)
      case I:
        return se(e, se(e) + t)
      case U:
        return fe(e, fe(e) + t)
      case W:
        return le(e, le(e) + t)
      case H:
        return le(e, le(e) + 7 * t)
      case V:
        return (function(e, t) {
          var n = ue(e) + t
          e = ue(e, n)
          for (; n < 0; ) n = 12 + n
          ue(e) !== n % 12 && (e = le(e, 0))
          return e
        })(e, t)
      case K:
        return fe(e, fe(e) + 10 * t)
      case B:
        return fe(e, fe(e) + 100 * t)
    }
    throw new TypeError('Invalid units: "' + n + '"')
  }
  function $(e, t, n) {
    return Y(e, -t, n)
  }
  function X(e, t, n) {
    switch (((e = new Date(e)), t)) {
      case B:
      case K:
      case U:
        e = ue(e, 0)
      case V:
        e = le(e, 1)
      case H:
      case W:
        e = se(e, 0)
      case I:
        e = ie(e, 0)
      case z:
        e = ae(e, 0)
      case F:
        e = oe(e, 0)
    }
    return (
      t === K && (e = $(e, fe(e) % 10, 'year')),
      t === B && (e = $(e, fe(e) % 100, 'year')),
      t === H &&
        (e = (function(e, t, n) {
          var r = (ce(e) + 7 - (n || 0)) % 7
          return void 0 === t ? r : Y(e, t - r, W)
        })(e, 0, n)),
      e
    )
  }
  function q(e, t, n) {
    return (e = $((e = Y((e = X((e = new Date(e)), t, n)), 1, t)), 1, L))
  }
  var G = pe(function(e, t) {
      return e === t
    }),
    Z = pe(function(e, t) {
      return e > t
    }),
    J = pe(function(e, t) {
      return e >= t
    }),
    Q = pe(function(e, t) {
      return e < t
    }),
    ee = pe(function(e, t) {
      return e <= t
    })
  function te() {
    return new Date(Math.min.apply(Math, arguments))
  }
  function ne() {
    return new Date(Math.max.apply(Math, arguments))
  }
  function re(e, t, n, r) {
    return (r = r || 'day'), (!t || J(e, t, r)) && (!n || ee(e, n, r))
  }
  var oe = de('Milliseconds'),
    ae = de('Seconds'),
    ie = de('Minutes'),
    se = de('Hours'),
    ce = de('Day'),
    le = de('Date'),
    ue = de('Month'),
    fe = de('FullYear')
  function de(e) {
    var t = (function(e) {
      switch (e) {
        case 'Milliseconds':
          return 36e5
        case 'Seconds':
          return 3600
        case 'Minutes':
          return 60
        case 'Hours':
          return 1
        default:
          return null
      }
    })(e)
    return function(n, r) {
      if (void 0 === r) return n['get' + e]()
      var o = new Date(n)
      return (
        o['set' + e](r),
        t &&
          o['get' + e]() != r &&
          ('Hours' === e ||
            (r >= t && o.getHours() - n.getHours() < Math.floor(r / t))) &&
          o['set' + e](r + t),
        o
      )
    }
  }
  function pe(e) {
    return function(t, n, r) {
      return e(+X(t, r), +X(n, r))
    }
  }
  var ve = { seconds: 1e3, minutes: 6e4, hours: 36e5, day: 864e5 }
  function he(e, t) {
    var n = X(e, 'month')
    return X(n, 'week', t.startOfWeek())
  }
  function me(e, t) {
    var n = q(e, 'month')
    return q(n, 'week', t.startOfWeek())
  }
  function ge(e, t) {
    var n = X(e, t)
    return G(n, e) ? n : Y(n, 1, t)
  }
  function ye(e, t, n) {
    void 0 === n && (n = 'day')
    for (var r = e, o = []; ee(r, t, n); ) o.push(r), (r = Y(r, 1, n))
    return o
  }
  function be(e, t) {
    return null == t && null == e
      ? null
      : (null == t && (t = new Date()),
        null == e && (e = new Date()),
        (e = X(e, 'day')),
        (e = se(e, se(t))),
        (e = ie(e, ie(t))),
        (e = ae(e, ae(t))),
        oe(e, oe(t)))
  }
  function we(e) {
    return 0 === se(e) && 0 === ie(e) && 0 === ae(e) && 0 === oe(e)
  }
  function Ee(e, t, n) {
    return n && 'milliseconds' !== n
      ? Math.round(Math.abs(+X(e, n) / ve[n] - +X(t, n) / ve[n]))
      : Math.abs(+e - +t)
  }
  function _e(e, t, n) {
    var r = -1,
      o = e.length
    t < 0 && (t = -t > o ? 0 : o + t),
      (n = n > o ? o : n) < 0 && (n += o),
      (o = t > n ? 0 : (n - t) >>> 0),
      (t >>>= 0)
    for (var a = Array(o); ++r < o; ) a[r] = e[r + t]
    return a
  }
  function Oe(e, t) {
    return e === t || (e != e && t != t)
  }
  var Me =
      'object' == typeof global && global && global.Object === Object && global,
    Se = 'object' == typeof self && self && self.Object === Object && self,
    De = Me || Se || Function('return this')(),
    xe = De.Symbol,
    je = Object.prototype,
    Te = je.hasOwnProperty,
    ke = je.toString,
    Pe = xe ? xe.toStringTag : void 0
  var Ne = Object.prototype.toString
  var Ae = '[object Null]',
    Re = '[object Undefined]',
    Ce = xe ? xe.toStringTag : void 0
  function Le(e) {
    return null == e
      ? void 0 === e
        ? Re
        : Ae
      : Ce && Ce in Object(e)
      ? (function(e) {
          var t = Te.call(e, Pe),
            n = e[Pe]
          try {
            e[Pe] = void 0
            var r = !0
          } catch (e) {}
          var o = ke.call(e)
          return r && (t ? (e[Pe] = n) : delete e[Pe]), o
        })(e)
      : (function(e) {
          return Ne.call(e)
        })(e)
  }
  function Fe(e) {
    var t = typeof e
    return null != e && ('object' == t || 'function' == t)
  }
  var ze = '[object AsyncFunction]',
    Ie = '[object Function]',
    We = '[object GeneratorFunction]',
    He = '[object Proxy]'
  function Ve(e) {
    if (!Fe(e)) return !1
    var t = Le(e)
    return t == Ie || t == We || t == ze || t == He
  }
  var Ue = 9007199254740991
  function Ke(e) {
    return 'number' == typeof e && e > -1 && e % 1 == 0 && e <= Ue
  }
  function Be(e) {
    return null != e && Ke(e.length) && !Ve(e)
  }
  var Ye = 9007199254740991,
    $e = /^(?:0|[1-9]\d*)$/
  function Xe(e, t) {
    var n = typeof e
    return (
      !!(t = null == t ? Ye : t) &&
      ('number' == n || ('symbol' != n && $e.test(e))) &&
      e > -1 &&
      e % 1 == 0 &&
      e < t
    )
  }
  function qe(e, t, n) {
    if (!Fe(n)) return !1
    var r = typeof t
    return (
      !!('number' == r ? Be(n) && Xe(t, n.length) : 'string' == r && t in n) &&
      Oe(n[t], e)
    )
  }
  function Ge(e) {
    return null != e && 'object' == typeof e
  }
  var Ze = '[object Symbol]'
  function Je(e) {
    return 'symbol' == typeof e || (Ge(e) && Le(e) == Ze)
  }
  var Qe = NaN,
    et = /^\s+|\s+$/g,
    tt = /^[-+]0x[0-9a-f]+$/i,
    nt = /^0b[01]+$/i,
    rt = /^0o[0-7]+$/i,
    ot = parseInt
  var at = 1 / 0,
    it = 1.7976931348623157e308
  function st(e) {
    return e
      ? (e = (function(e) {
          if ('number' == typeof e) return e
          if (Je(e)) return Qe
          if (Fe(e)) {
            var t = 'function' == typeof e.valueOf ? e.valueOf() : e
            e = Fe(t) ? t + '' : t
          }
          if ('string' != typeof e) return 0 === e ? e : +e
          e = e.replace(et, '')
          var n = nt.test(e)
          return n || rt.test(e)
            ? ot(e.slice(2), n ? 2 : 8)
            : tt.test(e)
            ? Qe
            : +e
        })(e)) === at || e === -at
        ? (e < 0 ? -1 : 1) * it
        : e == e
        ? e
        : 0
      : 0 === e
      ? e
      : 0
  }
  function ct(e) {
    var t = st(e),
      n = t % 1
    return t == t ? (n ? t - n : t) : 0
  }
  var lt = Math.ceil,
    ut = Math.max
  function ft() {
    return (ft =
      Object.assign ||
      function(e) {
        for (var t = 1; t < arguments.length; t++) {
          var n = arguments[t]
          for (var r in n)
            Object.prototype.hasOwnProperty.call(n, r) && (e[r] = n[r])
        }
        return e
      }).apply(this, arguments)
  }
  function dt(e) {
    return (e && e.ownerDocument) || document
  }
  function pt(e, t) {
    return (function(e) {
      var t = dt(e)
      return (t && t.defaultView) || window
    })(e).getComputedStyle(e, t)
  }
  var vt = /([A-Z])/g
  var ht = /^ms-/
  function mt(e) {
    return (function(e) {
      return e.replace(vt, '-$1').toLowerCase()
    })(e).replace(ht, '-ms-')
  }
  var gt = /^((translate|rotate|scale)(X|Y|Z|3d)?|matrix(3d)?|perspective|skew(X|Y)?)$/i
  function yt(e, t) {
    var n = '',
      r = ''
    if ('string' == typeof t)
      return e.style.getPropertyValue(mt(t)) || pt(e).getPropertyValue(mt(t))
    Object.keys(t).forEach(function(o) {
      var a = t[o]
      a || 0 === a
        ? !(function(e) {
            return !(!e || !gt.test(e))
          })(o)
          ? (n += mt(o) + ': ' + a + ';')
          : (r += o + '(' + a + ') ')
        : e.style.removeProperty(mt(o))
    }),
      r && (n += 'transform: ' + r + ';'),
      (e.style.cssText += ';' + n)
  }
  function bt(e, t) {
    return e.contains
      ? e.contains(t)
      : e.compareDocumentPosition
      ? e === t || !!(16 & e.compareDocumentPosition(t))
      : void 0
  }
  function wt(e) {
    return 'window' in e && e.window === e
      ? e
      : ('nodeType' in (t = e) &&
          t.nodeType === document.DOCUMENT_NODE &&
          e.defaultView) ||
          !1
    var t
  }
  function Et(e) {
    var t = 'pageXOffset' === e ? 'scrollLeft' : 'scrollTop'
    return function(n, r) {
      var o = wt(n)
      if (void 0 === r) return o ? o[e] : n[t]
      o ? o.scrollTo(r, o[e]) : (n[t] = r)
    }
  }
  var _t = Et('pageXOffset'),
    Ot = Et('pageYOffset')
  function Mt(e) {
    var t = dt(e),
      n = { top: 0, left: 0, height: 0, width: 0 },
      r = t && t.documentElement
    return r && bt(r, e)
      ? (void 0 !== e.getBoundingClientRect && (n = e.getBoundingClientRect()),
        (n = {
          top: n.top + Ot(e) - (r.clientTop || 0),
          left: n.left + _t(e) - (r.clientLeft || 0),
          width: n.width,
          height: n.height,
        }))
      : n
  }
  var St = function(e) {
    return !!e && 'offsetParent' in e
  }
  var Dt = function(e) {
    return e.nodeName && e.nodeName.toLowerCase()
  }
  function xt(e, t) {
    var n,
      r = { top: 0, left: 0 }
    if ('fixed' === yt(e, 'position')) n = e.getBoundingClientRect()
    else {
      var o =
        t ||
        (function(e) {
          for (
            var t = dt(e), n = e && e.offsetParent;
            St(n) && 'HTML' !== n.nodeName && 'static' === yt(n, 'position');

          )
            n = n.offsetParent
          return n || t.documentElement
        })(e)
      ;(n = Mt(e)), 'html' !== Dt(o) && (r = Mt(o))
      var a = String(yt(o, 'borderTopWidth') || 0)
      r.top += parseInt(a, 10) - Ot(o) || 0
      var i = String(yt(o, 'borderLeftWidth') || 0)
      r.left += parseInt(i, 10) - _t(o) || 0
    }
    var s = String(yt(e, 'marginTop') || 0),
      c = String(yt(e, 'marginLeft') || 0)
    return ft({}, n, {
      top: n.top - r.top - (parseInt(s, 10) || 0),
      left: n.left - r.left - (parseInt(c, 10) || 0),
    })
  }
  var jt = !(
      'undefined' == typeof window ||
      !window.document ||
      !window.document.createElement
    ),
    Tt = new Date().getTime()
  var kt = 'clearTimeout',
    Pt = function(e) {
      var t = new Date().getTime(),
        n = Math.max(0, 16 - (t - Tt)),
        r = setTimeout(e, n)
      return (Tt = t), r
    },
    Nt = function(e, t) {
      return e + (e ? t[0].toUpperCase() + t.substr(1) : t) + 'AnimationFrame'
    }
  jt &&
    ['', 'webkit', 'moz', 'o', 'ms'].some(function(e) {
      var t = Nt(e, 'request')
      return (
        t in window &&
          ((kt = Nt(e, 'cancel')),
          (Pt = function(e) {
            return window[t](e)
          })),
        !!Pt
      )
    })
  var At = function(e) {
      'function' == typeof window[kt] && window[kt](e)
    },
    Rt = Pt,
    Ct = (function(e) {
      function t() {
        return e.apply(this, arguments) || this
      }
      return (
        c(t, e),
        (t.prototype.render = function() {
          var e = this.props,
            t = e.style,
            n = e.className,
            o = e.event,
            a = e.selected,
            c = e.isAllDay,
            l = e.onSelect,
            u = e.onDoubleClick,
            f = e.onKeyPress,
            d = e.localizer,
            p = e.continuesPrior,
            v = e.continuesAfter,
            h = e.accessors,
            m = e.getters,
            g = e.children,
            y = e.components,
            b = y.event,
            w = y.eventWrapper,
            E = e.slotStart,
            _ = e.slotEnd,
            O = s(e, [
              'style',
              'className',
              'event',
              'selected',
              'isAllDay',
              'onSelect',
              'onDoubleClick',
              'onKeyPress',
              'localizer',
              'continuesPrior',
              'continuesAfter',
              'accessors',
              'getters',
              'children',
              'components',
              'slotStart',
              'slotEnd',
            ])
          delete O.resizable
          var M = h.title(o),
            S = h.tooltip(o),
            x = h.end(o),
            j = h.start(o),
            T = h.allDay(o),
            k = c || T || Ee(j, ge(x, 'day'), 'day') > 1,
            P = m.eventProp(o, j, x, a),
            N = r.createElement(
              'div',
              { className: 'rbc-event-content', title: S || void 0 },
              b
                ? r.createElement(b, {
                    event: o,
                    continuesPrior: p,
                    continuesAfter: v,
                    title: M,
                    isAllDay: T,
                    localizer: d,
                    slotStart: E,
                    slotEnd: _,
                  })
                : M
            )
          return r.createElement(
            w,
            i({}, this.props, { type: 'date' }),
            r.createElement(
              'div',
              i({}, O, {
                tabIndex: 0,
                style: i({}, P.style, t),
                className: D('rbc-event', n, P.className, {
                  'rbc-selected': a,
                  'rbc-event-allday': k,
                  'rbc-event-continues-prior': p,
                  'rbc-event-continues-after': v,
                }),
                onClick: function(e) {
                  return l && l(o, e)
                },
                onDoubleClick: function(e) {
                  return u && u(o, e)
                },
                onKeyPress: function(e) {
                  return f && f(o, e)
                },
              }),
              'function' == typeof g ? g(N) : N
            )
          )
        }),
        t
      )
    })(r.Component)
  function Lt(e, t) {
    return !(!e || null == t) && -1 !== [].concat(t).indexOf(e)
  }
  function Ft(e, t) {
    return (e.right - e.left) / t
  }
  function zt(e, t, n, r) {
    var o = Ft(e, r)
    return n
      ? r - 1 - Math.floor((t - e.left) / o)
      : Math.floor((t - e.left) / o)
  }
  Ct.propTypes = {}
  var It = (function(e) {
    function t() {
      return e.apply(this, arguments) || this
    }
    c(t, e)
    var n = t.prototype
    return (
      (n.componentDidMount = function() {
        var e,
          t,
          n = this.props,
          r = n.popupOffset,
          o = void 0 === r ? 5 : r,
          a = Mt(n.popperRef.current),
          i = a.top,
          s = a.left,
          c = a.width,
          l = a.height,
          u = window.innerHeight + Ot(window),
          f = window.innerWidth + _t(window),
          d = i + l,
          p = s + c
        ;(d > u || p > f) &&
          (d > u && (e = d - u + (o.y || +o || 0)),
          p > f && (t = p - f + (o.x || +o || 0)),
          this.setState({ topOffset: e, leftOffset: t }))
      }),
      (n.render = function() {
        var e = this,
          t = this.props,
          n = t.events,
          o = t.selected,
          a = t.getters,
          s = t.accessors,
          c = t.components,
          l = t.onSelect,
          u = t.onDoubleClick,
          f = t.onKeyPress,
          d = t.slotStart,
          p = t.slotEnd,
          v = t.localizer,
          h = t.popperRef,
          m = this.props.position.width,
          g = {
            top: -((this.state || {}).topOffset || 0),
            left: -((this.state || {}).leftOffset || 0),
            minWidth: m + m / 2,
          }
        return r.createElement(
          'div',
          {
            style: i({}, this.props.style, g),
            className: 'rbc-overlay',
            ref: h,
          },
          r.createElement(
            'div',
            { className: 'rbc-overlay-header' },
            v.format(d, 'dayHeaderFormat')
          ),
          n.map(function(t, n) {
            return r.createElement(Ct, {
              key: n,
              type: 'popup',
              event: t,
              getters: a,
              onSelect: l,
              accessors: s,
              components: c,
              onDoubleClick: u,
              onKeyPress: f,
              continuesPrior: Q(s.end(t), d, 'day'),
              continuesAfter: J(s.start(t), p, 'day'),
              slotStart: d,
              slotEnd: p,
              selected: Lt(t, o),
              draggable: !0,
              onDragStart: function() {
                return e.props.handleDragStart(t)
              },
              onDragEnd: function() {
                return e.props.show()
              },
            })
          })
        )
      }),
      t
    )
  })(r.Component)
  It.propTypes = {}
  var Wt = r.forwardRef(function(e, t) {
    return r.createElement(It, i({ popperRef: t }, e))
  })
  function Ht() {
    return (Ht =
      Object.assign ||
      function(e) {
        for (var t = 1; t < arguments.length; t++) {
          var n = arguments[t]
          for (var r in n)
            Object.prototype.hasOwnProperty.call(n, r) && (e[r] = n[r])
        }
        return e
      }).apply(this, arguments)
  }
  function Vt(e, t) {
    if (null == e) return {}
    var n,
      r,
      o = {},
      a = Object.keys(e)
    for (r = 0; r < a.length; r++)
      (n = a[r]), t.indexOf(n) >= 0 || (o[n] = e[n])
    return o
  }
  function Ut() {
    return t.useState(null)
  }
  var Kt = function(e) {
    return e && 'function' != typeof e
      ? function(t) {
          e.current = t
        }
      : e
  }
  function Bt(e, n) {
    return t.useMemo(
      function() {
        return (function(e, t) {
          var n = Kt(e),
            r = Kt(t)
          return function(e) {
            n && n(e), r && r(e)
          }
        })(e, n)
      },
      [e, n]
    )
  }
  var Yt = 'top',
    $t = 'bottom',
    Xt = 'right',
    qt = 'left',
    Gt = 'auto',
    Zt = [Yt, $t, Xt, qt],
    Jt = 'start',
    Qt = 'end',
    en = 'clippingParents',
    tn = 'viewport',
    nn = 'popper',
    rn = 'reference',
    on = Zt.reduce(function(e, t) {
      return e.concat([t + '-' + Jt, t + '-' + Qt])
    }, []),
    an = [].concat(Zt, [Gt]).reduce(function(e, t) {
      return e.concat([t, t + '-' + Jt, t + '-' + Qt])
    }, []),
    sn = [
      'beforeRead',
      'read',
      'afterRead',
      'beforeMain',
      'main',
      'afterMain',
      'beforeWrite',
      'write',
      'afterWrite',
    ]
  function cn(e) {
    return e.split('-')[0]
  }
  function ln(e) {
    return {
      x: e.offsetLeft,
      y: e.offsetTop,
      width: e.offsetWidth,
      height: e.offsetHeight,
    }
  }
  function un(e) {
    if ('[object Window]' !== e.toString()) {
      var t = e.ownerDocument
      return (t && t.defaultView) || window
    }
    return e
  }
  function fn(e) {
    return e instanceof un(e).Element || e instanceof Element
  }
  function dn(e) {
    return e instanceof un(e).HTMLElement || e instanceof HTMLElement
  }
  function pn(e, t) {
    var n,
      r = t.getRootNode && t.getRootNode()
    if (e.contains(t)) return !0
    if (r && ((n = r) instanceof un(n).ShadowRoot || n instanceof ShadowRoot)) {
      var o = t
      do {
        if (o && e.isSameNode(o)) return !0
        o = o.parentNode || o.host
      } while (o)
    }
    return !1
  }
  function vn(e) {
    return e ? (e.nodeName || '').toLowerCase() : null
  }
  function hn(e) {
    return un(e).getComputedStyle(e)
  }
  function mn(e) {
    return ['table', 'td', 'th'].indexOf(vn(e)) >= 0
  }
  function gn(e) {
    return (
      (fn(e) ? e.ownerDocument : e.document) || window.document
    ).documentElement
  }
  function yn(e) {
    return 'html' === vn(e)
      ? e
      : e.assignedSlot || e.parentNode || e.host || gn(e)
  }
  function bn(e) {
    if (!dn(e) || 'fixed' === hn(e).position) return null
    var t = e.offsetParent
    if (t) {
      var n = gn(t)
      if (
        'body' === vn(t) &&
        'static' === hn(t).position &&
        'static' !== hn(n).position
      )
        return n
    }
    return t
  }
  function wn(e) {
    for (var t = un(e), n = bn(e); n && mn(n) && 'static' === hn(n).position; )
      n = bn(n)
    return n && 'body' === vn(n) && 'static' === hn(n).position
      ? t
      : n ||
          (function(e) {
            for (
              var t = yn(e);
              dn(t) && ['html', 'body'].indexOf(vn(t)) < 0;

            ) {
              var n = hn(t)
              if (
                'none' !== n.transform ||
                'none' !== n.perspective ||
                (n.willChange && 'auto' !== n.willChange)
              )
                return t
              t = t.parentNode
            }
            return null
          })(e) ||
          t
  }
  function En(e) {
    return ['top', 'bottom'].indexOf(e) >= 0 ? 'x' : 'y'
  }
  function _n(e, t, n) {
    return Math.max(e, Math.min(t, n))
  }
  function On(e) {
    return Object.assign(
      Object.assign({}, { top: 0, right: 0, bottom: 0, left: 0 }),
      e
    )
  }
  function Mn(e, t) {
    return t.reduce(function(t, n) {
      return (t[n] = e), t
    }, {})
  }
  var Sn = {
      name: 'arrow',
      enabled: !0,
      phase: 'main',
      fn: function(e) {
        var t,
          n = e.state,
          r = e.name,
          o = n.elements.arrow,
          a = n.modifiersData.popperOffsets,
          i = cn(n.placement),
          s = En(i),
          c = [qt, Xt].indexOf(i) >= 0 ? 'height' : 'width'
        if (o && a) {
          var l = n.modifiersData[r + '#persistent'].padding,
            u = ln(o),
            f = 'y' === s ? Yt : qt,
            d = 'y' === s ? $t : Xt,
            p =
              n.rects.reference[c] +
              n.rects.reference[s] -
              a[s] -
              n.rects.popper[c],
            v = a[s] - n.rects.reference[s],
            h = wn(o),
            m = h ? ('y' === s ? h.clientHeight || 0 : h.clientWidth || 0) : 0,
            g = p / 2 - v / 2,
            y = l[f],
            b = m - u[c] - l[d],
            w = m / 2 - u[c] / 2 + g,
            E = _n(y, w, b),
            _ = s
          n.modifiersData[r] = (((t = {})[_] = E), (t.centerOffset = E - w), t)
        }
      },
      effect: function(e) {
        var t = e.state,
          n = e.options,
          r = e.name,
          o = n.element,
          a = void 0 === o ? '[data-popper-arrow]' : o,
          i = n.padding,
          s = void 0 === i ? 0 : i
        null != a &&
          ('string' != typeof a || (a = t.elements.popper.querySelector(a))) &&
          pn(t.elements.popper, a) &&
          ((t.elements.arrow = a),
          (t.modifiersData[r + '#persistent'] = {
            padding: On('number' != typeof s ? s : Mn(s, Zt)),
          }))
      },
      requires: ['popperOffsets'],
      requiresIfExists: ['preventOverflow'],
    },
    Dn = Math.round,
    xn = { top: 'auto', right: 'auto', bottom: 'auto', left: 'auto' }
  function jn(e) {
    var t,
      n = e.popper,
      r = e.popperRect,
      o = e.placement,
      a = e.offsets,
      i = e.position,
      s = e.gpuAcceleration,
      c = e.adaptive,
      l = e.roundOffsets,
      u =
        !0 === l
          ? (function(e) {
              var t = e.x,
                n = e.y,
                r = window.devicePixelRatio || 1
              return { x: Dn(Dn(t * r) / r) || 0, y: Dn(Dn(n * r) / r) || 0 }
            })(a)
          : 'function' == typeof l
          ? l(a)
          : a,
      f = u.x,
      d = void 0 === f ? 0 : f,
      p = u.y,
      v = void 0 === p ? 0 : p,
      h = a.hasOwnProperty('x'),
      m = a.hasOwnProperty('y'),
      g = qt,
      y = Yt,
      b = window
    if (c) {
      var w = wn(n)
      w === un(n) && (w = gn(n)),
        o === Yt &&
          ((y = $t), (v -= w.clientHeight - r.height), (v *= s ? 1 : -1)),
        o === qt &&
          ((g = Xt), (d -= w.clientWidth - r.width), (d *= s ? 1 : -1))
    }
    var E,
      _ = Object.assign({ position: i }, c && xn)
    return s
      ? Object.assign(
          Object.assign({}, _),
          {},
          (((E = {})[y] = m ? '0' : ''),
          (E[g] = h ? '0' : ''),
          (E.transform =
            (b.devicePixelRatio || 1) < 2
              ? 'translate(' + d + 'px, ' + v + 'px)'
              : 'translate3d(' + d + 'px, ' + v + 'px, 0)'),
          E)
        )
      : Object.assign(
          Object.assign({}, _),
          {},
          (((t = {})[y] = m ? v + 'px' : ''),
          (t[g] = h ? d + 'px' : ''),
          (t.transform = ''),
          t)
        )
  }
  var Tn = { passive: !0 }
  var kn = { left: 'right', right: 'left', bottom: 'top', top: 'bottom' }
  function Pn(e) {
    return e.replace(/left|right|bottom|top/g, function(e) {
      return kn[e]
    })
  }
  var Nn = { start: 'end', end: 'start' }
  function An(e) {
    return e.replace(/start|end/g, function(e) {
      return Nn[e]
    })
  }
  function Rn(e) {
    var t = e.getBoundingClientRect()
    return {
      width: t.width,
      height: t.height,
      top: t.top,
      right: t.right,
      bottom: t.bottom,
      left: t.left,
      x: t.left,
      y: t.top,
    }
  }
  function Cn(e) {
    var t = un(e)
    return { scrollLeft: t.pageXOffset, scrollTop: t.pageYOffset }
  }
  function Ln(e) {
    return Rn(gn(e)).left + Cn(e).scrollLeft
  }
  function Fn(e) {
    var t = hn(e),
      n = t.overflow,
      r = t.overflowX,
      o = t.overflowY
    return /auto|scroll|overlay|hidden/.test(n + o + r)
  }
  function zn(e, t) {
    void 0 === t && (t = [])
    var n = (function e(t) {
        return ['html', 'body', '#document'].indexOf(vn(t)) >= 0
          ? t.ownerDocument.body
          : dn(t) && Fn(t)
          ? t
          : e(yn(t))
      })(e),
      r = n === e.ownerDocument.body,
      o = un(n),
      a = r ? [o].concat(o.visualViewport || [], Fn(n) ? n : []) : n,
      i = t.concat(a)
    return r ? i : i.concat(zn(yn(a)))
  }
  function In(e) {
    return Object.assign(
      Object.assign({}, e),
      {},
      { left: e.x, top: e.y, right: e.x + e.width, bottom: e.y + e.height }
    )
  }
  function Wn(e, t) {
    return t === tn
      ? In(
          (function(e) {
            var t = un(e),
              n = gn(e),
              r = t.visualViewport,
              o = n.clientWidth,
              a = n.clientHeight,
              i = 0,
              s = 0
            return (
              r &&
                ((o = r.width),
                (a = r.height),
                /^((?!chrome|android).)*safari/i.test(navigator.userAgent) ||
                  ((i = r.offsetLeft), (s = r.offsetTop))),
              { width: o, height: a, x: i + Ln(e), y: s }
            )
          })(e)
        )
      : dn(t)
      ? (function(e) {
          var t = Rn(e)
          return (
            (t.top = t.top + e.clientTop),
            (t.left = t.left + e.clientLeft),
            (t.bottom = t.top + e.clientHeight),
            (t.right = t.left + e.clientWidth),
            (t.width = e.clientWidth),
            (t.height = e.clientHeight),
            (t.x = t.left),
            (t.y = t.top),
            t
          )
        })(t)
      : In(
          (function(e) {
            var t = gn(e),
              n = Cn(e),
              r = e.ownerDocument.body,
              o = Math.max(
                t.scrollWidth,
                t.clientWidth,
                r ? r.scrollWidth : 0,
                r ? r.clientWidth : 0
              ),
              a = Math.max(
                t.scrollHeight,
                t.clientHeight,
                r ? r.scrollHeight : 0,
                r ? r.clientHeight : 0
              ),
              i = -n.scrollLeft + Ln(e),
              s = -n.scrollTop
            return (
              'rtl' === hn(r || t).direction &&
                (i += Math.max(t.clientWidth, r ? r.clientWidth : 0) - o),
              { width: o, height: a, x: i, y: s }
            )
          })(gn(e))
        )
  }
  function Hn(e, t, n) {
    var r =
        'clippingParents' === t
          ? (function(e) {
              var t = zn(yn(e)),
                n =
                  ['absolute', 'fixed'].indexOf(hn(e).position) >= 0 && dn(e)
                    ? wn(e)
                    : e
              return fn(n)
                ? t.filter(function(e) {
                    return fn(e) && pn(e, n) && 'body' !== vn(e)
                  })
                : []
            })(e)
          : [].concat(t),
      o = [].concat(r, [n]),
      a = o[0],
      i = o.reduce(function(t, n) {
        var r = Wn(e, n)
        return (
          (t.top = Math.max(r.top, t.top)),
          (t.right = Math.min(r.right, t.right)),
          (t.bottom = Math.min(r.bottom, t.bottom)),
          (t.left = Math.max(r.left, t.left)),
          t
        )
      }, Wn(e, a))
    return (
      (i.width = i.right - i.left),
      (i.height = i.bottom - i.top),
      (i.x = i.left),
      (i.y = i.top),
      i
    )
  }
  function Vn(e) {
    return e.split('-')[1]
  }
  function Un(e) {
    var t,
      n = e.reference,
      r = e.element,
      o = e.placement,
      a = o ? cn(o) : null,
      i = o ? Vn(o) : null,
      s = n.x + n.width / 2 - r.width / 2,
      c = n.y + n.height / 2 - r.height / 2
    switch (a) {
      case Yt:
        t = { x: s, y: n.y - r.height }
        break
      case $t:
        t = { x: s, y: n.y + n.height }
        break
      case Xt:
        t = { x: n.x + n.width, y: c }
        break
      case qt:
        t = { x: n.x - r.width, y: c }
        break
      default:
        t = { x: n.x, y: n.y }
    }
    var l = a ? En(a) : null
    if (null != l) {
      var u = 'y' === l ? 'height' : 'width'
      switch (i) {
        case Jt:
          t[l] = t[l] - (n[u] / 2 - r[u] / 2)
          break
        case Qt:
          t[l] = t[l] + (n[u] / 2 - r[u] / 2)
      }
    }
    return t
  }
  function Kn(e, t) {
    void 0 === t && (t = {})
    var n = t,
      r = n.placement,
      o = void 0 === r ? e.placement : r,
      a = n.boundary,
      i = void 0 === a ? en : a,
      s = n.rootBoundary,
      c = void 0 === s ? tn : s,
      l = n.elementContext,
      u = void 0 === l ? nn : l,
      f = n.altBoundary,
      d = void 0 !== f && f,
      p = n.padding,
      v = void 0 === p ? 0 : p,
      h = On('number' != typeof v ? v : Mn(v, Zt)),
      m = u === nn ? rn : nn,
      g = e.elements.reference,
      y = e.rects.popper,
      b = e.elements[d ? m : u],
      w = Hn(fn(b) ? b : b.contextElement || gn(e.elements.popper), i, c),
      E = Rn(g),
      _ = Un({ reference: E, element: y, strategy: 'absolute', placement: o }),
      O = In(Object.assign(Object.assign({}, y), _)),
      M = u === nn ? O : E,
      S = {
        top: w.top - M.top + h.top,
        bottom: M.bottom - w.bottom + h.bottom,
        left: w.left - M.left + h.left,
        right: M.right - w.right + h.right,
      },
      D = e.modifiersData.offset
    if (u === nn && D) {
      var x = D[o]
      Object.keys(S).forEach(function(e) {
        var t = [Xt, $t].indexOf(e) >= 0 ? 1 : -1,
          n = [Yt, $t].indexOf(e) >= 0 ? 'y' : 'x'
        S[e] += x[n] * t
      })
    }
    return S
  }
  function Bn(e, t) {
    void 0 === t && (t = {})
    var n = t,
      r = n.placement,
      o = n.boundary,
      a = n.rootBoundary,
      i = n.padding,
      s = n.flipVariations,
      c = n.allowedAutoPlacements,
      l = void 0 === c ? an : c,
      u = Vn(r),
      f = u
        ? s
          ? on
          : on.filter(function(e) {
              return Vn(e) === u
            })
        : Zt,
      d = f.filter(function(e) {
        return l.indexOf(e) >= 0
      })
    0 === d.length && (d = f)
    var p = d.reduce(function(t, n) {
      return (
        (t[n] = Kn(e, {
          placement: n,
          boundary: o,
          rootBoundary: a,
          padding: i,
        })[cn(n)]),
        t
      )
    }, {})
    return Object.keys(p).sort(function(e, t) {
      return p[e] - p[t]
    })
  }
  function Yn(e, t, n) {
    return (
      void 0 === n && (n = { x: 0, y: 0 }),
      {
        top: e.top - t.height - n.y,
        right: e.right - t.width + n.x,
        bottom: e.bottom - t.height + n.y,
        left: e.left - t.width - n.x,
      }
    )
  }
  function $n(e) {
    return [Yt, Xt, $t, qt].some(function(t) {
      return e[t] >= 0
    })
  }
  function Xn(e, t, n) {
    void 0 === n && (n = !1)
    var r,
      o,
      a = gn(t),
      i = Rn(e),
      s = dn(t),
      c = { scrollLeft: 0, scrollTop: 0 },
      l = { x: 0, y: 0 }
    return (
      (s || (!s && !n)) &&
        (('body' !== vn(t) || Fn(a)) &&
          (c =
            (r = t) !== un(r) && dn(r)
              ? { scrollLeft: (o = r).scrollLeft, scrollTop: o.scrollTop }
              : Cn(r)),
        dn(t)
          ? (((l = Rn(t)).x += t.clientLeft), (l.y += t.clientTop))
          : a && (l.x = Ln(a))),
      {
        x: i.left + c.scrollLeft - l.x,
        y: i.top + c.scrollTop - l.y,
        width: i.width,
        height: i.height,
      }
    )
  }
  function qn(e) {
    var t = new Map(),
      n = new Set(),
      r = []
    return (
      e.forEach(function(e) {
        t.set(e.name, e)
      }),
      e.forEach(function(e) {
        n.has(e.name) ||
          (function e(o) {
            n.add(o.name),
              []
                .concat(o.requires || [], o.requiresIfExists || [])
                .forEach(function(r) {
                  if (!n.has(r)) {
                    var o = t.get(r)
                    o && e(o)
                  }
                }),
              r.push(o)
          })(e)
      }),
      r
    )
  }
  var Gn = { placement: 'bottom', modifiers: [], strategy: 'absolute' }
  function Zn() {
    for (var e = arguments.length, t = new Array(e), n = 0; n < e; n++)
      t[n] = arguments[n]
    return !t.some(function(e) {
      return !(e && 'function' == typeof e.getBoundingClientRect)
    })
  }
  var Jn = (function(e) {
    void 0 === e && (e = {})
    var t = e,
      n = t.defaultModifiers,
      r = void 0 === n ? [] : n,
      o = t.defaultOptions,
      a = void 0 === o ? Gn : o
    return function(e, t, n) {
      void 0 === n && (n = a)
      var o,
        i,
        s = {
          placement: 'bottom',
          orderedModifiers: [],
          options: Object.assign(Object.assign({}, Gn), a),
          modifiersData: {},
          elements: { reference: e, popper: t },
          attributes: {},
          styles: {},
        },
        c = [],
        l = !1,
        u = {
          state: s,
          setOptions: function(n) {
            f(),
              (s.options = Object.assign(
                Object.assign(Object.assign({}, a), s.options),
                n
              )),
              (s.scrollParents = {
                reference: fn(e)
                  ? zn(e)
                  : e.contextElement
                  ? zn(e.contextElement)
                  : [],
                popper: zn(t),
              })
            var o,
              i,
              l = (function(e) {
                var t = qn(e)
                return sn.reduce(function(e, n) {
                  return e.concat(
                    t.filter(function(e) {
                      return e.phase === n
                    })
                  )
                }, [])
              })(
                ((o = [].concat(r, s.options.modifiers)),
                (i = o.reduce(function(e, t) {
                  var n = e[t.name]
                  return (
                    (e[t.name] = n
                      ? Object.assign(
                          Object.assign(Object.assign({}, n), t),
                          {},
                          {
                            options: Object.assign(
                              Object.assign({}, n.options),
                              t.options
                            ),
                            data: Object.assign(
                              Object.assign({}, n.data),
                              t.data
                            ),
                          }
                        )
                      : t),
                    e
                  )
                }, {})),
                Object.keys(i).map(function(e) {
                  return i[e]
                }))
              )
            return (
              (s.orderedModifiers = l.filter(function(e) {
                return e.enabled
              })),
              s.orderedModifiers.forEach(function(e) {
                var t = e.name,
                  n = e.options,
                  r = void 0 === n ? {} : n,
                  o = e.effect
                if ('function' == typeof o) {
                  var a = o({ state: s, name: t, instance: u, options: r })
                  c.push(a || function() {})
                }
              }),
              u.update()
            )
          },
          forceUpdate: function() {
            if (!l) {
              var e = s.elements,
                t = e.reference,
                n = e.popper
              if (Zn(t, n)) {
                ;(s.rects = {
                  reference: Xn(t, wn(n), 'fixed' === s.options.strategy),
                  popper: ln(n),
                }),
                  (s.reset = !1),
                  (s.placement = s.options.placement),
                  s.orderedModifiers.forEach(function(e) {
                    return (s.modifiersData[e.name] = Object.assign({}, e.data))
                  })
                for (var r = 0; r < s.orderedModifiers.length; r++)
                  if (!0 !== s.reset) {
                    var o = s.orderedModifiers[r],
                      a = o.fn,
                      i = o.options,
                      c = void 0 === i ? {} : i,
                      f = o.name
                    'function' == typeof a &&
                      (s =
                        a({ state: s, options: c, name: f, instance: u }) || s)
                  } else (s.reset = !1), (r = -1)
              }
            }
          },
          update: ((o = function() {
            return new Promise(function(e) {
              u.forceUpdate(), e(s)
            })
          }),
          function() {
            return (
              i ||
                (i = new Promise(function(e) {
                  Promise.resolve().then(function() {
                    ;(i = void 0), e(o())
                  })
                })),
              i
            )
          }),
          destroy: function() {
            f(), (l = !0)
          },
        }
      if (!Zn(e, t)) return u
      function f() {
        c.forEach(function(e) {
          return e()
        }),
          (c = [])
      }
      return (
        u.setOptions(n).then(function(e) {
          !l && n.onFirstUpdate && n.onFirstUpdate(e)
        }),
        u
      )
    }
  })({
    defaultModifiers: [
      {
        name: 'hide',
        enabled: !0,
        phase: 'main',
        requiresIfExists: ['preventOverflow'],
        fn: function(e) {
          var t = e.state,
            n = e.name,
            r = t.rects.reference,
            o = t.rects.popper,
            a = t.modifiersData.preventOverflow,
            i = Kn(t, { elementContext: 'reference' }),
            s = Kn(t, { altBoundary: !0 }),
            c = Yn(i, r),
            l = Yn(s, o, a),
            u = $n(c),
            f = $n(l)
          ;(t.modifiersData[n] = {
            referenceClippingOffsets: c,
            popperEscapeOffsets: l,
            isReferenceHidden: u,
            hasPopperEscaped: f,
          }),
            (t.attributes.popper = Object.assign(
              Object.assign({}, t.attributes.popper),
              {},
              { 'data-popper-reference-hidden': u, 'data-popper-escaped': f }
            ))
        },
      },
      {
        name: 'popperOffsets',
        enabled: !0,
        phase: 'read',
        fn: function(e) {
          var t = e.state,
            n = e.name
          t.modifiersData[n] = Un({
            reference: t.rects.reference,
            element: t.rects.popper,
            strategy: 'absolute',
            placement: t.placement,
          })
        },
        data: {},
      },
      {
        name: 'computeStyles',
        enabled: !0,
        phase: 'beforeWrite',
        fn: function(e) {
          var t = e.state,
            n = e.options,
            r = n.gpuAcceleration,
            o = void 0 === r || r,
            a = n.adaptive,
            i = void 0 === a || a,
            s = n.roundOffsets,
            c = void 0 === s || s,
            l = {
              placement: cn(t.placement),
              popper: t.elements.popper,
              popperRect: t.rects.popper,
              gpuAcceleration: o,
            }
          null != t.modifiersData.popperOffsets &&
            (t.styles.popper = Object.assign(
              Object.assign({}, t.styles.popper),
              jn(
                Object.assign(
                  Object.assign({}, l),
                  {},
                  {
                    offsets: t.modifiersData.popperOffsets,
                    position: t.options.strategy,
                    adaptive: i,
                    roundOffsets: c,
                  }
                )
              )
            )),
            null != t.modifiersData.arrow &&
              (t.styles.arrow = Object.assign(
                Object.assign({}, t.styles.arrow),
                jn(
                  Object.assign(
                    Object.assign({}, l),
                    {},
                    {
                      offsets: t.modifiersData.arrow,
                      position: 'absolute',
                      adaptive: !1,
                      roundOffsets: c,
                    }
                  )
                )
              )),
            (t.attributes.popper = Object.assign(
              Object.assign({}, t.attributes.popper),
              {},
              { 'data-popper-placement': t.placement }
            ))
        },
        data: {},
      },
      {
        name: 'eventListeners',
        enabled: !0,
        phase: 'write',
        fn: function() {},
        effect: function(e) {
          var t = e.state,
            n = e.instance,
            r = e.options,
            o = r.scroll,
            a = void 0 === o || o,
            i = r.resize,
            s = void 0 === i || i,
            c = un(t.elements.popper),
            l = [].concat(t.scrollParents.reference, t.scrollParents.popper)
          return (
            a &&
              l.forEach(function(e) {
                e.addEventListener('scroll', n.update, Tn)
              }),
            s && c.addEventListener('resize', n.update, Tn),
            function() {
              a &&
                l.forEach(function(e) {
                  e.removeEventListener('scroll', n.update, Tn)
                }),
                s && c.removeEventListener('resize', n.update, Tn)
            }
          )
        },
        data: {},
      },
      {
        name: 'offset',
        enabled: !0,
        phase: 'main',
        requires: ['popperOffsets'],
        fn: function(e) {
          var t = e.state,
            n = e.options,
            r = e.name,
            o = n.offset,
            a = void 0 === o ? [0, 0] : o,
            i = an.reduce(function(e, n) {
              return (
                (e[n] = (function(e, t, n) {
                  var r = cn(e),
                    o = [qt, Yt].indexOf(r) >= 0 ? -1 : 1,
                    a =
                      'function' == typeof n
                        ? n(
                            Object.assign(
                              Object.assign({}, t),
                              {},
                              { placement: e }
                            )
                          )
                        : n,
                    i = a[0],
                    s = a[1]
                  return (
                    (i = i || 0),
                    (s = (s || 0) * o),
                    [qt, Xt].indexOf(r) >= 0 ? { x: s, y: i } : { x: i, y: s }
                  )
                })(n, t.rects, a)),
                e
              )
            }, {}),
            s = i[t.placement],
            c = s.x,
            l = s.y
          null != t.modifiersData.popperOffsets &&
            ((t.modifiersData.popperOffsets.x += c),
            (t.modifiersData.popperOffsets.y += l)),
            (t.modifiersData[r] = i)
        },
      },
      {
        name: 'flip',
        enabled: !0,
        phase: 'main',
        fn: function(e) {
          var t = e.state,
            n = e.options,
            r = e.name
          if (!t.modifiersData[r]._skip) {
            for (
              var o = n.mainAxis,
                a = void 0 === o || o,
                i = n.altAxis,
                s = void 0 === i || i,
                c = n.fallbackPlacements,
                l = n.padding,
                u = n.boundary,
                f = n.rootBoundary,
                d = n.altBoundary,
                p = n.flipVariations,
                v = void 0 === p || p,
                h = n.allowedAutoPlacements,
                m = t.options.placement,
                g = cn(m),
                y =
                  c ||
                  (g !== m && v
                    ? (function(e) {
                        if (cn(e) === Gt) return []
                        var t = Pn(e)
                        return [An(e), t, An(t)]
                      })(m)
                    : [Pn(m)]),
                b = [m].concat(y).reduce(function(e, n) {
                  return e.concat(
                    cn(n) === Gt
                      ? Bn(t, {
                          placement: n,
                          boundary: u,
                          rootBoundary: f,
                          padding: l,
                          flipVariations: v,
                          allowedAutoPlacements: h,
                        })
                      : n
                  )
                }, []),
                w = t.rects.reference,
                E = t.rects.popper,
                _ = new Map(),
                O = !0,
                M = b[0],
                S = 0;
              S < b.length;
              S++
            ) {
              var D = b[S],
                x = cn(D),
                j = Vn(D) === Jt,
                T = [Yt, $t].indexOf(x) >= 0,
                k = T ? 'width' : 'height',
                P = Kn(t, {
                  placement: D,
                  boundary: u,
                  rootBoundary: f,
                  altBoundary: d,
                  padding: l,
                }),
                N = T ? (j ? Xt : qt) : j ? $t : Yt
              w[k] > E[k] && (N = Pn(N))
              var A = Pn(N),
                R = []
              if (
                (a && R.push(P[x] <= 0),
                s && R.push(P[N] <= 0, P[A] <= 0),
                R.every(function(e) {
                  return e
                }))
              ) {
                ;(M = D), (O = !1)
                break
              }
              _.set(D, R)
            }
            if (O)
              for (
                var C = function(e) {
                    var t = b.find(function(t) {
                      var n = _.get(t)
                      if (n)
                        return n.slice(0, e).every(function(e) {
                          return e
                        })
                    })
                    if (t) return (M = t), 'break'
                  },
                  L = v ? 3 : 1;
                L > 0 && 'break' !== C(L);
                L--
              );
            t.placement !== M &&
              ((t.modifiersData[r]._skip = !0),
              (t.placement = M),
              (t.reset = !0))
          }
        },
        requiresIfExists: ['offset'],
        data: { _skip: !1 },
      },
      {
        name: 'preventOverflow',
        enabled: !0,
        phase: 'main',
        fn: function(e) {
          var t = e.state,
            n = e.options,
            r = e.name,
            o = n.mainAxis,
            a = void 0 === o || o,
            i = n.altAxis,
            s = void 0 !== i && i,
            c = n.boundary,
            l = n.rootBoundary,
            u = n.altBoundary,
            f = n.padding,
            d = n.tether,
            p = void 0 === d || d,
            v = n.tetherOffset,
            h = void 0 === v ? 0 : v,
            m = Kn(t, {
              boundary: c,
              rootBoundary: l,
              padding: f,
              altBoundary: u,
            }),
            g = cn(t.placement),
            y = Vn(t.placement),
            b = !y,
            w = En(g),
            E = 'x' === w ? 'y' : 'x',
            _ = t.modifiersData.popperOffsets,
            O = t.rects.reference,
            M = t.rects.popper,
            S =
              'function' == typeof h
                ? h(
                    Object.assign(
                      Object.assign({}, t.rects),
                      {},
                      { placement: t.placement }
                    )
                  )
                : h,
            D = { x: 0, y: 0 }
          if (_) {
            if (a) {
              var x = 'y' === w ? Yt : qt,
                j = 'y' === w ? $t : Xt,
                T = 'y' === w ? 'height' : 'width',
                k = _[w],
                P = _[w] + m[x],
                N = _[w] - m[j],
                A = p ? -M[T] / 2 : 0,
                R = y === Jt ? O[T] : M[T],
                C = y === Jt ? -M[T] : -O[T],
                L = t.elements.arrow,
                F = p && L ? ln(L) : { width: 0, height: 0 },
                z = t.modifiersData['arrow#persistent']
                  ? t.modifiersData['arrow#persistent'].padding
                  : { top: 0, right: 0, bottom: 0, left: 0 },
                I = z[x],
                W = z[j],
                H = _n(0, O[T], F[T]),
                V = b ? O[T] / 2 - A - H - I - S : R - H - I - S,
                U = b ? -O[T] / 2 + A + H + W + S : C + H + W + S,
                K = t.elements.arrow && wn(t.elements.arrow),
                B = K ? ('y' === w ? K.clientTop || 0 : K.clientLeft || 0) : 0,
                Y = t.modifiersData.offset
                  ? t.modifiersData.offset[t.placement][w]
                  : 0,
                $ = _[w] + V - Y - B,
                X = _[w] + U - Y,
                q = _n(p ? Math.min(P, $) : P, k, p ? Math.max(N, X) : N)
              ;(_[w] = q), (D[w] = q - k)
            }
            if (s) {
              var G = 'x' === w ? Yt : qt,
                Z = 'x' === w ? $t : Xt,
                J = _[E],
                Q = _n(J + m[G], J, J - m[Z])
              ;(_[E] = Q), (D[E] = Q - J)
            }
            t.modifiersData[r] = D
          }
        },
        requiresIfExists: ['offset'],
      },
      Sn,
    ],
  })
  function Qn(e) {
    var n = (function() {
      var e = t.useRef(!0),
        n = t.useRef(function() {
          return e.current
        })
      return (
        t.useEffect(function() {
          return function() {
            e.current = !1
          }
        }, []),
        n.current
      )
    })()
    return [
      e[0],
      t.useCallback(
        function(t) {
          if (n()) return e[1](t)
        },
        [n, e[1]]
      ),
    ]
  }
  var er = function(e) {
      return {
        position: e,
        top: '0',
        left: '0',
        opacity: '0',
        pointerEvents: 'none',
      }
    },
    tr = { name: 'applyStyles', enabled: !1 },
    nr = {
      name: 'ariaDescribedBy',
      enabled: !0,
      phase: 'afterWrite',
      effect: function(e) {
        var t = e.state
        return function() {
          var e = t.elements,
            n = e.reference,
            r = e.popper
          if ('removeAttribute' in n) {
            var o = (n.getAttribute('aria-describedby') || '')
              .split(',')
              .filter(function(e) {
                return e.trim() !== r.id
              })
            o.length
              ? n.setAttribute('aria-describedby', o.join(','))
              : n.removeAttribute('aria-describedby')
          }
        }
      },
      fn: function(e) {
        var t,
          n = e.state.elements,
          r = n.popper,
          o = n.reference,
          a = null == (t = r.getAttribute('role')) ? void 0 : t.toLowerCase()
        if (r.id && 'tooltip' === a && 'setAttribute' in o) {
          var i = o.getAttribute('aria-describedby')
          if (i && -1 !== i.split(',').indexOf(r.id)) return
          o.setAttribute('aria-describedby', i ? i + ',' + r.id : r.id)
        }
      },
    },
    rr = []
  var or = !(
      'undefined' == typeof window ||
      !window.document ||
      !window.document.createElement
    ),
    ar = !1,
    ir = !1
  try {
    var sr = {
      get passive() {
        return (ar = !0)
      },
      get once() {
        return (ir = ar = !0)
      },
    }
    or &&
      (window.addEventListener('test', sr, sr),
      window.removeEventListener('test', sr, !0))
  } catch (e) {}
  function cr(e, t, n, r) {
    return (
      (function(e, t, n, r) {
        if (r && 'boolean' != typeof r && !ir) {
          var o = r.once,
            a = r.capture,
            i = n
          !ir &&
            o &&
            ((i =
              n.__once ||
              function e(r) {
                this.removeEventListener(t, e, a), n.call(this, r)
              }),
            (n.__once = i)),
            e.addEventListener(t, i, ar ? r : a)
        }
        e.addEventListener(t, n, r)
      })(e, t, n, r),
      function() {
        !(function(e, t, n, r) {
          var o = r && 'boolean' != typeof r ? r.capture : r
          e.removeEventListener(t, n, o),
            n.__once && e.removeEventListener(t, n.__once, o)
        })(e, t, n, r)
      }
    )
  }
  function lr(e) {
    var n = (function(e) {
      var n = t.useRef(e)
      return (
        t.useEffect(
          function() {
            n.current = e
          },
          [e]
        ),
        n
      )
    })(e)
    return t.useCallback(
      function() {
        return n.current && n.current.apply(n, arguments)
      },
      [n]
    )
  }
  var ur = function() {}
  function fr(e) {
    return (e && e.ownerDocument) || document
  }
  var dr = function(e) {
      return fr(
        (function(e) {
          return e && 'setState' in e ? o.findDOMNode(e) : null != e ? e : null
        })(e)
      )
    },
    pr = 27,
    vr = function() {}
  var hr = function(e) {
    return e && ('current' in e ? e.current : e)
  }
  var mr = function(e) {
    var t
    return 'undefined' == typeof document
      ? null
      : null == e
      ? fr().body
      : ('function' == typeof e && (e = e()),
        e && 'current' in e && (e = e.current),
        ((null == (t = e) ? void 0 : t.nodeType) && e) || null)
  }
  function gr(e, n) {
    var r = t.useState(function() {
        return mr(e)
      }),
      o = r[0],
      a = r[1]
    if (!o) {
      var i = mr(e)
      i && a(i)
    }
    return (
      t.useEffect(
        function() {
          n && o && n(o)
        },
        [n, o]
      ),
      t.useEffect(
        function() {
          var t = mr(e)
          t !== o && a(t)
        },
        [e, o]
      ),
      o
    )
  }
  function yr(e) {
    var t,
      n,
      r,
      o,
      a,
      i = e.enabled,
      s = e.enableEvents,
      c = e.placement,
      l = e.flip,
      u = e.offset,
      f = e.containerPadding,
      d = e.arrowElement,
      p = e.popperConfig,
      v = void 0 === p ? {} : p,
      h = (function(e) {
        var t = {}
        return Array.isArray(e)
          ? (null == e ||
              e.forEach(function(e) {
                t[e.name] = e
              }),
            t)
          : e || t
      })(v.modifiers)
    return Ht({}, v, {
      placement: c,
      enabled: i,
      modifiers: ((a = Ht({}, h, {
        eventListeners: { enabled: s },
        preventOverflow: Ht({}, h.preventOverflow, {
          options: f
            ? Ht(
                { padding: f },
                null == (t = h.preventOverflow) ? void 0 : t.options
              )
            : null == (n = h.preventOverflow)
            ? void 0
            : n.options,
        }),
        offset: {
          options: Ht(
            { offset: u },
            null == (r = h.offset) ? void 0 : r.options
          ),
        },
        arrow: Ht({}, h.arrow, {
          enabled: !!d,
          options: Ht({}, null == (o = h.arrow) ? void 0 : o.options, {
            element: d,
          }),
        }),
        flip: Ht({ enabled: !!l }, h.flip),
      })),
      void 0 === a && (a = {}),
      Array.isArray(a)
        ? a
        : Object.keys(a).map(function(e) {
            return (a[e].name = e), a[e]
          })),
    })
  }
  var br = r.forwardRef(function(e, n) {
    var a = e.flip,
      i = e.offset,
      s = e.placement,
      c = e.containerPadding,
      l = void 0 === c ? 5 : c,
      u = e.popperConfig,
      f = void 0 === u ? {} : u,
      d = e.transition,
      p = Ut(),
      v = p[0],
      h = p[1],
      m = Ut(),
      g = m[0],
      y = m[1],
      b = Bt(h, n),
      w = gr(e.container),
      E = gr(e.target),
      _ = t.useState(!e.show),
      O = _[0],
      M = _[1],
      S = (function(e, n, r) {
        var o = void 0 === r ? {} : r,
          a = o.enabled,
          i = void 0 === a || a,
          s = o.placement,
          c = void 0 === s ? 'bottom' : s,
          l = o.strategy,
          u = void 0 === l ? 'absolute' : l,
          f = o.modifiers,
          d = void 0 === f ? rr : f,
          p = Vt(o, ['enabled', 'placement', 'strategy', 'modifiers']),
          v = t.useRef(),
          h = t.useCallback(function() {
            var e
            null == (e = v.current) || e.update()
          }, []),
          m = t.useCallback(function() {
            var e
            null == (e = v.current) || e.forceUpdate()
          }, []),
          g = Qn(
            t.useState({
              placement: c,
              update: h,
              forceUpdate: m,
              attributes: {},
              styles: { popper: er(u), arrow: {} },
            })
          ),
          y = g[0],
          b = g[1],
          w = t.useMemo(
            function() {
              return {
                name: 'updateStateModifier',
                enabled: !0,
                phase: 'write',
                requires: ['computeStyles'],
                fn: function(e) {
                  var t = e.state,
                    n = {},
                    r = {}
                  Object.keys(t.elements).forEach(function(e) {
                    ;(n[e] = t.styles[e]), (r[e] = t.attributes[e])
                  }),
                    b({
                      state: t,
                      styles: n,
                      attributes: r,
                      update: h,
                      forceUpdate: m,
                      placement: t.placement,
                    })
                },
              }
            },
            [h, m, b]
          )
        return (
          t.useEffect(
            function() {
              v.current &&
                i &&
                v.current.setOptions({
                  placement: c,
                  strategy: u,
                  modifiers: [].concat(d, [w, tr]),
                })
            },
            [u, c, w, i]
          ),
          t.useEffect(
            function() {
              if (i && null != e && null != n)
                return (
                  (v.current = Jn(
                    e,
                    n,
                    Ht({}, p, {
                      placement: c,
                      strategy: u,
                      modifiers: [].concat(d, [nr, w]),
                    })
                  )),
                  function() {
                    null != v.current &&
                      (v.current.destroy(),
                      (v.current = void 0),
                      b(function(e) {
                        return Ht({}, e, {
                          attributes: {},
                          styles: { popper: er(u) },
                        })
                      }))
                  }
                )
            },
            [i, e, n]
          ),
          y
        )
      })(
        E,
        v,
        yr({
          placement: s,
          enableEvents: !!e.show,
          containerPadding: l || 5,
          flip: a,
          offset: i,
          arrowElement: g,
          popperConfig: f,
        })
      ),
      D = S.styles,
      x = S.attributes,
      j = Vt(S, ['styles', 'attributes'])
    e.show ? O && M(!1) : e.transition || O || M(!0)
    var T,
      k,
      P,
      N,
      A,
      R,
      C,
      L,
      F,
      z,
      I,
      W,
      H = e.show || (d && !O)
    if (
      ((T = v),
      (k = e.onHide),
      (P = {
        disabled: !e.rootClose || e.rootCloseDisabled,
        clickTrigger: e.rootCloseEvent,
      }),
      (A = (N = void 0 === P ? {} : P).disabled),
      (R = N.clickTrigger),
      (C = void 0 === R ? 'click' : R),
      (L = t.useRef(!1)),
      (F = k || vr),
      (z = t.useCallback(
        function(e) {
          var t,
            n,
            r,
            o = hr(T)
          ur(
            !!o,
            'RootClose captured a close event but does not have a ref to compare it to. useRootClose(), should be passed a ref that resolves to a DOM node'
          ),
            (L.current = !(
              o &&
              ((r = e), !(r.metaKey || r.altKey || r.ctrlKey || r.shiftKey)) &&
              (function(e) {
                return 0 === e.button
              })(e) &&
              ((t = o),
              (n = e.target),
              !(t.contains
                ? t.contains(n)
                : t.compareDocumentPosition
                ? t === n || 16 & t.compareDocumentPosition(n)
                : void 0))
            ))
        },
        [T]
      )),
      (I = lr(function(e) {
        L.current || F(e)
      })),
      (W = lr(function(e) {
        e.keyCode === pr && F(e)
      })),
      t.useEffect(
        function() {
          if (!A && null != T) {
            var e = window.event,
              t = dr(hr(T)),
              n = cr(t, C, z, !0),
              r = cr(t, C, function(t) {
                t !== e ? I(t) : (e = void 0)
              }),
              o = cr(t, 'keyup', function(t) {
                t !== e ? W(t) : (e = void 0)
              }),
              a = []
            return (
              'ontouchstart' in t.documentElement &&
                (a = [].slice.call(t.body.children).map(function(e) {
                  return cr(e, 'mousemove', vr)
                })),
              function() {
                n(),
                  r(),
                  o(),
                  a.forEach(function(e) {
                    return e()
                  })
              }
            )
          }
        },
        [T, A, C, z, I, W]
      ),
      !H)
    )
      return null
    var V = e.children(
      Ht({}, j, {
        show: !!e.show,
        props: Ht({}, x.popper, { style: D.popper, ref: b }),
        arrowProps: Ht({}, x.arrow, { style: D.arrow, ref: y }),
      })
    )
    if (d) {
      var U = e.onExit,
        K = e.onExiting,
        B = e.onEnter,
        Y = e.onEntering,
        $ = e.onEntered
      V = r.createElement(
        d,
        {
          in: e.show,
          appear: !0,
          onExit: U,
          onExiting: K,
          onExited: function() {
            M(!0), e.onExited && e.onExited.apply(e, arguments)
          },
          onEnter: B,
          onEntering: Y,
          onEntered: $,
        },
        V
      )
    }
    return w ? o.createPortal(V, w) : null
  })
  function wr(e, t) {
    var n = wt(e)
    return n ? n.innerHeight : t ? e.clientHeight : Mt(e).height
  }
  ;(br.displayName = 'Overlay'),
    (br.propTypes = {
      show: b.bool,
      placement: b.oneOf(an),
      target: b.any,
      container: b.any,
      flip: b.bool,
      children: b.func.isRequired,
      containerPadding: b.number,
      popperConfig: b.object,
      rootClose: b.bool,
      rootCloseEvent: b.oneOf(['click', 'mousedown']),
      rootCloseDisabled: b.bool,
      onHide: function(e) {
        for (
          var t = arguments.length, n = new Array(t > 1 ? t - 1 : 0), r = 1;
          r < t;
          r++
        )
          n[r - 1] = arguments[r]
        var o
        return e.rootClose
          ? (o = b.func).isRequired.apply(o, [e].concat(n))
          : b.func.apply(b, [e].concat(n))
      },
      transition: b.elementType,
      onEnter: b.func,
      onEntering: b.func,
      onEntered: b.func,
      onExit: b.func,
      onExiting: b.func,
      onExited: b.func,
    })
  var Er,
    _r = Function.prototype.bind.call(Function.prototype.call, [].slice)
  function Or(e, t) {
    if (!Er) {
      var n = document.body,
        r =
          n.matches ||
          n.matchesSelector ||
          n.webkitMatchesSelector ||
          n.mozMatchesSelector ||
          n.msMatchesSelector
      Er = function(e, t) {
        return r.call(e, t)
      }
    }
    return Er(e, t)
  }
  var Mr = !1,
    Sr = !1
  try {
    var Dr = {
      get passive() {
        return (Mr = !0)
      },
      get once() {
        return (Sr = Mr = !0)
      },
    }
    jt &&
      (window.addEventListener('test', Dr, Dr),
      window.removeEventListener('test', Dr, !0))
  } catch (e) {}
  function xr(e, t, n, r) {
    return (
      (function(e, t, n, r) {
        if (r && 'boolean' != typeof r && !Sr) {
          var o = r.once,
            a = r.capture,
            i = n
          !Sr &&
            o &&
            ((i =
              n.__once ||
              function e(r) {
                this.removeEventListener(t, e, a), n.call(this, r)
              }),
            (n.__once = i)),
            e.addEventListener(t, i, Mr ? r : a)
        }
        e.addEventListener(t, n, r)
      })(e, t, n, r),
      function() {
        !(function(e, t, n, r) {
          var o = r && 'boolean' != typeof r ? r.capture : r
          e.removeEventListener(t, n, o),
            n.__once && e.removeEventListener(t, n.__once, o)
        })(e, t, n, r)
      }
    )
  }
  function jr(e, t, n) {
    return void 0 === n && (n = document), xr(n, e, t, { passive: !1 })
  }
  function Tr(e, t) {
    var n = t.clientX,
      r = t.clientY
    return (function(e, t, n) {
      e.closest && !n && e.closest(t)
      var r = e
      do {
        if (Or(r, t)) return r
        r = r.parentElement
      } while (r && r !== n && r.nodeType === document.ELEMENT_NODE)
      return null
    })(document.elementFromPoint(n, r), '.rbc-event', e)
  }
  function kr(e, t) {
    return !!Tr(e, t)
  }
  function Pr(e) {
    var t = e
    return (
      e.touches && e.touches.length && (t = e.touches[0]),
      { clientX: t.clientX, clientY: t.clientY, pageX: t.pageX, pageY: t.pageY }
    )
  }
  var Nr = (function() {
    function e(e, t) {
      var n = void 0 === t ? {} : t,
        r = n.global,
        o = void 0 !== r && r,
        a = n.longPressThreshold,
        i = void 0 === a ? 250 : a
      ;(this.isDetached = !1),
        (this.container = e),
        (this.globalMouse = !e || o),
        (this.longPressThreshold = i),
        (this._listeners = Object.create(null)),
        (this._handleInitialEvent = this._handleInitialEvent.bind(this)),
        (this._handleMoveEvent = this._handleMoveEvent.bind(this)),
        (this._handleTerminatingEvent = this._handleTerminatingEvent.bind(
          this
        )),
        (this._keyListener = this._keyListener.bind(this)),
        (this._dropFromOutsideListener = this._dropFromOutsideListener.bind(
          this
        )),
        (this._dragOverFromOutsideListener = this._dragOverFromOutsideListener.bind(
          this
        )),
        (this._removeTouchMoveWindowListener = jr(
          'touchmove',
          function() {},
          window
        )),
        (this._removeKeyDownListener = jr('keydown', this._keyListener)),
        (this._removeKeyUpListener = jr('keyup', this._keyListener)),
        (this._removeDropFromOutsideListener = jr(
          'drop',
          this._dropFromOutsideListener
        )),
        (this._onDragOverfromOutisde = jr(
          'dragover',
          this._dragOverFromOutsideListener
        )),
        this._addInitialEventListener()
    }
    var t = e.prototype
    return (
      (t.on = function(e, t) {
        var n = this._listeners[e] || (this._listeners[e] = [])
        return (
          n.push(t),
          {
            remove: function() {
              var e = n.indexOf(t)
              ;-1 !== e && n.splice(e, 1)
            },
          }
        )
      }),
      (t.emit = function(e) {
        for (
          var t = arguments.length, n = new Array(t > 1 ? t - 1 : 0), r = 1;
          r < t;
          r++
        )
          n[r - 1] = arguments[r]
        var o
        return (
          (this._listeners[e] || []).forEach(function(e) {
            void 0 === o && (o = e.apply(void 0, n))
          }),
          o
        )
      }),
      (t.teardown = function() {
        ;(this.isDetached = !0),
          (this.listeners = Object.create(null)),
          this._removeTouchMoveWindowListener &&
            this._removeTouchMoveWindowListener(),
          this._removeInitialEventListener &&
            this._removeInitialEventListener(),
          this._removeEndListener && this._removeEndListener(),
          this._onEscListener && this._onEscListener(),
          this._removeMoveListener && this._removeMoveListener(),
          this._removeKeyUpListener && this._removeKeyUpListener(),
          this._removeKeyDownListener && this._removeKeyDownListener(),
          this._removeDropFromOutsideListener &&
            this._removeDropFromOutsideListener()
      }),
      (t.isSelected = function(e) {
        var t = this._selectRect
        return !(!t || !this.selecting) && Ar(t, Rr(e))
      }),
      (t.filter = function(e) {
        return this._selectRect && this.selecting
          ? e.filter(this.isSelected, this)
          : []
      }),
      (t._addLongPressListener = function(e, t) {
        var n = this,
          r = null,
          o = null,
          a = null,
          i = function(t) {
            ;(r = setTimeout(function() {
              c(), e(t)
            }, n.longPressThreshold)),
              (o = jr('touchmove', function() {
                return c()
              })),
              (a = jr('touchend', function() {
                return c()
              }))
          },
          s = jr('touchstart', i),
          c = function() {
            r && clearTimeout(r),
              o && o(),
              a && a(),
              (r = null),
              (o = null),
              (a = null)
          }
        return (
          t && i(t),
          function() {
            c(), s()
          }
        )
      }),
      (t._addInitialEventListener = function() {
        var e = this,
          t = jr('mousedown', function(t) {
            e._removeInitialEventListener(),
              e._handleInitialEvent(t),
              (e._removeInitialEventListener = jr(
                'mousedown',
                e._handleInitialEvent
              ))
          }),
          n = jr('touchstart', function(t) {
            e._removeInitialEventListener(),
              (e._removeInitialEventListener = e._addLongPressListener(
                e._handleInitialEvent,
                t
              ))
          })
        this._removeInitialEventListener = function() {
          t(), n()
        }
      }),
      (t._dropFromOutsideListener = function(e) {
        var t = Pr(e),
          n = t.pageX,
          r = t.pageY,
          o = t.clientX,
          a = t.clientY
        this.emit('dropFromOutside', { x: n, y: r, clientX: o, clientY: a }),
          e.preventDefault()
      }),
      (t._dragOverFromOutsideListener = function(e) {
        var t = Pr(e),
          n = t.pageX,
          r = t.pageY,
          o = t.clientX,
          a = t.clientY
        this.emit('dragOverFromOutside', {
          x: n,
          y: r,
          clientX: o,
          clientY: a,
        }),
          e.preventDefault()
      }),
      (t._handleInitialEvent = function(e) {
        if (!this.isDetached) {
          var t,
            n = Pr(e),
            r = n.clientX,
            o = n.clientY,
            a = n.pageX,
            i = n.pageY,
            s = this.container()
          if (
            3 !== e.which &&
            2 !== e.button &&
            ((l = r),
            (u = o),
            !(c = s) || bt(c, document.elementFromPoint(l, u)))
          ) {
            var c, l, u
            if (!this.globalMouse && s && !bt(s, e.target)) {
              var f = (function(e) {
                  void 0 === e && (e = 0)
                  'object' != typeof e &&
                    (e = { top: e, left: e, right: e, bottom: e })
                  return e
                })(0),
                d = f.top,
                p = f.left,
                v = f.bottom,
                h = f.right
              if (
                !Ar(
                  {
                    top: (t = Rr(s)).top - d,
                    left: t.left - p,
                    bottom: t.bottom + v,
                    right: t.right + h,
                  },
                  { top: i, left: a }
                )
              )
                return
            }
            if (
              !1 !==
              this.emit(
                'beforeSelect',
                (this._initialEventData = {
                  isTouch: /^touch/.test(e.type),
                  x: a,
                  y: i,
                  clientX: r,
                  clientY: o,
                })
              )
            )
              switch (e.type) {
                case 'mousedown':
                  ;(this._removeEndListener = jr(
                    'mouseup',
                    this._handleTerminatingEvent
                  )),
                    (this._onEscListener = jr(
                      'keydown',
                      this._handleTerminatingEvent
                    )),
                    (this._removeMoveListener = jr(
                      'mousemove',
                      this._handleMoveEvent
                    ))
                  break
                case 'touchstart':
                  this._handleMoveEvent(e),
                    (this._removeEndListener = jr(
                      'touchend',
                      this._handleTerminatingEvent
                    )),
                    (this._removeMoveListener = jr(
                      'touchmove',
                      this._handleMoveEvent
                    ))
              }
          }
        }
      }),
      (t._handleTerminatingEvent = function(e) {
        var t = Pr(e),
          n = t.pageX,
          r = t.pageY
        if (
          ((this.selecting = !1),
          this._removeEndListener && this._removeEndListener(),
          this._removeMoveListener && this._removeMoveListener(),
          this._initialEventData)
        ) {
          var o = !this.container || bt(this.container(), e.target),
            a = this._selectRect,
            i = this.isClick(n, r)
          return (
            (this._initialEventData = null),
            'Escape' === e.key
              ? this.emit('reset')
              : o
              ? i && o
                ? this._handleClickEvent(e)
                : i
                ? void 0
                : this.emit('select', a)
              : this.emit('reset')
          )
        }
      }),
      (t._handleClickEvent = function(e) {
        var t = Pr(e),
          n = t.pageX,
          r = t.pageY,
          o = t.clientX,
          a = t.clientY,
          i = new Date().getTime()
        return this._lastClickData && i - this._lastClickData.timestamp < 250
          ? ((this._lastClickData = null),
            this.emit('doubleClick', { x: n, y: r, clientX: o, clientY: a }))
          : ((this._lastClickData = { timestamp: i }),
            this.emit('click', { x: n, y: r, clientX: o, clientY: a }))
      }),
      (t._handleMoveEvent = function(e) {
        if (null !== this._initialEventData && !this.isDetached) {
          var t = this._initialEventData,
            n = t.x,
            r = t.y,
            o = Pr(e),
            a = o.pageX,
            i = o.pageY,
            s = Math.abs(n - a),
            c = Math.abs(r - i),
            l = Math.min(a, n),
            u = Math.min(i, r),
            f = this.selecting
          ;(!this.isClick(a, i) || f || s || c) &&
            ((this.selecting = !0),
            (this._selectRect = {
              top: u,
              left: l,
              x: a,
              y: i,
              right: l + s,
              bottom: u + c,
            }),
            f || this.emit('selectStart', this._initialEventData),
            this.isClick(a, i) || this.emit('selecting', this._selectRect),
            e.preventDefault())
        }
      }),
      (t._keyListener = function(e) {
        this.ctrl = e.metaKey || e.ctrlKey
      }),
      (t.isClick = function(e, t) {
        var n = this._initialEventData,
          r = n.x,
          o = n.y
        return !n.isTouch && Math.abs(e - r) <= 5 && Math.abs(t - o) <= 5
      }),
      e
    )
  })()
  function Ar(e, t, n) {
    void 0 === n && (n = 0)
    var r = Rr(e),
      o = r.top,
      a = r.left,
      i = r.right,
      s = void 0 === i ? a : i,
      c = r.bottom,
      l = void 0 === c ? o : c,
      u = Rr(t),
      f = u.top,
      d = u.left,
      p = u.right,
      v = void 0 === p ? d : p,
      h = u.bottom
    return !(
      l - n < f ||
      o + n > (void 0 === h ? f : h) ||
      s - n < d ||
      a + n > v
    )
  }
  function Rr(e) {
    if (!e.getBoundingClientRect) return e
    var t = e.getBoundingClientRect(),
      n = t.left + Cr('left'),
      r = t.top + Cr('top')
    return {
      top: r,
      left: n,
      right: (e.offsetWidth || 0) + n,
      bottom: (e.offsetHeight || 0) + r,
    }
  }
  function Cr(e) {
    return 'left' === e
      ? window.pageXOffset || document.body.scrollLeft || 0
      : 'top' === e
      ? window.pageYOffset || document.body.scrollTop || 0
      : void 0
  }
  var Lr = (function(e) {
    function t(t, n) {
      var r
      return ((r = e.call(this, t, n) || this).state = { selecting: !1 }), r
    }
    c(t, e)
    var o = t.prototype
    return (
      (o.componentDidMount = function() {
        this.props.selectable && this._selectable()
      }),
      (o.componentWillUnmount = function() {
        this._teardownSelectable()
      }),
      (o.UNSAFE_componentWillReceiveProps = function(e) {
        e.selectable && !this.props.selectable && this._selectable(),
          !e.selectable && this.props.selectable && this._teardownSelectable()
      }),
      (o.render = function() {
        var e = this.props,
          t = e.range,
          n = e.getNow,
          o = e.getters,
          a = e.date,
          i = e.components.dateCellWrapper,
          s = this.state,
          c = s.selecting,
          l = s.startIdx,
          u = s.endIdx,
          f = n()
        return r.createElement(
          'div',
          { className: 'rbc-row-bg' },
          t.map(function(e, n) {
            var s = c && n >= l && n <= u,
              d = o.dayProp(e),
              p = d.className,
              v = d.style
            return r.createElement(
              i,
              { key: n, value: e, range: t },
              r.createElement('div', {
                style: v,
                className: D(
                  'rbc-day-bg',
                  p,
                  s && 'rbc-selected-cell',
                  G(e, f, 'day') && 'rbc-today',
                  a && ue(a) !== ue(e) && 'rbc-off-range-bg'
                ),
              })
            )
          })
        )
      }),
      (o._selectable = function() {
        var e = this,
          t = n.findDOMNode(this),
          r = (this._selector = new Nr(this.props.container, {
            longPressThreshold: this.props.longPressThreshold,
          })),
          o = function(r, o) {
            if (!kr(n.findDOMNode(e), r)) {
              var a = Rr(t),
                i = e.props,
                s = i.range,
                c = i.rtl
              if (
                ((u = a),
                (d = (f = r).x),
                (p = f.y) >= u.top &&
                  p <= u.bottom &&
                  d >= u.left &&
                  d <= u.right)
              ) {
                var l = zt(a, r.x, c, s.length)
                e._selectSlot({ startIdx: l, endIdx: l, action: o, box: r })
              }
            }
            var u, f, d, p
            ;(e._initial = {}), e.setState({ selecting: !1 })
          }
        r.on('selecting', function(n) {
          var o = e.props,
            a = o.range,
            i = o.rtl,
            s = -1,
            c = -1
          if (
            (e.state.selecting ||
              (k(e.props.onSelectStart, [n]),
              (e._initial = { x: n.x, y: n.y })),
            r.isSelected(t))
          ) {
            var l = Rr(t),
              u = (function(e, t, n, r, o) {
                var a = -1,
                  i = -1,
                  s = r - 1,
                  c = Ft(t, r),
                  l = zt(t, n.x, o, r),
                  u = t.top < n.y && t.bottom > n.y,
                  f = t.top < e.y && t.bottom > e.y,
                  d = e.y > t.bottom,
                  p = t.top > e.y
                return (
                  n.top < t.top && n.bottom > t.bottom && ((a = 0), (i = s)),
                  u && (p ? ((a = 0), (i = l)) : d && ((a = l), (i = s))),
                  f &&
                    ((a = i = o
                      ? s - Math.floor((e.x - t.left) / c)
                      : Math.floor((e.x - t.left) / c)),
                    u
                      ? l < a
                        ? (a = l)
                        : (i = l)
                      : e.y < n.y
                      ? (i = s)
                      : (a = 0)),
                  { startIdx: a, endIdx: i }
                )
              })(e._initial, l, n, a.length, i)
            ;(s = u.startIdx), (c = u.endIdx)
          }
          e.setState({ selecting: !0, startIdx: s, endIdx: c })
        }),
          r.on('beforeSelect', function(t) {
            if ('ignoreEvents' === e.props.selectable)
              return !kr(n.findDOMNode(e), t)
          }),
          r.on('click', function(e) {
            return o(e, 'click')
          }),
          r.on('doubleClick', function(e) {
            return o(e, 'doubleClick')
          }),
          r.on('select', function(t) {
            e._selectSlot(i({}, e.state, { action: 'select', bounds: t })),
              (e._initial = {}),
              e.setState({ selecting: !1 }),
              k(e.props.onSelectEnd, [e.state])
          })
      }),
      (o._teardownSelectable = function() {
        this._selector && (this._selector.teardown(), (this._selector = null))
      }),
      (o._selectSlot = function(e) {
        var t = e.endIdx,
          n = e.startIdx,
          r = e.action,
          o = e.bounds,
          a = e.box
        ;-1 !== t &&
          -1 !== n &&
          this.props.onSelectSlot &&
          this.props.onSelectSlot({
            start: n,
            end: t,
            action: r,
            bounds: o,
            box: a,
            resourceId: this.props.resourceId,
          })
      }),
      t
    )
  })(r.Component)
  Lr.propTypes = {}
  b.object.isRequired,
    b.object,
    b.bool,
    b.object.isRequired,
    b.object.isRequired,
    b.object.isRequired,
    b.object.isRequired,
    b.func,
    b.func,
    b.func
  var Fr = { segments: [], selected: {} },
    zr = function(e, t) {
      var n = e.selected,
        o = (e.isAllDay, e.accessors),
        a = e.getters,
        i = e.onSelect,
        s = e.onDoubleClick,
        c = e.onKeyPress,
        l = e.localizer,
        u = e.slotMetrics,
        f = e.components,
        d = e.resizable,
        p = u.continuesPrior(t),
        v = u.continuesAfter(t)
      return r.createElement(Ct, {
        event: t,
        getters: a,
        localizer: l,
        accessors: o,
        components: f,
        onSelect: i,
        onDoubleClick: s,
        onKeyPress: c,
        continuesPrior: p,
        continuesAfter: v,
        slotStart: u.first,
        slotEnd: u.last,
        selected: Lt(t, n),
        resizable: d,
      })
    },
    Ir = function(e, t, n, o) {
      void 0 === o && (o = ' ')
      var a = (Math.abs(t) / e) * 100 + '%'
      return r.createElement(
        'div',
        {
          key: n,
          className: 'rbc-row-segment',
          style: { WebkitFlexBasis: a, flexBasis: a, maxWidth: a },
        },
        o
      )
    },
    Wr = (function(e) {
      function t() {
        return e.apply(this, arguments) || this
      }
      return (
        c(t, e),
        (t.prototype.render = function() {
          var e = this,
            t = this.props,
            n = t.segments,
            o = t.slotMetrics.slots,
            a = t.className,
            i = 1
          return r.createElement(
            'div',
            { className: D(a, 'rbc-row') },
            n.reduce(function(t, n, r) {
              var a = n.event,
                s = n.left,
                c = n.right,
                l = n.span,
                u = '_lvl_' + r,
                f = s - i,
                d = zr(e.props, a)
              return (
                f && t.push(Ir(o, f, u + '_gap')),
                t.push(Ir(o, l, u, d)),
                (i = c + 1),
                t
              )
            }, [])
          )
        }),
        t
      )
    })(r.Component)
  function Hr(e, t) {
    for (var n = e.length; n--; ) if (Oe(e[n][0], t)) return n
    return -1
  }
  ;(Wr.propTypes = {}), (Wr.defaultProps = i({}, Fr))
  var Vr = Array.prototype.splice
  function Ur(e) {
    var t = -1,
      n = null == e ? 0 : e.length
    for (this.clear(); ++t < n; ) {
      var r = e[t]
      this.set(r[0], r[1])
    }
  }
  ;(Ur.prototype.clear = function() {
    ;(this.__data__ = []), (this.size = 0)
  }),
    (Ur.prototype.delete = function(e) {
      var t = this.__data__,
        n = Hr(t, e)
      return !(
        n < 0 ||
        (n == t.length - 1 ? t.pop() : Vr.call(t, n, 1), --this.size, 0)
      )
    }),
    (Ur.prototype.get = function(e) {
      var t = this.__data__,
        n = Hr(t, e)
      return n < 0 ? void 0 : t[n][1]
    }),
    (Ur.prototype.has = function(e) {
      return Hr(this.__data__, e) > -1
    }),
    (Ur.prototype.set = function(e, t) {
      var n = this.__data__,
        r = Hr(n, e)
      return r < 0 ? (++this.size, n.push([e, t])) : (n[r][1] = t), this
    })
  var Kr,
    Br = De['__core-js_shared__'],
    Yr = (Kr = /[^.]+$/.exec((Br && Br.keys && Br.keys.IE_PROTO) || ''))
      ? 'Symbol(src)_1.' + Kr
      : ''
  var $r = Function.prototype.toString
  function Xr(e) {
    if (null != e) {
      try {
        return $r.call(e)
      } catch (e) {}
      try {
        return e + ''
      } catch (e) {}
    }
    return ''
  }
  var qr = /^\[object .+?Constructor\]$/,
    Gr = Function.prototype,
    Zr = Object.prototype,
    Jr = Gr.toString,
    Qr = Zr.hasOwnProperty,
    eo = RegExp(
      '^' +
        Jr.call(Qr)
          .replace(/[\\^$.*+?()[\]{}|]/g, '\\$&')
          .replace(
            /hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g,
            '$1.*?'
          ) +
        '$'
    )
  function to(e) {
    return (
      !(!Fe(e) || ((t = e), Yr && Yr in t)) && (Ve(e) ? eo : qr).test(Xr(e))
    )
    var t
  }
  function no(e, t) {
    var n = (function(e, t) {
      return null == e ? void 0 : e[t]
    })(e, t)
    return to(n) ? n : void 0
  }
  var ro = no(De, 'Map'),
    oo = no(Object, 'create')
  var ao = '__lodash_hash_undefined__',
    io = Object.prototype.hasOwnProperty
  var so = Object.prototype.hasOwnProperty
  var co = '__lodash_hash_undefined__'
  function lo(e) {
    var t = -1,
      n = null == e ? 0 : e.length
    for (this.clear(); ++t < n; ) {
      var r = e[t]
      this.set(r[0], r[1])
    }
  }
  function uo(e, t) {
    var n,
      r,
      o = e.__data__
    return ('string' == (r = typeof (n = t)) ||
    'number' == r ||
    'symbol' == r ||
    'boolean' == r
    ? '__proto__' !== n
    : null === n)
      ? o['string' == typeof t ? 'string' : 'hash']
      : o.map
  }
  function fo(e) {
    var t = -1,
      n = null == e ? 0 : e.length
    for (this.clear(); ++t < n; ) {
      var r = e[t]
      this.set(r[0], r[1])
    }
  }
  ;(lo.prototype.clear = function() {
    ;(this.__data__ = oo ? oo(null) : {}), (this.size = 0)
  }),
    (lo.prototype.delete = function(e) {
      var t = this.has(e) && delete this.__data__[e]
      return (this.size -= t ? 1 : 0), t
    }),
    (lo.prototype.get = function(e) {
      var t = this.__data__
      if (oo) {
        var n = t[e]
        return n === ao ? void 0 : n
      }
      return io.call(t, e) ? t[e] : void 0
    }),
    (lo.prototype.has = function(e) {
      var t = this.__data__
      return oo ? void 0 !== t[e] : so.call(t, e)
    }),
    (lo.prototype.set = function(e, t) {
      var n = this.__data__
      return (
        (this.size += this.has(e) ? 0 : 1),
        (n[e] = oo && void 0 === t ? co : t),
        this
      )
    }),
    (fo.prototype.clear = function() {
      ;(this.size = 0),
        (this.__data__ = {
          hash: new lo(),
          map: new (ro || Ur)(),
          string: new lo(),
        })
    }),
    (fo.prototype.delete = function(e) {
      var t = uo(this, e).delete(e)
      return (this.size -= t ? 1 : 0), t
    }),
    (fo.prototype.get = function(e) {
      return uo(this, e).get(e)
    }),
    (fo.prototype.has = function(e) {
      return uo(this, e).has(e)
    }),
    (fo.prototype.set = function(e, t) {
      var n = uo(this, e),
        r = n.size
      return n.set(e, t), (this.size += n.size == r ? 0 : 1), this
    })
  var po = 200
  function vo(e) {
    var t = (this.__data__ = new Ur(e))
    this.size = t.size
  }
  ;(vo.prototype.clear = function() {
    ;(this.__data__ = new Ur()), (this.size = 0)
  }),
    (vo.prototype.delete = function(e) {
      var t = this.__data__,
        n = t.delete(e)
      return (this.size = t.size), n
    }),
    (vo.prototype.get = function(e) {
      return this.__data__.get(e)
    }),
    (vo.prototype.has = function(e) {
      return this.__data__.has(e)
    }),
    (vo.prototype.set = function(e, t) {
      var n = this.__data__
      if (n instanceof Ur) {
        var r = n.__data__
        if (!ro || r.length < po - 1)
          return r.push([e, t]), (this.size = ++n.size), this
        n = this.__data__ = new fo(r)
      }
      return n.set(e, t), (this.size = n.size), this
    })
  var ho = '__lodash_hash_undefined__'
  function mo(e) {
    var t = -1,
      n = null == e ? 0 : e.length
    for (this.__data__ = new fo(); ++t < n; ) this.add(e[t])
  }
  function go(e, t) {
    for (var n = -1, r = null == e ? 0 : e.length; ++n < r; )
      if (t(e[n], n, e)) return !0
    return !1
  }
  ;(mo.prototype.add = mo.prototype.push = function(e) {
    return this.__data__.set(e, ho), this
  }),
    (mo.prototype.has = function(e) {
      return this.__data__.has(e)
    })
  var yo = 1,
    bo = 2
  function wo(e, t, n, r, o, a) {
    var i = n & yo,
      s = e.length,
      c = t.length
    if (s != c && !(i && c > s)) return !1
    var l = a.get(e)
    if (l && a.get(t)) return l == t
    var u = -1,
      f = !0,
      d = n & bo ? new mo() : void 0
    for (a.set(e, t), a.set(t, e); ++u < s; ) {
      var p = e[u],
        v = t[u]
      if (r) var h = i ? r(v, p, u, t, e, a) : r(p, v, u, e, t, a)
      if (void 0 !== h) {
        if (h) continue
        f = !1
        break
      }
      if (d) {
        if (
          !go(t, function(e, t) {
            if (((i = t), !d.has(i) && (p === e || o(p, e, n, r, a))))
              return d.push(t)
            var i
          })
        ) {
          f = !1
          break
        }
      } else if (p !== v && !o(p, v, n, r, a)) {
        f = !1
        break
      }
    }
    return a.delete(e), a.delete(t), f
  }
  var Eo = De.Uint8Array
  function _o(e) {
    var t = -1,
      n = Array(e.size)
    return (
      e.forEach(function(e, r) {
        n[++t] = [r, e]
      }),
      n
    )
  }
  function Oo(e) {
    var t = -1,
      n = Array(e.size)
    return (
      e.forEach(function(e) {
        n[++t] = e
      }),
      n
    )
  }
  var Mo = 1,
    So = 2,
    Do = '[object Boolean]',
    xo = '[object Date]',
    jo = '[object Error]',
    To = '[object Map]',
    ko = '[object Number]',
    Po = '[object RegExp]',
    No = '[object Set]',
    Ao = '[object String]',
    Ro = '[object Symbol]',
    Co = '[object ArrayBuffer]',
    Lo = '[object DataView]',
    Fo = xe ? xe.prototype : void 0,
    zo = Fo ? Fo.valueOf : void 0
  function Io(e, t) {
    for (var n = -1, r = t.length, o = e.length; ++n < r; ) e[o + n] = t[n]
    return e
  }
  var Wo = Array.isArray
  function Ho(e, t, n) {
    var r = t(e)
    return Wo(e) ? r : Io(r, n(e))
  }
  function Vo() {
    return []
  }
  var Uo = Object.prototype.propertyIsEnumerable,
    Ko = Object.getOwnPropertySymbols,
    Bo = Ko
      ? function(e) {
          return null == e
            ? []
            : ((e = Object(e)),
              (function(e, t) {
                for (
                  var n = -1, r = null == e ? 0 : e.length, o = 0, a = [];
                  ++n < r;

                ) {
                  var i = e[n]
                  t(i, n, e) && (a[o++] = i)
                }
                return a
              })(Ko(e), function(t) {
                return Uo.call(e, t)
              }))
        }
      : Vo
  var Yo = '[object Arguments]'
  function $o(e) {
    return Ge(e) && Le(e) == Yo
  }
  var Xo = Object.prototype,
    qo = Xo.hasOwnProperty,
    Go = Xo.propertyIsEnumerable,
    Zo = $o(
      (function() {
        return arguments
      })()
    )
      ? $o
      : function(e) {
          return Ge(e) && qo.call(e, 'callee') && !Go.call(e, 'callee')
        }
  var Jo = 'object' == typeof e && e && !e.nodeType && e,
    Qo =
      Jo && 'object' == typeof module && module && !module.nodeType && module,
    ea = Qo && Qo.exports === Jo ? De.Buffer : void 0,
    ta =
      (ea ? ea.isBuffer : void 0) ||
      function() {
        return !1
      },
    na = {}
  function ra(e) {
    return function(t) {
      return e(t)
    }
  }
  ;(na['[object Float32Array]'] = na['[object Float64Array]'] = na[
    '[object Int8Array]'
  ] = na['[object Int16Array]'] = na['[object Int32Array]'] = na[
    '[object Uint8Array]'
  ] = na['[object Uint8ClampedArray]'] = na['[object Uint16Array]'] = na[
    '[object Uint32Array]'
  ] = !0),
    (na['[object Arguments]'] = na['[object Array]'] = na[
      '[object ArrayBuffer]'
    ] = na['[object Boolean]'] = na['[object DataView]'] = na[
      '[object Date]'
    ] = na['[object Error]'] = na['[object Function]'] = na[
      '[object Map]'
    ] = na['[object Number]'] = na['[object Object]'] = na[
      '[object RegExp]'
    ] = na['[object Set]'] = na['[object String]'] = na[
      '[object WeakMap]'
    ] = !1)
  var oa = 'object' == typeof e && e && !e.nodeType && e,
    aa =
      oa && 'object' == typeof module && module && !module.nodeType && module,
    ia = aa && aa.exports === oa && Me.process,
    sa = (function() {
      try {
        var e = aa && aa.require && aa.require('util').types
        return e || (ia && ia.binding && ia.binding('util'))
      } catch (e) {}
    })(),
    ca = sa && sa.isTypedArray,
    la = ca
      ? ra(ca)
      : function(e) {
          return Ge(e) && Ke(e.length) && !!na[Le(e)]
        },
    ua = Object.prototype.hasOwnProperty
  function fa(e, t) {
    var n = Wo(e),
      r = !n && Zo(e),
      o = !n && !r && ta(e),
      a = !n && !r && !o && la(e),
      i = n || r || o || a,
      s = i
        ? (function(e, t) {
            for (var n = -1, r = Array(e); ++n < e; ) r[n] = t(n)
            return r
          })(e.length, String)
        : [],
      c = s.length
    for (var l in e)
      (!t && !ua.call(e, l)) ||
        (i &&
          ('length' == l ||
            (o && ('offset' == l || 'parent' == l)) ||
            (a && ('buffer' == l || 'byteLength' == l || 'byteOffset' == l)) ||
            Xe(l, c))) ||
        s.push(l)
    return s
  }
  var da = Object.prototype
  function pa(e) {
    var t = e && e.constructor
    return e === (('function' == typeof t && t.prototype) || da)
  }
  function va(e, t) {
    return function(n) {
      return e(t(n))
    }
  }
  var ha = va(Object.keys, Object),
    ma = Object.prototype.hasOwnProperty
  function ga(e) {
    return Be(e)
      ? fa(e)
      : (function(e) {
          if (!pa(e)) return ha(e)
          var t = []
          for (var n in Object(e))
            ma.call(e, n) && 'constructor' != n && t.push(n)
          return t
        })(e)
  }
  function ya(e) {
    return Ho(e, ga, Bo)
  }
  var ba = 1,
    wa = Object.prototype.hasOwnProperty
  var Ea = no(De, 'DataView'),
    _a = no(De, 'Promise'),
    Oa = no(De, 'Set'),
    Ma = no(De, 'WeakMap'),
    Sa = Xr(Ea),
    Da = Xr(ro),
    xa = Xr(_a),
    ja = Xr(Oa),
    Ta = Xr(Ma),
    ka = Le
  ;((Ea && '[object DataView]' != ka(new Ea(new ArrayBuffer(1)))) ||
    (ro && '[object Map]' != ka(new ro())) ||
    (_a && '[object Promise]' != ka(_a.resolve())) ||
    (Oa && '[object Set]' != ka(new Oa())) ||
    (Ma && '[object WeakMap]' != ka(new Ma()))) &&
    (ka = function(e) {
      var t = Le(e),
        n = '[object Object]' == t ? e.constructor : void 0,
        r = n ? Xr(n) : ''
      if (r)
        switch (r) {
          case Sa:
            return '[object DataView]'
          case Da:
            return '[object Map]'
          case xa:
            return '[object Promise]'
          case ja:
            return '[object Set]'
          case Ta:
            return '[object WeakMap]'
        }
      return t
    })
  var Pa = ka,
    Na = 1,
    Aa = '[object Arguments]',
    Ra = '[object Array]',
    Ca = '[object Object]',
    La = Object.prototype.hasOwnProperty
  function Fa(e, t, n, r, o, a) {
    var i = Wo(e),
      s = Wo(t),
      c = i ? Ra : Pa(e),
      l = s ? Ra : Pa(t),
      u = (c = c == Aa ? Ca : c) == Ca,
      f = (l = l == Aa ? Ca : l) == Ca,
      d = c == l
    if (d && ta(e)) {
      if (!ta(t)) return !1
      ;(i = !0), (u = !1)
    }
    if (d && !u)
      return (
        a || (a = new vo()),
        i || la(e)
          ? wo(e, t, n, r, o, a)
          : (function(e, t, n, r, o, a, i) {
              switch (n) {
                case Lo:
                  if (
                    e.byteLength != t.byteLength ||
                    e.byteOffset != t.byteOffset
                  )
                    return !1
                  ;(e = e.buffer), (t = t.buffer)
                case Co:
                  return !(
                    e.byteLength != t.byteLength || !a(new Eo(e), new Eo(t))
                  )
                case Do:
                case xo:
                case ko:
                  return Oe(+e, +t)
                case jo:
                  return e.name == t.name && e.message == t.message
                case Po:
                case Ao:
                  return e == t + ''
                case To:
                  var s = _o
                case No:
                  var c = r & Mo
                  if ((s || (s = Oo), e.size != t.size && !c)) return !1
                  var l = i.get(e)
                  if (l) return l == t
                  ;(r |= So), i.set(e, t)
                  var u = wo(s(e), s(t), r, o, a, i)
                  return i.delete(e), u
                case Ro:
                  if (zo) return zo.call(e) == zo.call(t)
              }
              return !1
            })(e, t, c, n, r, o, a)
      )
    if (!(n & Na)) {
      var p = u && La.call(e, '__wrapped__'),
        v = f && La.call(t, '__wrapped__')
      if (p || v) {
        var h = p ? e.value() : e,
          m = v ? t.value() : t
        return a || (a = new vo()), o(h, m, n, r, a)
      }
    }
    return (
      !!d &&
      (a || (a = new vo()),
      (function(e, t, n, r, o, a) {
        var i = n & ba,
          s = ya(e),
          c = s.length
        if (c != ya(t).length && !i) return !1
        for (var l = c; l--; ) {
          var u = s[l]
          if (!(i ? u in t : wa.call(t, u))) return !1
        }
        var f = a.get(e)
        if (f && a.get(t)) return f == t
        var d = !0
        a.set(e, t), a.set(t, e)
        for (var p = i; ++l < c; ) {
          var v = e[(u = s[l])],
            h = t[u]
          if (r) var m = i ? r(h, v, u, t, e, a) : r(v, h, u, e, t, a)
          if (!(void 0 === m ? v === h || o(v, h, n, r, a) : m)) {
            d = !1
            break
          }
          p || (p = 'constructor' == u)
        }
        if (d && !p) {
          var g = e.constructor,
            y = t.constructor
          g != y &&
            'constructor' in e &&
            'constructor' in t &&
            !(
              'function' == typeof g &&
              g instanceof g &&
              'function' == typeof y &&
              y instanceof y
            ) &&
            (d = !1)
        }
        return a.delete(e), a.delete(t), d
      })(e, t, n, r, o, a))
    )
  }
  function za(e, t, n, r, o) {
    return (
      e === t ||
      (null == e || null == t || (!Ge(e) && !Ge(t))
        ? e != e && t != t
        : Fa(e, t, n, r, za, o))
    )
  }
  var Ia = 1,
    Wa = 2
  function Ha(e) {
    return e == e && !Fe(e)
  }
  function Va(e, t) {
    return function(n) {
      return null != n && (n[e] === t && (void 0 !== t || e in Object(n)))
    }
  }
  function Ua(e) {
    var t = (function(e) {
      for (var t = ga(e), n = t.length; n--; ) {
        var r = t[n],
          o = e[r]
        t[n] = [r, o, Ha(o)]
      }
      return t
    })(e)
    return 1 == t.length && t[0][2]
      ? Va(t[0][0], t[0][1])
      : function(n) {
          return (
            n === e ||
            (function(e, t, n, r) {
              var o = n.length,
                a = o,
                i = !r
              if (null == e) return !a
              for (e = Object(e); o--; ) {
                var s = n[o]
                if (i && s[2] ? s[1] !== e[s[0]] : !(s[0] in e)) return !1
              }
              for (; ++o < a; ) {
                var c = (s = n[o])[0],
                  l = e[c],
                  u = s[1]
                if (i && s[2]) {
                  if (void 0 === l && !(c in e)) return !1
                } else {
                  var f = new vo()
                  if (r) var d = r(l, u, c, e, t, f)
                  if (!(void 0 === d ? za(u, l, Ia | Wa, r, f) : d)) return !1
                }
              }
              return !0
            })(n, e, t)
          )
        }
  }
  var Ka = /\.|\[(?:[^[\]]*|(["'])(?:(?!\1)[^\\]|\\.)*?\1)\]/,
    Ba = /^\w*$/
  function Ya(e, t) {
    if (Wo(e)) return !1
    var n = typeof e
    return (
      !(
        'number' != n &&
        'symbol' != n &&
        'boolean' != n &&
        null != e &&
        !Je(e)
      ) ||
      (Ba.test(e) || !Ka.test(e) || (null != t && e in Object(t)))
    )
  }
  var $a = 'Expected a function'
  function Xa(e, t) {
    if ('function' != typeof e || (null != t && 'function' != typeof t))
      throw new TypeError($a)
    var n = function() {
      var r = arguments,
        o = t ? t.apply(this, r) : r[0],
        a = n.cache
      if (a.has(o)) return a.get(o)
      var i = e.apply(this, r)
      return (n.cache = a.set(o, i) || a), i
    }
    return (n.cache = new (Xa.Cache || fo)()), n
  }
  Xa.Cache = fo
  var qa = 500
  var Ga,
    Za,
    Ja,
    Qa = /[^.[\]]+|\[(?:(-?\d+(?:\.\d+)?)|(["'])((?:(?!\2)[^\\]|\\.)*?)\2)\]|(?=(?:\.|\[\])(?:\.|\[\]|$))/g,
    ei = /\\(\\)?/g,
    ti = ((Ga = function(e) {
      var t = []
      return (
        46 === e.charCodeAt(0) && t.push(''),
        e.replace(Qa, function(e, n, r, o) {
          t.push(r ? o.replace(ei, '$1') : n || e)
        }),
        t
      )
    }),
    (Za = Xa(Ga, function(e) {
      return Ja.size === qa && Ja.clear(), e
    })),
    (Ja = Za.cache),
    Za)
  function ni(e, t) {
    for (var n = -1, r = null == e ? 0 : e.length, o = Array(r); ++n < r; )
      o[n] = t(e[n], n, e)
    return o
  }
  var ri = 1 / 0,
    oi = xe ? xe.prototype : void 0,
    ai = oi ? oi.toString : void 0
  function ii(e) {
    if ('string' == typeof e) return e
    if (Wo(e)) return ni(e, ii) + ''
    if (Je(e)) return ai ? ai.call(e) : ''
    var t = e + ''
    return '0' == t && 1 / e == -ri ? '-0' : t
  }
  function si(e, t) {
    return Wo(e)
      ? e
      : Ya(e, t)
      ? [e]
      : ti(
          (function(e) {
            return null == e ? '' : ii(e)
          })(e)
        )
  }
  var ci = 1 / 0
  function li(e) {
    if ('string' == typeof e || Je(e)) return e
    var t = e + ''
    return '0' == t && 1 / e == -ci ? '-0' : t
  }
  function ui(e, t) {
    for (var n = 0, r = (t = si(t, e)).length; null != e && n < r; )
      e = e[li(t[n++])]
    return n && n == r ? e : void 0
  }
  function fi(e, t) {
    return null != e && t in Object(e)
  }
  function di(e, t) {
    return (
      null != e &&
      (function(e, t, n) {
        for (var r = -1, o = (t = si(t, e)).length, a = !1; ++r < o; ) {
          var i = li(t[r])
          if (!(a = null != e && n(e, i))) break
          e = e[i]
        }
        return a || ++r != o
          ? a
          : !!(o = null == e ? 0 : e.length) &&
              Ke(o) &&
              Xe(i, o) &&
              (Wo(e) || Zo(e))
      })(e, t, fi)
    )
  }
  var pi = 1,
    vi = 2
  function hi(e, t) {
    return Ya(e) && Ha(t)
      ? Va(li(e), t)
      : function(n) {
          var r = (function(e, t, n) {
            var r = null == e ? void 0 : ui(e, t)
            return void 0 === r ? n : r
          })(n, e)
          return void 0 === r && r === t ? di(n, e) : za(t, r, pi | vi)
        }
  }
  function mi(e) {
    return e
  }
  function gi(e) {
    return Ya(e)
      ? ((t = li(e)),
        function(e) {
          return null == e ? void 0 : e[t]
        })
      : (function(e) {
          return function(t) {
            return ui(t, e)
          }
        })(e)
    var t
  }
  function yi(e) {
    return 'function' == typeof e
      ? e
      : null == e
      ? mi
      : 'object' == typeof e
      ? Wo(e)
        ? hi(e[0], e[1])
        : Ua(e)
      : gi(e)
  }
  var bi = Math.max
  function wi(e, t) {
    return (
      void 0 === t && (t = 'day'),
      { first: e[0], last: Y(e[e.length - 1], 1, t) }
    )
  }
  function Ei(e, t, n) {
    var r = wi(t),
      o = r.first,
      a = r.last,
      i = Ee(o, a, 'day'),
      s = ne(X(n.start(e), 'day'), o),
      c = te(ge(n.end(e), 'day'), a),
      l = (function(e, t, n) {
        var r = null == e ? 0 : e.length
        if (!r) return -1
        var o = null == n ? 0 : ct(n)
        return (
          o < 0 && (o = bi(r + o, 0)),
          (function(e, t, n, r) {
            for (var o = e.length, a = n + (r ? 1 : -1); r ? a-- : ++a < o; )
              if (t(e[a], a, e)) return a
            return -1
          })(e, yi(t), o)
        )
      })(t, function(e) {
        return G(e, s, 'day')
      }),
      u = Ee(s, c, 'day')
    return (
      (u = Math.min(u, i)),
      {
        event: e,
        span: (u = Math.max(u, 1)),
        left: l + 1,
        right: Math.max(l + u, 1),
      }
    )
  }
  function _i(e, t) {
    void 0 === t && (t = 1 / 0)
    var n,
      r,
      o,
      a = [],
      i = []
    for (n = 0; n < e.length; n++) {
      for (o = e[n], r = 0; r < a.length && Mi(o, a[r]); r++);
      r >= t ? i.push(o) : (a[r] || (a[r] = [])).push(o)
    }
    for (n = 0; n < a.length; n++)
      a[n].sort(function(e, t) {
        return e.left - t.left
      })
    return { levels: a, extra: i }
  }
  function Oi(e, t, n, r) {
    var o = X(r.start(e), 'day'),
      a = r.end(e),
      i = ee(o, n, 'day'),
      s = G(o, a, 'minutes') ? J(a, t, 'minutes') : Z(a, t, 'minutes')
    return i && s
  }
  function Mi(e, t) {
    return t.some(function(t) {
      return t.left <= e.right && t.right >= e.left
    })
  }
  function Si(e, t, n) {
    var r = +X(n.start(e), 'day') - +X(n.start(t), 'day'),
      o = Ee(n.start(e), ge(n.end(e), 'day'), 'day'),
      a = Ee(n.start(t), ge(n.end(t), 'day'), 'day')
    return (
      r ||
      Math.max(a, 1) - Math.max(o, 1) ||
      !!n.allDay(t) - !!n.allDay(e) ||
      +n.start(e) - +n.start(t)
    )
  }
  var Di = Math.ceil,
    xi = Math.max
  var ji,
    Ti = function(e, t, n) {
      return (
        n && 'number' != typeof n && qe(e, t, n) && (t = n = void 0),
        (e = st(e)),
        void 0 === t ? ((t = e), (e = 0)) : (t = st(t)),
        (function(e, t, n, r) {
          for (
            var o = -1, a = xi(Di((t - e) / (n || 1)), 0), i = Array(a);
            a--;

          )
            (i[r ? a : ++o] = e), (e += n)
          return i
        })(e, t, (n = void 0 === n ? (e < t ? 1 : -1) : st(n)), ji)
      )
    },
    ki = function(e, t) {
      return e.left <= t && e.right >= t
    },
    Pi = function(e, t) {
      return e.filter(function(e) {
        return ki(e, t)
      }).length
    },
    Ni = (function(e) {
      function t() {
        return e.apply(this, arguments) || this
      }
      c(t, e)
      var n = t.prototype
      return (
        (n.render = function() {
          for (
            var e = this.props,
              t = e.segments,
              n = e.slotMetrics.slots,
              o = _i(t).levels[0],
              a = 1,
              i = 1,
              s = [];
            a <= n;

          ) {
            var c = '_lvl_' + a,
              l =
                o.filter(function(e) {
                  return ki(e, a)
                })[0] || {},
              u = l.event,
              f = l.left,
              d = l.right,
              p = l.span
            if (u) {
              var v = Math.max(0, f - i)
              if (this.canRenderSlotEvent(f, p)) {
                var h = zr(this.props, u)
                v && s.push(Ir(n, v, c + '_gap')),
                  s.push(Ir(n, p, c, h)),
                  (i = a = d + 1)
              } else
                v && s.push(Ir(n, v, c + '_gap')),
                  s.push(Ir(n, 1, c, this.renderShowMore(t, a))),
                  (i = a += 1)
            } else a++
          }
          return r.createElement('div', { className: 'rbc-row' }, s)
        }),
        (n.canRenderSlotEvent = function(e, t) {
          var n = this.props.segments
          return Ti(e, e + t).every(function(e) {
            return 1 === Pi(n, e)
          })
        }),
        (n.renderShowMore = function(e, t) {
          var n = this,
            o = this.props.localizer,
            a = Pi(e, t)
          return (
            !!a &&
            r.createElement(
              'a',
              {
                key: 'sm_' + t,
                href: '#',
                className: 'rbc-show-more',
                onClick: function(e) {
                  return n.showMore(t, e)
                },
              },
              o.messages.showMore(a)
            )
          )
        }),
        (n.showMore = function(e, t) {
          t.preventDefault(), this.props.onShowMore(e, t.target)
        }),
        t
      )
    })(r.Component)
  ;(Ni.propTypes = {}), (Ni.defaultProps = i({}, Fr))
  var Ai = function(e) {
    var t = e.children
    return r.createElement(
      'div',
      { className: 'rbc-row-content-scroll-container' },
      t
    )
  }
  function Ri(e, t) {
    if (e.length !== t.length) return !1
    for (var n = 0; n < e.length; n++) if (e[n] !== t[n]) return !1
    return !0
  }
  function Ci(e, t) {
    var n
    void 0 === t && (t = Ri)
    var r,
      o = [],
      a = !1
    return function() {
      for (var i = [], s = 0; s < arguments.length; s++) i[s] = arguments[s]
      return a && n === this && t(i, o)
        ? r
        : ((r = e.apply(this, i)), (a = !0), (n = this), (o = i), r)
    }
  }
  var Li = function(e, t) {
      return e.left <= t && e.right >= t
    },
    Fi = function(e, t) {
      return e[0].range === t[0].range && e[0].events === t[0].events
    }
  var zi = (function(e) {
    function t() {
      for (var t, o = arguments.length, a = new Array(o), s = 0; s < o; s++)
        a[s] = arguments[s]
      return (
        ((t =
          e.call.apply(e, [this].concat(a)) ||
          this).handleSelectSlot = function(e) {
          var n = t.props,
            r = n.range
          ;(0, n.onSelectSlot)(r.slice(e.start, e.end + 1), e)
        }),
        (t.handleShowMore = function(e, r) {
          var o,
            a,
            i,
            s = t.props,
            c = s.range,
            l = s.onShowMore,
            u = t.slotMetrics(t.props),
            f = ((o = n.findDOMNode(C(t))),
            (a = '.rbc-row-bg'),
            _r(o.querySelectorAll(a)))[0]
          f && (i = f.children[e - 1]),
            l(u.getEventsForSlot(e), c[e - 1], i, e, r)
        }),
        (t.createHeadingRef = function(e) {
          t.headingRow = e
        }),
        (t.createEventRef = function(e) {
          t.eventRow = e
        }),
        (t.getContainer = function() {
          var e = t.props.container
          return e ? e() : n.findDOMNode(C(t))
        }),
        (t.renderHeadingCell = function(e, n) {
          var r = t.props,
            o = r.renderHeader,
            a = r.getNow
          return o({
            date: e,
            key: 'header_' + n,
            className: D('rbc-date-cell', G(e, a(), 'day') && 'rbc-now'),
          })
        }),
        (t.renderDummy = function() {
          var e = t.props,
            n = e.className,
            o = e.range,
            a = e.renderHeader,
            i = e.showAllEvents
          return r.createElement(
            'div',
            { className: n },
            r.createElement(
              'div',
              {
                className: D(
                  'rbc-row-content',
                  i && 'rbc-row-content-scrollable'
                ),
              },
              a &&
                r.createElement(
                  'div',
                  { className: 'rbc-row', ref: t.createHeadingRef },
                  o.map(t.renderHeadingCell)
                ),
              r.createElement(
                'div',
                { className: 'rbc-row', ref: t.createEventRef },
                r.createElement(
                  'div',
                  { className: 'rbc-row-segment' },
                  r.createElement(
                    'div',
                    { className: 'rbc-event' },
                    r.createElement(
                      'div',
                      { className: 'rbc-event-content' },
                      ' '
                    )
                  )
                )
              )
            )
          )
        }),
        (t.slotMetrics = (function e() {
          return Ci(function(t) {
            for (
              var n = t.range,
                r = t.events,
                o = t.maxRows,
                a = t.minRows,
                s = t.accessors,
                c = wi(n),
                l = c.first,
                u = c.last,
                f = r.map(function(e) {
                  return Ei(e, n, s)
                }),
                d = _i(f, Math.max(o - 1, 1)),
                p = d.levels,
                v = d.extra;
              p.length < a;

            )
              p.push([])
            return {
              first: l,
              last: u,
              levels: p,
              extra: v,
              range: n,
              slots: n.length,
              clone: function(n) {
                return e()(i({}, t, n))
              },
              getDateForSlot: function(e) {
                return n[e]
              },
              getSlotForDate: function(e) {
                return n.find(function(t) {
                  return G(t, e, 'day')
                })
              },
              getEventsForSlot: function(e) {
                return f
                  .filter(function(t) {
                    return Li(t, e)
                  })
                  .map(function(e) {
                    return e.event
                  })
              },
              continuesPrior: function(e) {
                return Q(s.start(e), l, 'day')
              },
              continuesAfter: function(e) {
                var t = s.end(e)
                return G(s.start(e), t, 'minutes')
                  ? J(t, u, 'minutes')
                  : Z(t, u, 'minutes')
              },
            }
          }, Fi)
        })()),
        t
      )
    }
    c(t, e)
    var o = t.prototype
    return (
      (o.getRowLimit = function() {
        var e = wr(this.eventRow),
          t = this.headingRow ? wr(this.headingRow) : 0,
          r = wr(n.findDOMNode(this)) - t
        return Math.max(Math.floor(r / e), 1)
      }),
      (o.render = function() {
        var e = this.props,
          t = e.date,
          n = e.rtl,
          o = e.range,
          s = e.className,
          c = e.selected,
          l = e.selectable,
          u = e.renderForMeasure,
          f = e.accessors,
          d = e.getters,
          p = e.components,
          v = e.getNow,
          h = e.renderHeader,
          m = e.onSelect,
          g = e.localizer,
          y = e.onSelectStart,
          b = e.onSelectEnd,
          w = e.onDoubleClick,
          E = e.onKeyPress,
          _ = e.resourceId,
          O = e.longPressThreshold,
          M = e.isAllDay,
          S = e.resizable,
          x = e.showAllEvents
        if (u) return this.renderDummy()
        var j = this.slotMetrics(this.props),
          T = j.levels,
          k = j.extra,
          P = x ? Ai : a,
          N = p.weekWrapper,
          A = {
            selected: c,
            accessors: f,
            getters: d,
            localizer: g,
            components: p,
            onSelect: m,
            onDoubleClick: w,
            onKeyPress: E,
            resourceId: _,
            slotMetrics: j,
            resizable: S,
          }
        return r.createElement(
          'div',
          { className: s, role: 'rowgroup' },
          r.createElement(Lr, {
            date: t,
            getNow: v,
            rtl: n,
            range: o,
            selectable: l,
            container: this.getContainer,
            getters: d,
            onSelectStart: y,
            onSelectEnd: b,
            onSelectSlot: this.handleSelectSlot,
            components: p,
            longPressThreshold: O,
            resourceId: _,
          }),
          r.createElement(
            'div',
            {
              className: D(
                'rbc-row-content',
                x && 'rbc-row-content-scrollable'
              ),
              role: 'row',
            },
            h &&
              r.createElement(
                'div',
                { className: 'rbc-row ', ref: this.createHeadingRef },
                o.map(this.renderHeadingCell)
              ),
            r.createElement(
              P,
              null,
              r.createElement(
                N,
                i({ isAllDay: M }, A),
                T.map(function(e, t) {
                  return r.createElement(Wr, i({ key: t, segments: e }, A))
                }),
                !!k.length &&
                  r.createElement(
                    Ni,
                    i({ segments: k, onShowMore: this.handleShowMore }, A)
                  )
              )
            )
          )
        )
      }),
      t
    )
  })(r.Component)
  ;(zi.propTypes = {}), (zi.defaultProps = { minRows: 0, maxRows: 1 / 0 })
  var Ii = function(e) {
    var t = e.label
    return r.createElement(
      'span',
      { role: 'columnheader', 'aria-sort': 'none' },
      t
    )
  }
  Ii.propTypes = {}
  var Wi = function(e) {
    var t = e.label,
      n = e.drilldownView,
      o = e.onDrillDown
    return n
      ? r.createElement('a', { href: '#', onClick: o, role: 'cell' }, t)
      : r.createElement('span', null, t)
  }
  Wi.propTypes = {}
  var Hi = function(e, t, n, r) {
      return e.filter(function(e) {
        return Oi(e, t, n, r)
      })
    },
    Vi = (function(e) {
      function t() {
        for (var t, o = arguments.length, a = new Array(o), c = 0; c < o; c++)
          a[c] = arguments[c]
        return (
          ((t =
            e.call.apply(e, [this].concat(a)) ||
            this).getContainer = function() {
            return n.findDOMNode(C(t))
          }),
          (t.renderWeek = function(e, n) {
            var o = t.props,
              a = o.events,
              i = o.components,
              s = o.selectable,
              c = o.getNow,
              l = o.selected,
              u = o.date,
              f = o.localizer,
              d = o.longPressThreshold,
              p = o.accessors,
              v = o.getters,
              h = o.showAllEvents,
              m = t.state,
              g = m.needLimitMeasure,
              y = m.rowLimit
            return (
              (a = Hi(a, e[0], e[e.length - 1], p)).sort(function(e, t) {
                return Si(e, t, p)
              }),
              r.createElement(zi, {
                key: n,
                ref: 0 === n ? t.slotRowRef : void 0,
                container: t.getContainer,
                className: 'rbc-month-row',
                getNow: c,
                date: u,
                range: e,
                events: a,
                maxRows: h ? 1 / 0 : y,
                selected: l,
                selectable: s,
                components: i,
                accessors: p,
                getters: v,
                localizer: f,
                renderHeader: t.readerDateHeading,
                renderForMeasure: g,
                onShowMore: t.handleShowMore,
                onSelect: t.handleSelectEvent,
                onDoubleClick: t.handleDoubleClickEvent,
                onKeyPress: t.handleKeyPressEvent,
                onSelectSlot: t.handleSelectSlot,
                longPressThreshold: d,
                rtl: t.props.rtl,
                resizable: t.props.resizable,
                showAllEvents: h,
              })
            )
          }),
          (t.readerDateHeading = function(e) {
            var n = e.date,
              o = e.className,
              a = s(e, ['date', 'className']),
              c = t.props,
              l = c.date,
              u = c.getDrilldownView,
              f = c.localizer,
              d = ue(n) !== ue(l),
              p = G(n, l, 'day'),
              v = u(n),
              h = f.format(n, 'dateFormat'),
              m = t.props.components.dateHeader || Wi
            return r.createElement(
              'div',
              i({}, a, {
                className: D(o, d && 'rbc-off-range', p && 'rbc-current'),
                role: 'cell',
              }),
              r.createElement(m, {
                label: h,
                date: n,
                drilldownView: v,
                isOffRange: d,
                onDrillDown: function(e) {
                  return t.handleHeadingClick(n, v, e)
                },
              })
            )
          }),
          (t.handleSelectSlot = function(e, n) {
            ;(t._pendingSelection = t._pendingSelection.concat(e)),
              clearTimeout(t._selectTimer),
              (t._selectTimer = setTimeout(function() {
                return t.selectDates(n)
              }))
          }),
          (t.handleHeadingClick = function(e, n, r) {
            r.preventDefault(),
              t.clearSelection(),
              k(t.props.onDrillDown, [e, n])
          }),
          (t.handleSelectEvent = function() {
            t.clearSelection()
            for (var e = arguments.length, n = new Array(e), r = 0; r < e; r++)
              n[r] = arguments[r]
            k(t.props.onSelectEvent, n)
          }),
          (t.handleDoubleClickEvent = function() {
            t.clearSelection()
            for (var e = arguments.length, n = new Array(e), r = 0; r < e; r++)
              n[r] = arguments[r]
            k(t.props.onDoubleClickEvent, n)
          }),
          (t.handleKeyPressEvent = function() {
            t.clearSelection()
            for (var e = arguments.length, n = new Array(e), r = 0; r < e; r++)
              n[r] = arguments[r]
            k(t.props.onKeyPressEvent, n)
          }),
          (t.handleShowMore = function(e, r, o, a, i) {
            var s = t.props,
              c = s.popup,
              l = s.onDrillDown,
              u = s.onShowMore,
              f = s.getDrilldownView
            if ((t.clearSelection(), c)) {
              var d = xt(o, n.findDOMNode(C(t)))
              t.setState({
                overlay: { date: r, events: e, position: d, target: i },
              })
            } else k(l, [r, f(r) || j.DAY])
            k(u, [e, r, a])
          }),
          (t.overlayDisplay = function() {
            t.setState({ overlay: null })
          }),
          (t._bgRows = []),
          (t._pendingSelection = []),
          (t.slotRowRef = r.createRef()),
          (t.state = { rowLimit: 5, needLimitMeasure: !0 }),
          t
        )
      }
      c(t, e)
      var o = t.prototype
      return (
        (o.UNSAFE_componentWillReceiveProps = function(e) {
          var t = e.date
          this.setState({ needLimitMeasure: !G(t, this.props.date, 'month') })
        }),
        (o.componentDidMount = function() {
          var e,
            t = this
          this.state.needLimitMeasure && this.measureRowLimit(this.props),
            window.addEventListener(
              'resize',
              (this._resizeListener = function() {
                e ||
                  Rt(function() {
                    ;(e = !1), t.setState({ needLimitMeasure: !0 })
                  })
              }),
              !1
            )
        }),
        (o.componentDidUpdate = function() {
          this.state.needLimitMeasure && this.measureRowLimit(this.props)
        }),
        (o.componentWillUnmount = function() {
          window.removeEventListener('resize', this._resizeListener, !1)
        }),
        (o.render = function() {
          var e = this.props,
            t = e.date,
            n = e.localizer,
            o = e.className,
            a = (function(e, t, n) {
              t = (n ? qe(e, t, n) : void 0 === t) ? 1 : ut(ct(t), 0)
              var r = null == e ? 0 : e.length
              if (!r || t < 1) return []
              for (var o = 0, a = 0, i = Array(lt(r / t)); o < r; )
                i[a++] = _e(e, o, (o += t))
              return i
            })(
              (function(e, t) {
                for (var n = he(e, t), r = me(e, t), o = []; ee(n, r, 'day'); )
                  o.push(n), (n = Y(n, 1, 'day'))
                return o
              })(t, n),
              7
            )
          return (
            (this._weekCount = a.length),
            r.createElement(
              'div',
              {
                className: D('rbc-month-view', o),
                role: 'table',
                'aria-label': 'Month View',
              },
              r.createElement(
                'div',
                { className: 'rbc-row rbc-month-header', role: 'row' },
                this.renderHeaders(a[0])
              ),
              a.map(this.renderWeek),
              this.props.popup && this.renderOverlay()
            )
          )
        }),
        (o.renderHeaders = function(e) {
          var t = this.props,
            n = t.localizer,
            o = t.components,
            a = e[0],
            i = e[e.length - 1],
            s = o.header || Ii
          return ye(a, i, 'day').map(function(e, t) {
            return r.createElement(
              'div',
              { key: 'header_' + t, className: 'rbc-header' },
              r.createElement(s, {
                date: e,
                localizer: n,
                label: n.format(e, 'weekdayFormat'),
              })
            )
          })
        }),
        (o.renderOverlay = function() {
          var e = this,
            t = (this.state && this.state.overlay) || {},
            n = this.props,
            o = n.accessors,
            a = n.localizer,
            s = n.components,
            c = n.getters,
            l = n.selected,
            u = n.popupOffset
          return r.createElement(
            br,
            {
              rootClose: !0,
              placement: 'bottom',
              show: !!t.position,
              onHide: function() {
                return e.setState({ overlay: null })
              },
              target: function() {
                return t.target
              },
            },
            function(n) {
              var f = n.props
              return r.createElement(
                Wt,
                i({}, f, {
                  popupOffset: u,
                  accessors: o,
                  getters: c,
                  selected: l,
                  components: s,
                  localizer: a,
                  position: t.position,
                  show: e.overlayDisplay,
                  events: t.events,
                  slotStart: t.date,
                  slotEnd: t.end,
                  onSelect: e.handleSelectEvent,
                  onDoubleClick: e.handleDoubleClickEvent,
                  onKeyPress: e.handleKeyPressEvent,
                  handleDragStart: e.props.handleDragStart,
                })
              )
            }
          )
        }),
        (o.measureRowLimit = function() {
          this.setState({
            needLimitMeasure: !1,
            rowLimit: this.slotRowRef.current.getRowLimit(),
          })
        }),
        (o.selectDates = function(e) {
          var t = this._pendingSelection.slice()
          ;(this._pendingSelection = []),
            t.sort(function(e, t) {
              return +e - +t
            }),
            k(this.props.onSelectSlot, {
              slots: t,
              start: t[0],
              end: t[t.length - 1],
              action: e.action,
              bounds: e.bounds,
              box: e.box,
            })
        }),
        (o.clearSelection = function() {
          clearTimeout(this._selectTimer), (this._pendingSelection = [])
        }),
        t
      )
    })(r.Component)
  ;(Vi.propTypes = {}),
    (Vi.range = function(e, t) {
      var n = t.localizer
      return { start: he(e, n), end: me(e, n) }
    }),
    (Vi.navigate = function(e, t) {
      switch (t) {
        case x.PREVIOUS:
          return Y(e, -1, 'month')
        case x.NEXT:
          return Y(e, 1, 'month')
        default:
          return e
      }
    }),
    (Vi.title = function(e, t) {
      return t.localizer.format(e, 'monthHeaderFormat')
    })
  var Ui = function(e, t) {
      return e.getTimezoneOffset() - t.getTimezoneOffset()
    },
    Ki = function(e, t, n, r) {
      return +X(e, 'minutes') + '' + +X(t, 'minutes') + n + '-' + r
    }
  function Bi(e) {
    for (
      var t = e.min,
        n = e.max,
        r = e.step,
        o = e.timeslots,
        a = Ki(t, n, r, o),
        i = X(t, 'day'),
        s = Ui(i, t),
        c = 1 + Ee(t, n, 'minutes') + Ui(t, n),
        l = Ee(i, t, 'minutes') + s,
        u = Math.ceil(c / (r * o)),
        f = u * o,
        d = new Array(u),
        p = new Array(f),
        v = 0;
      v < u;
      v++
    ) {
      d[v] = new Array(o)
      for (var h = 0; h < o; h++) {
        var m = v * o + h,
          g = m * r
        p[m] = d[v][h] = new Date(
          t.getFullYear(),
          t.getMonth(),
          t.getDate(),
          0,
          l + g,
          0,
          0
        )
      }
    }
    var y = p.length * r
    function b(e) {
      var n = Ee(t, e, 'minutes') + Ui(t, e)
      return Math.min(n, c)
    }
    return (
      p.push(
        new Date(t.getFullYear(), t.getMonth(), t.getDate(), 0, l + y, 0, 0)
      ),
      {
        groups: d,
        update: function(e) {
          return Ki(e) !== a ? Bi(e) : this
        },
        dateIsInGroup: function(e, t) {
          var r = d[t + 1]
          return re(e, d[t][0], r ? r[0] : n, 'minutes')
        },
        nextSlot: function(e) {
          var t = p[Math.min(p.indexOf(e) + 1, p.length - 1)]
          return t === e && (t = Y(e, r, 'minutes')), t
        },
        closestSlotToPosition: function(e) {
          var t = Math.min(p.length - 1, Math.max(0, Math.floor(e * f)))
          return p[t]
        },
        closestSlotFromPoint: function(e, t) {
          var n = Math.abs(t.top - t.bottom)
          return this.closestSlotToPosition((e.y - t.top) / n)
        },
        closestSlotFromDate: function(e, n) {
          if ((void 0 === n && (n = 0), Q(e, t, 'minutes'))) return p[0]
          var o = Ee(t, e, 'minutes')
          return p[(o - (o % r)) / r + n]
        },
        startsBeforeDay: function(e) {
          return Q(e, t, 'day')
        },
        startsAfterDay: function(e) {
          return Z(e, n, 'day')
        },
        startsBefore: function(e) {
          return Q(be(t, e), t, 'minutes')
        },
        startsAfter: function(e) {
          return Z(be(n, e), n, 'minutes')
        },
        getRange: function(e, o, a, i) {
          a || (e = te(n, ne(t, e))), i || (o = te(n, ne(t, o)))
          var s = b(e),
            c = b(o),
            l =
              c > r * f && !G(n, o)
                ? ((s - r) / (r * f)) * 100
                : (s / (r * f)) * 100
          return {
            top: l,
            height: (c / (r * f)) * 100 - l,
            start: b(e),
            startDate: e,
            end: b(o),
            endDate: o,
          }
        },
        getCurrentTimePosition: function(e) {
          return (b(e) / (r * f)) * 100
        },
      }
    )
  }
  function Yi(e, t) {
    for (var n = 0; n < t.length; n++) {
      var r = t[n]
      ;(r.enumerable = r.enumerable || !1),
        (r.configurable = !0),
        'value' in r && (r.writable = !0),
        Object.defineProperty(e, r.key, r)
    }
  }
  var $i = xe ? xe.isConcatSpreadable : void 0
  function Xi(e) {
    return Wo(e) || Zo(e) || !!($i && e && e[$i])
  }
  function qi(e, t, n, r, o) {
    var a = -1,
      i = e.length
    for (n || (n = Xi), o || (o = []); ++a < i; ) {
      var s = e[a]
      t > 0 && n(s)
        ? t > 1
          ? qi(s, t - 1, n, r, o)
          : Io(o, s)
        : r || (o[o.length] = s)
    }
    return o
  }
  var Gi = (function(e) {
    return function(t, n, r) {
      for (var o = -1, a = Object(t), i = r(t), s = i.length; s--; ) {
        var c = i[e ? s : ++o]
        if (!1 === n(a[c], c, a)) break
      }
      return t
    }
  })()
  function Zi(e, t) {
    return e && Gi(e, t, ga)
  }
  var Ji = (function(e, t) {
    return function(n, r) {
      if (null == n) return n
      if (!Be(n)) return e(n, r)
      for (
        var o = n.length, a = t ? o : -1, i = Object(n);
        (t ? a-- : ++a < o) && !1 !== r(i[a], a, i);

      );
      return n
    }
  })(Zi)
  function Qi(e, t) {
    if (e !== t) {
      var n = void 0 !== e,
        r = null === e,
        o = e == e,
        a = Je(e),
        i = void 0 !== t,
        s = null === t,
        c = t == t,
        l = Je(t)
      if (
        (!s && !l && !a && e > t) ||
        (a && i && c && !s && !l) ||
        (r && i && c) ||
        (!n && c) ||
        !o
      )
        return 1
      if (
        (!r && !a && !l && e < t) ||
        (l && n && o && !r && !a) ||
        (s && n && o) ||
        (!i && o) ||
        !c
      )
        return -1
    }
    return 0
  }
  function es(e, t, n) {
    var r = -1
    return (
      (t = ni(t.length ? t : [mi], ra(yi))),
      (function(e, t) {
        var n = e.length
        for (e.sort(t); n--; ) e[n] = e[n].value
        return e
      })(
        (function(e, t) {
          var n = -1,
            r = Be(e) ? Array(e.length) : []
          return (
            Ji(e, function(e, o, a) {
              r[++n] = t(e, o, a)
            }),
            r
          )
        })(e, function(e, n, o) {
          return {
            criteria: ni(t, function(t) {
              return t(e)
            }),
            index: ++r,
            value: e,
          }
        }),
        function(e, t) {
          return (function(e, t, n) {
            for (
              var r = -1,
                o = e.criteria,
                a = t.criteria,
                i = o.length,
                s = n.length;
              ++r < i;

            ) {
              var c = Qi(o[r], a[r])
              if (c) return r >= s ? c : c * ('desc' == n[r] ? -1 : 1)
            }
            return e.index - t.index
          })(e, t, n)
        }
      )
    )
  }
  var ts = Math.max
  function ns(e, t, n) {
    return (
      (t = ts(void 0 === t ? e.length - 1 : t, 0)),
      function() {
        for (
          var r = arguments, o = -1, a = ts(r.length - t, 0), i = Array(a);
          ++o < a;

        )
          i[o] = r[t + o]
        o = -1
        for (var s = Array(t + 1); ++o < t; ) s[o] = r[o]
        return (
          (s[t] = n(i)),
          (function(e, t, n) {
            switch (n.length) {
              case 0:
                return e.call(t)
              case 1:
                return e.call(t, n[0])
              case 2:
                return e.call(t, n[0], n[1])
              case 3:
                return e.call(t, n[0], n[1], n[2])
            }
            return e.apply(t, n)
          })(e, this, s)
        )
      }
    )
  }
  var rs = (function() {
      try {
        var e = no(Object, 'defineProperty')
        return e({}, '', {}), e
      } catch (e) {}
    })(),
    os = rs
      ? function(e, t) {
          return rs(e, 'toString', {
            configurable: !0,
            enumerable: !1,
            value: ((n = t),
            function() {
              return n
            }),
            writable: !0,
          })
          var n
        }
      : mi,
    as = 800,
    is = 16,
    ss = Date.now
  var cs = (function(e) {
    var t = 0,
      n = 0
    return function() {
      var r = ss(),
        o = is - (r - n)
      if (((n = r), o > 0)) {
        if (++t >= as) return arguments[0]
      } else t = 0
      return e.apply(void 0, arguments)
    }
  })(os)
  function ls(e, t) {
    return cs(ns(e, t, mi), e + '')
  }
  var us = ls(function(e, t) {
      if (null == e) return []
      var n = t.length
      return (
        n > 1 && qe(e, t[0], t[1])
          ? (t = [])
          : n > 2 && qe(t[0], t[1], t[2]) && (t = [t[0]]),
        es(e, qi(t, 1), [])
      )
    }),
    fs = (function() {
      function e(e, t) {
        var n = t.accessors,
          r = t.slotMetrics.getRange(n.start(e), n.end(e)),
          o = r.start,
          a = r.startDate,
          i = r.end,
          s = r.endDate,
          c = r.top,
          l = r.height
        ;(this.start = o),
          (this.end = i),
          (this.startMs = +a),
          (this.endMs = +s),
          (this.top = c),
          (this.height = l),
          (this.data = e)
      }
      var t, n, r
      return (
        (t = e),
        (n = [
          {
            key: '_width',
            get: function() {
              if (this.rows)
                return (
                  100 /
                  (this.rows.reduce(function(e, t) {
                    return Math.max(e, t.leaves.length + 1)
                  }, 0) +
                    1)
                )
              var e = 100 - this.container._width
              return this.leaves
                ? e / (this.leaves.length + 1)
                : this.row._width
            },
          },
          {
            key: 'width',
            get: function() {
              var e = this._width,
                t = Math.min(100, 1.7 * this._width)
              if (this.rows) return t
              if (this.leaves) return this.leaves.length > 0 ? t : e
              var n = this.row.leaves
              return n.indexOf(this) === n.length - 1 ? e : t
            },
          },
          {
            key: 'xOffset',
            get: function() {
              if (this.rows) return 0
              if (this.leaves) return this.container._width
              var e = this.row,
                t = e.leaves,
                n = e.xOffset,
                r = e._width
              return n + (t.indexOf(this) + 1) * r
            },
          },
        ]) && Yi(t.prototype, n),
        r && Yi(t, r),
        e
      )
    })()
  function ds(e, t, n) {
    return (
      Math.abs(t.start - e.start) < n || (t.start > e.start && t.start < e.end)
    )
  }
  function ps(e) {
    for (
      var t = e.events,
        n = e.minimumStartDifference,
        r = e.slotMetrics,
        o = e.accessors,
        a = (function(e) {
          for (
            var t = us(e, [
                'startMs',
                function(e) {
                  return -e.endMs
                },
              ]),
              n = [];
            t.length > 0;

          ) {
            var r = t.shift()
            n.push(r)
            for (var o = 0; o < t.length; o++) {
              var a = t[o]
              if (!(r.endMs > a.startMs)) {
                if (o > 0) {
                  var i = t.splice(o, 1)[0]
                  n.push(i)
                }
                break
              }
            }
          }
          return n
        })(
          t.map(function(e) {
            return new fs(e, { slotMetrics: r, accessors: o })
          })
        ),
        i = [],
        s = function(e) {
          var t = a[e],
            r = i.find(function(e) {
              return e.end > t.start || Math.abs(t.start - e.start) < n
            })
          if (!r) return (t.rows = []), i.push(t), 'continue'
          t.container = r
          for (var o = null, s = r.rows.length - 1; !o && s >= 0; s--)
            ds(r.rows[s], t, n) && (o = r.rows[s])
          o
            ? (o.leaves.push(t), (t.row = o))
            : ((t.leaves = []), r.rows.push(t))
        },
        c = 0;
      c < a.length;
      c++
    )
      s(c)
    return a.map(function(e) {
      return {
        event: e.data,
        style: {
          top: e.top,
          height: e.height,
          width: e.width,
          xOffset: Math.max(0, e.xOffset),
        },
      }
    })
  }
  function vs(e, t, n) {
    for (var r = 0; r < e.friends.length; ++r)
      if (!(n.indexOf(e.friends[r]) > -1)) {
        ;(t = t > e.friends[r].idx ? t : e.friends[r].idx), n.push(e.friends[r])
        var o = vs(e.friends[r], t, n)
        t = t > o ? t : o
      }
    return t
  }
  var hs = {
    overlap: ps,
    'no-overlap': function(e) {
      var t = ps({
        events: e.events,
        minimumStartDifference: e.minimumStartDifference,
        slotMetrics: e.slotMetrics,
        accessors: e.accessors,
      })
      t.sort(function(e, t) {
        return (
          (e = e.style),
          (t = t.style),
          e.top !== t.top
            ? e.top > t.top
              ? 1
              : -1
            : e.top + e.height < t.top + t.height
            ? 1
            : -1
        )
      })
      for (var n = 0; n < t.length; ++n)
        (t[n].friends = []),
          delete t[n].style.left,
          delete t[n].style.left,
          delete t[n].idx,
          delete t[n].size
      for (var r = 0; r < t.length - 1; ++r)
        for (
          var o = t[r],
            a = o.style.top,
            i = o.style.top + o.style.height,
            s = r + 1;
          s < t.length;
          ++s
        ) {
          var c = t[s],
            l = c.style.top,
            u = c.style.top + c.style.height
          ;((l <= a && a < u) || (a <= l && l < i)) &&
            (o.friends.push(c), c.friends.push(o))
        }
      for (var f = 0; f < t.length; ++f) {
        for (var d = t[f], p = [], v = 0; v < 100; ++v) p.push(1)
        for (var h = 0; h < d.friends.length; ++h)
          void 0 !== d.friends[h].idx && (p[d.friends[h].idx] = 0)
        d.idx = p.indexOf(1)
      }
      for (var m = 0; m < t.length; ++m) {
        var g
        if (!t[m].size) {
          var y = []
          ;(g = 100 / (vs(t[m], 0, y) + 1)), (t[m].size = g)
          for (var b = 0; b < y.length; ++b) y[b].size = g
        }
      }
      for (var w = 0; w < t.length; ++w) {
        var E = t[w]
        E.style.left = E.idx * E.size
        for (var _ = 0, O = 0; O < E.friends.length; ++O) {
          var M = E.friends[O]
          _ = _ > M ? _ : M
        }
        _ <= E.idx && (E.size = 100 - E.idx * E.size)
        var S = 0 === E.idx ? 0 : 3
        ;(E.style.width = 'calc(' + E.size + '% - ' + S + 'px)'),
          (E.style.height = 'calc(' + E.style.height + '% - 2px)'),
          (E.style.xOffset = 'calc(' + E.style.left + '% + ' + S + 'px)')
      }
      return t
    },
  }
  var ms = (function(e) {
    function t() {
      return e.apply(this, arguments) || this
    }
    return (
      c(t, e),
      (t.prototype.render = function() {
        var e = this.props,
          t = e.renderSlot,
          n = e.resource,
          o = e.group,
          s = e.getters,
          c = e.components,
          l = (c = void 0 === c ? {} : c).timeSlotWrapper,
          u = void 0 === l ? a : l,
          f = s ? s.slotGroupProp() : {}
        return r.createElement(
          'div',
          i({ className: 'rbc-timeslot-group' }, f),
          o.map(function(e, o) {
            var a = s ? s.slotProp(e, n) : {}
            return r.createElement(
              u,
              { key: o, value: e, resource: n },
              r.createElement(
                'div',
                i({}, a, { className: D('rbc-time-slot', a.className) }),
                t && t(e, o)
              )
            )
          })
        )
      }),
      t
    )
  })(t.Component)
  function gs(e) {
    return 'string' == typeof e ? e : e + '%'
  }
  function ys(e) {
    var t,
      n,
      o = e.style,
      a = e.className,
      s = e.event,
      c = e.accessors,
      l = e.rtl,
      u = e.selected,
      f = e.label,
      d = e.continuesEarlier,
      p = e.continuesLater,
      v = e.getters,
      h = e.onClick,
      m = e.onDoubleClick,
      g = e.isBackgroundEvent,
      y = e.onKeyPress,
      b = e.components,
      w = b.event,
      E = b.eventWrapper,
      _ = c.title(s),
      O = c.tooltip(s),
      M = c.end(s),
      S = c.start(s),
      x = v.eventProp(s, S, M, u),
      j = o.height,
      T = o.top,
      k = o.width,
      P = o.xOffset,
      N = [
        r.createElement('div', { key: '1', className: 'rbc-event-label' }, f),
        r.createElement(
          'div',
          { key: '2', className: 'rbc-event-content' },
          w ? r.createElement(w, { event: s, title: _ }) : _
        ),
      ],
      A = i(
        {},
        x.style,
        g
          ? (((t = {
              top: gs(T),
              height: gs(j),
              width: 'calc(' + k + ' + 10px)',
            })[l ? 'right' : 'left'] = gs(Math.max(0, P))),
            t)
          : (((n = { top: gs(T), width: gs(k), height: gs(j) })[
              l ? 'right' : 'left'
            ] = gs(P)),
            n)
      )
    return r.createElement(
      E,
      i({ type: 'time' }, e),
      r.createElement(
        'div',
        {
          onClick: h,
          onDoubleClick: m,
          style: A,
          onKeyPress: y,
          title: O ? ('string' == typeof f ? f + ': ' : '') + O : void 0,
          className: D(
            g ? 'rbc-background-event' : 'rbc-event',
            a,
            x.className,
            {
              'rbc-selected': u,
              'rbc-event-continues-earlier': d,
              'rbc-event-continues-later': p,
            }
          ),
        },
        N
      )
    )
  }
  ms.propTypes = {}
  var bs = (function(e) {
    function t() {
      for (var t, o = arguments.length, a = new Array(o), s = 0; s < o; s++)
        a[s] = arguments[s]
      return (
        ((t = e.call.apply(e, [this].concat(a)) || this).state = {
          selecting: !1,
          timeIndicatorPosition: null,
        }),
        (t.intervalTriggered = !1),
        (t.renderEvents = function(e) {
          var n = e.events,
            o = e.isBackgroundEvent,
            a = t.props,
            i = a.rtl,
            s = a.selected,
            c = a.accessors,
            l = a.localizer,
            u = a.getters,
            f = a.components,
            d = a.step,
            p = a.timeslots,
            v = a.dayLayoutAlgorithm,
            h = a.resizable,
            m = C(t).slotMetrics,
            g = l.messages
          return (function(e) {
            e.events, e.minimumStartDifference, e.slotMetrics, e.accessors
            var t,
              n = e.dayLayoutAlgorithm,
              r = n
            return (
              n in hs && (r = hs[n]),
              (t = r) && t.constructor && t.call && t.apply
                ? r.apply(this, arguments)
                : []
            )
          })({
            events: n,
            accessors: c,
            slotMetrics: m,
            minimumStartDifference: Math.ceil((d * p) / 2),
            dayLayoutAlgorithm: v,
          }).map(function(e, n) {
            var a,
              d = e.event,
              p = e.style,
              v = c.end(d),
              y = c.start(d),
              b = 'eventTimeRangeFormat',
              w = m.startsBeforeDay(y),
              E = m.startsAfterDay(v)
            w
              ? (b = 'eventTimeRangeEndFormat')
              : E && (b = 'eventTimeRangeStartFormat'),
              (a = w && E ? g.allDay : l.format({ start: y, end: v }, b))
            var _ = w || m.startsBefore(y),
              O = E || m.startsAfter(v)
            return r.createElement(ys, {
              style: p,
              event: d,
              label: a,
              key: 'evt_' + n,
              getters: u,
              rtl: i,
              components: f,
              continuesEarlier: _,
              continuesLater: O,
              accessors: c,
              selected: Lt(d, s),
              onClick: function(e) {
                return t._select(d, e)
              },
              onDoubleClick: function(e) {
                return t._doubleClick(d, e)
              },
              isBackgroundEvent: o,
              onKeyPress: function(e) {
                return t._keyPress(d, e)
              },
              resizable: h,
            })
          })
        }),
        (t._selectable = function() {
          var e = n.findDOMNode(C(t)),
            r = (t._selector = new Nr(
              function() {
                return n.findDOMNode(C(t))
              },
              { longPressThreshold: t.props.longPressThreshold }
            )),
            o = function(e) {
              var n = t.props.onSelecting,
                r = t.state || {},
                o = a(e),
                i = o.startDate,
                s = o.endDate
              ;(n &&
                ((G(r.startDate, i, 'minutes') && G(r.endDate, s, 'minutes')) ||
                  !1 ===
                    n({ start: i, end: s, resourceId: t.props.resource }))) ||
                (t.state.start === o.start &&
                  t.state.end === o.end &&
                  t.state.selecting === o.selecting) ||
                t.setState(o)
            },
            a = function(n) {
              var r = t.slotMetrics.closestSlotFromPoint(n, Rr(e))
              t.state.selecting || (t._initialSlot = r)
              var o = t._initialSlot
              ee(o, r)
                ? (r = t.slotMetrics.nextSlot(r))
                : Z(o, r) && (o = t.slotMetrics.nextSlot(o))
              var a = t.slotMetrics.getRange(te(o, r), ne(o, r))
              return i({}, a, {
                selecting: !0,
                top: a.top + '%',
                height: a.height + '%',
              })
            },
            s = function(e, r) {
              if (!kr(n.findDOMNode(C(t)), e)) {
                var o = a(e),
                  i = o.startDate,
                  s = o.endDate
                t._selectSlot({ startDate: i, endDate: s, action: r, box: e })
              }
              t.setState({ selecting: !1 })
            }
          r.on('selecting', o),
            r.on('selectStart', o),
            r.on('beforeSelect', function(e) {
              if ('ignoreEvents' === t.props.selectable)
                return !kr(n.findDOMNode(C(t)), e)
            }),
            r.on('click', function(e) {
              return s(e, 'click')
            }),
            r.on('doubleClick', function(e) {
              return s(e, 'doubleClick')
            }),
            r.on('select', function(e) {
              t.state.selecting &&
                (t._selectSlot(i({}, t.state, { action: 'select', bounds: e })),
                t.setState({ selecting: !1 }))
            }),
            r.on('reset', function() {
              t.state.selecting && t.setState({ selecting: !1 })
            })
        }),
        (t._teardownSelectable = function() {
          t._selector && (t._selector.teardown(), (t._selector = null))
        }),
        (t._selectSlot = function(e) {
          for (
            var n = e.startDate,
              r = e.endDate,
              o = e.action,
              a = e.bounds,
              i = e.box,
              s = n,
              c = [];
            ee(s, r);

          )
            c.push(s), (s = new Date(+s + 60 * t.props.step * 1e3))
          k(t.props.onSelectSlot, {
            slots: c,
            start: n,
            end: r,
            resourceId: t.props.resource,
            action: o,
            bounds: a,
            box: i,
          })
        }),
        (t._select = function() {
          for (var e = arguments.length, n = new Array(e), r = 0; r < e; r++)
            n[r] = arguments[r]
          k(t.props.onSelectEvent, n)
        }),
        (t._doubleClick = function() {
          for (var e = arguments.length, n = new Array(e), r = 0; r < e; r++)
            n[r] = arguments[r]
          k(t.props.onDoubleClickEvent, n)
        }),
        (t._keyPress = function() {
          for (var e = arguments.length, n = new Array(e), r = 0; r < e; r++)
            n[r] = arguments[r]
          k(t.props.onKeyPressEvent, n)
        }),
        (t.slotMetrics = Bi(t.props)),
        t
      )
    }
    c(t, e)
    var o = t.prototype
    return (
      (o.componentDidMount = function() {
        this.props.selectable && this._selectable(),
          this.props.isNow && this.setTimeIndicatorPositionUpdateInterval()
      }),
      (o.componentWillUnmount = function() {
        this._teardownSelectable(), this.clearTimeIndicatorInterval()
      }),
      (o.UNSAFE_componentWillReceiveProps = function(e) {
        e.selectable && !this.props.selectable && this._selectable(),
          !e.selectable && this.props.selectable && this._teardownSelectable(),
          (this.slotMetrics = this.slotMetrics.update(e))
      }),
      (o.componentDidUpdate = function(e, t) {
        var n = !G(e.getNow(), this.props.getNow(), 'minutes')
        if (e.isNow !== this.props.isNow || n) {
          if ((this.clearTimeIndicatorInterval(), this.props.isNow)) {
            var r =
              !n &&
              G(e.date, this.props.date, 'minutes') &&
              t.timeIndicatorPosition === this.state.timeIndicatorPosition
            this.setTimeIndicatorPositionUpdateInterval(r)
          }
        } else
          !this.props.isNow ||
            (G(e.min, this.props.min, 'minutes') &&
              G(e.max, this.props.max, 'minutes')) ||
            this.positionTimeIndicator()
      }),
      (o.setTimeIndicatorPositionUpdateInterval = function(e) {
        var t = this
        void 0 === e && (e = !1),
          this.intervalTriggered || e || this.positionTimeIndicator(),
          (this._timeIndicatorTimeout = window.setTimeout(function() {
            ;(t.intervalTriggered = !0),
              t.positionTimeIndicator(),
              t.setTimeIndicatorPositionUpdateInterval()
          }, 6e4))
      }),
      (o.clearTimeIndicatorInterval = function() {
        ;(this.intervalTriggered = !1),
          window.clearTimeout(this._timeIndicatorTimeout)
      }),
      (o.positionTimeIndicator = function() {
        var e = this.props,
          t = e.min,
          n = e.max,
          r = (0, e.getNow)()
        if (r >= t && r <= n) {
          var o = this.slotMetrics.getCurrentTimePosition(r)
          ;(this.intervalTriggered = !0),
            this.setState({ timeIndicatorPosition: o })
        } else this.clearTimeIndicatorInterval()
      }),
      (o.render = function() {
        var e = this.props,
          t = e.max,
          n = e.rtl,
          o = e.isNow,
          a = e.resource,
          i = e.accessors,
          c = e.localizer,
          l = e.getters,
          u = l.dayProp,
          f = s(l, ['dayProp']),
          d = e.components,
          p = d.eventContainerWrapper,
          v = s(d, ['eventContainerWrapper']),
          h = this.slotMetrics,
          m = this.state,
          g = m.selecting,
          y = m.top,
          b = m.height,
          w = { start: m.startDate, end: m.endDate },
          E = u(t),
          _ = E.className,
          O = E.style
        return r.createElement(
          'div',
          {
            style: O,
            className: D(
              _,
              'rbc-day-slot',
              'rbc-time-column',
              o && 'rbc-now',
              o && 'rbc-today',
              g && 'rbc-slot-selecting'
            ),
          },
          h.groups.map(function(e, t) {
            return r.createElement(ms, {
              key: t,
              group: e,
              resource: a,
              getters: f,
              components: v,
            })
          }),
          r.createElement(
            p,
            {
              localizer: c,
              resource: a,
              accessors: i,
              getters: f,
              components: v,
              slotMetrics: h,
            },
            r.createElement(
              'div',
              { className: D('rbc-events-container', n && 'rtl') },
              this.renderEvents({
                events: this.props.backgroundEvents,
                isBackgroundEvent: !0,
              }),
              this.renderEvents({ events: this.props.events })
            )
          ),
          g &&
            r.createElement(
              'div',
              { className: 'rbc-slot-selection', style: { top: y, height: b } },
              r.createElement('span', null, c.format(w, 'selectRangeFormat'))
            ),
          o &&
            this.intervalTriggered &&
            r.createElement('div', {
              className: 'rbc-current-time-indicator',
              style: { top: this.state.timeIndicatorPosition + '%' },
            })
        )
      }),
      t
    )
  })(r.Component)
  ;(bs.propTypes = {}),
    (bs.defaultProps = { dragThroughEvents: !0, timeslots: 2 })
  var ws,
    Es = (function(e) {
      function t() {
        for (var t, n = arguments.length, o = new Array(n), a = 0; a < n; a++)
          o[a] = arguments[a]
        ;(t = e.call.apply(e, [this].concat(o)) || this).renderSlot = function(
          e,
          n
        ) {
          if (0 !== n) return null
          var o = t.props,
            a = o.localizer,
            i = o.getNow,
            s = t.slotMetrics.dateIsInGroup(i(), n)
          return r.createElement(
            'span',
            { className: D('rbc-label', s && 'rbc-now') },
            a.format(e, 'timeGutterFormat')
          )
        }
        var i = t.props,
          s = i.min,
          c = i.max,
          l = i.timeslots,
          u = i.step
        return (
          (t.slotMetrics = Bi({ min: s, max: c, timeslots: l, step: u })), t
        )
      }
      c(t, e)
      var n = t.prototype
      return (
        (n.UNSAFE_componentWillReceiveProps = function(e) {
          var t = e.min,
            n = e.max,
            r = e.timeslots,
            o = e.step
          this.slotMetrics = this.slotMetrics.update({
            min: t,
            max: n,
            timeslots: r,
            step: o,
          })
        }),
        (n.render = function() {
          var e = this,
            t = this.props,
            n = t.resource,
            o = t.components,
            a = t.getters
          return r.createElement(
            'div',
            { className: 'rbc-time-gutter rbc-time-column' },
            this.slotMetrics.groups.map(function(t, i) {
              return r.createElement(ms, {
                key: i,
                group: t,
                resource: n,
                components: o,
                renderSlot: e.renderSlot,
                getters: a,
              })
            })
          )
        }),
        t
      )
    })(t.Component)
  function _s(e, t) {
    var n = wt(e)
    return n ? n.innerWidth : t ? e.clientWidth : Mt(e).width
  }
  function Os(e) {
    if (((!ws && 0 !== ws) || e) && jt) {
      var t = document.createElement('div')
      ;(t.style.position = 'absolute'),
        (t.style.top = '-9999px'),
        (t.style.width = '50px'),
        (t.style.height = '50px'),
        (t.style.overflow = 'scroll'),
        document.body.appendChild(t),
        (ws = t.offsetWidth - t.clientWidth),
        document.body.removeChild(t)
    }
    return ws
  }
  Es.propTypes = {}
  var Ms = function(e) {
    var t = e.label
    return r.createElement(r.Fragment, null, t)
  }
  Ms.propTypes = {}
  var Ss = (function(e) {
    function t() {
      for (var t, n = arguments.length, o = new Array(n), a = 0; a < n; a++)
        o[a] = arguments[a]
      return (
        ((t =
          e.call.apply(e, [this].concat(o)) ||
          this).handleHeaderClick = function(e, n, r) {
          r.preventDefault(), k(t.props.onDrillDown, [e, n])
        }),
        (t.renderRow = function(e) {
          var n = t.props,
            o = n.events,
            a = n.rtl,
            i = n.selectable,
            s = n.getNow,
            c = n.range,
            l = n.getters,
            u = n.localizer,
            f = n.accessors,
            d = n.components,
            p = n.resizable,
            v = f.resourceId(e),
            h = e
              ? o.filter(function(e) {
                  return f.resource(e) === v
                })
              : o
          return r.createElement(zi, {
            isAllDay: !0,
            rtl: a,
            getNow: s,
            minRows: 2,
            range: c,
            events: h,
            resourceId: v,
            className: 'rbc-allday-cell',
            selectable: i,
            selected: t.props.selected,
            components: d,
            accessors: f,
            getters: l,
            localizer: u,
            onSelect: t.props.onSelectEvent,
            onDoubleClick: t.props.onDoubleClickEvent,
            onKeyPress: t.props.onKeyPressEvent,
            onSelectSlot: t.props.onSelectSlot,
            longPressThreshold: t.props.longPressThreshold,
            resizable: p,
          })
        }),
        t
      )
    }
    c(t, e)
    var n = t.prototype
    return (
      (n.renderHeaderCells = function(e) {
        var t = this,
          n = this.props,
          o = n.localizer,
          a = n.getDrilldownView,
          i = n.getNow,
          s = n.getters.dayProp,
          c = n.components.header,
          l = void 0 === c ? Ii : c,
          u = i()
        return e.map(function(e, n) {
          var i = a(e),
            c = o.format(e, 'dayFormat'),
            f = s(e),
            d = f.className,
            p = f.style,
            v = r.createElement(l, { date: e, label: c, localizer: o })
          return r.createElement(
            'div',
            {
              key: n,
              style: p,
              className: D('rbc-header', d, G(e, u, 'day') && 'rbc-today'),
            },
            i
              ? r.createElement(
                  'a',
                  {
                    href: '#',
                    onClick: function(n) {
                      return t.handleHeaderClick(e, i, n)
                    },
                  },
                  v
                )
              : r.createElement('span', null, v)
          )
        })
      }),
      (n.render = function() {
        var e = this,
          t = this.props,
          n = t.width,
          o = t.rtl,
          a = t.resources,
          i = t.range,
          s = t.events,
          c = t.getNow,
          l = t.accessors,
          u = t.selectable,
          f = t.components,
          d = t.getters,
          p = t.scrollRef,
          v = t.localizer,
          h = t.isOverflowing,
          m = t.components,
          g = m.timeGutterHeader,
          y = m.resourceHeader,
          b = void 0 === y ? Ms : y,
          w = t.resizable,
          E = {}
        h && (E[o ? 'marginLeft' : 'marginRight'] = Os() + 'px')
        var _ = a.groupEvents(s)
        return r.createElement(
          'div',
          {
            style: E,
            ref: p,
            className: D('rbc-time-header', h && 'rbc-overflowing'),
          },
          r.createElement(
            'div',
            {
              className: 'rbc-label rbc-time-header-gutter',
              style: { width: n, minWidth: n, maxWidth: n },
            },
            g && r.createElement(g, null)
          ),
          a.map(function(t, n) {
            var a = t[0],
              s = t[1]
            return r.createElement(
              'div',
              { className: 'rbc-time-header-content', key: a || n },
              s &&
                r.createElement(
                  'div',
                  {
                    className: 'rbc-row rbc-row-resource',
                    key: 'resource_' + n,
                  },
                  r.createElement(
                    'div',
                    { className: 'rbc-header' },
                    r.createElement(b, {
                      index: n,
                      label: l.resourceTitle(s),
                      resource: s,
                    })
                  )
                ),
              r.createElement(
                'div',
                {
                  className:
                    'rbc-row rbc-time-header-cell' +
                    (i.length <= 1 ? ' rbc-time-header-cell-single-day' : ''),
                },
                e.renderHeaderCells(i)
              ),
              r.createElement(zi, {
                isAllDay: !0,
                rtl: o,
                getNow: c,
                minRows: 2,
                range: i,
                events: _.get(a) || [],
                resourceId: s && a,
                className: 'rbc-allday-cell',
                selectable: u,
                selected: e.props.selected,
                components: f,
                accessors: l,
                getters: d,
                localizer: v,
                onSelect: e.props.onSelectEvent,
                onDoubleClick: e.props.onDoubleClickEvent,
                onKeyPress: e.props.onKeyPressEvent,
                onSelectSlot: e.props.onSelectSlot,
                longPressThreshold: e.props.longPressThreshold,
                resizable: w,
              })
            )
          })
        )
      }),
      t
    )
  })(r.Component)
  Ss.propTypes = {}
  var Ds = {}
  var xs = (function(e) {
    function t(t) {
      var o
      return (
        ((o = e.call(this, t) || this).handleScroll = function(e) {
          o.scrollRef.current &&
            (o.scrollRef.current.scrollLeft = e.target.scrollLeft)
        }),
        (o.handleResize = function() {
          At(o.rafHandle), (o.rafHandle = Rt(o.checkOverflow))
        }),
        (o.gutterRef = function(e) {
          o.gutter = e && n.findDOMNode(e)
        }),
        (o.handleSelectAlldayEvent = function() {
          o.clearSelection()
          for (var e = arguments.length, t = new Array(e), n = 0; n < e; n++)
            t[n] = arguments[n]
          k(o.props.onSelectEvent, t)
        }),
        (o.handleSelectAllDaySlot = function(e, t) {
          k(o.props.onSelectSlot, {
            slots: e,
            start: e[0],
            end: e[e.length - 1],
            action: t.action,
            resourceId: t.resourceId,
          })
        }),
        (o.checkOverflow = function() {
          if (!o._updatingOverflow) {
            var e = o.contentRef.current,
              t = e.scrollHeight > e.clientHeight
            o.state.isOverflowing !== t &&
              ((o._updatingOverflow = !0),
              o.setState({ isOverflowing: t }, function() {
                o._updatingOverflow = !1
              }))
          }
        }),
        (o.memoizedResources = Ci(function(e, t) {
          return (function(e, t) {
            return {
              map: function(n) {
                return e
                  ? e.map(function(e, r) {
                      return n([t.resourceId(e), e], r)
                    })
                  : [n([Ds, null], 0)]
              },
              groupEvents: function(n) {
                var r = new Map()
                return e
                  ? (n.forEach(function(e) {
                      var n = t.resource(e) || Ds,
                        o = r.get(n) || []
                      o.push(e), r.set(n, o)
                    }),
                    r)
                  : (r.set(Ds, n), r)
              },
            }
          })(e, t)
        })),
        (o.state = { gutterWidth: void 0, isOverflowing: null }),
        (o.scrollRef = r.createRef()),
        (o.contentRef = r.createRef()),
        (o._scrollRatio = null),
        o
      )
    }
    c(t, e)
    var o = t.prototype
    return (
      (o.UNSAFE_componentWillMount = function() {
        this.calculateScroll()
      }),
      (o.componentDidMount = function() {
        this.checkOverflow(),
          null == this.props.width && this.measureGutter(),
          this.applyScroll(),
          window.addEventListener('resize', this.handleResize)
      }),
      (o.componentWillUnmount = function() {
        window.removeEventListener('resize', this.handleResize),
          At(this.rafHandle),
          this.measureGutterAnimationFrameRequest &&
            window.cancelAnimationFrame(this.measureGutterAnimationFrameRequest)
      }),
      (o.componentDidUpdate = function() {
        null == this.props.width && this.measureGutter(), this.applyScroll()
      }),
      (o.UNSAFE_componentWillReceiveProps = function(e) {
        var t = this.props,
          n = t.range,
          r = t.scrollToTime
        ;(G(e.range[0], n[0], 'minute') && G(e.scrollToTime, r, 'minute')) ||
          this.calculateScroll(e)
      }),
      (o.renderEvents = function(e, t, n, o) {
        var a = this,
          s = this.props,
          c = s.min,
          l = s.max,
          u = s.components,
          f = s.accessors,
          d = s.localizer,
          p = s.dayLayoutAlgorithm,
          v = this.memoizedResources(this.props.resources, f),
          h = v.groupEvents(t),
          m = v.groupEvents(n)
        return v.map(function(t, n) {
          var s = t[0],
            v = t[1]
          return e.map(function(e, t) {
            var g = (h.get(s) || []).filter(function(t) {
                return re(e, f.start(t), f.end(t), 'day')
              }),
              y = (m.get(s) || []).filter(function(t) {
                return re(e, f.start(t), f.end(t), 'day')
              })
            return r.createElement(
              bs,
              i({}, a.props, {
                localizer: d,
                min: be(e, c),
                max: be(e, l),
                resource: v && s,
                components: u,
                isNow: G(e, o, 'day'),
                key: n + '-' + t,
                date: e,
                events: g,
                backgroundEvents: y,
                dayLayoutAlgorithm: p,
              })
            )
          })
        })
      }),
      (o.render = function() {
        var e = this.props,
          t = e.events,
          n = e.backgroundEvents,
          o = e.range,
          a = e.width,
          i = e.rtl,
          s = e.selected,
          c = e.getNow,
          l = e.resources,
          u = e.components,
          f = e.accessors,
          d = e.getters,
          p = e.localizer,
          v = e.min,
          h = e.max,
          m = e.showMultiDayTimes,
          g = e.longPressThreshold,
          y = e.resizable
        a = a || this.state.gutterWidth
        var b = o[0],
          w = o[o.length - 1]
        this.slots = o.length
        var E = [],
          _ = [],
          O = []
        return (
          t.forEach(function(e) {
            if (Oi(e, b, w, f)) {
              var t = f.start(e),
                n = f.end(e)
              f.allDay(e) || (we(t) && we(n)) || (!m && !G(t, n, 'day'))
                ? E.push(e)
                : _.push(e)
            }
          }),
          n.forEach(function(e) {
            Oi(e, b, w, f) && O.push(e)
          }),
          E.sort(function(e, t) {
            return Si(e, t, f)
          }),
          r.createElement(
            'div',
            { className: D('rbc-time-view', l && 'rbc-time-view-resources') },
            r.createElement(Ss, {
              range: o,
              events: E,
              width: a,
              rtl: i,
              getNow: c,
              localizer: p,
              selected: s,
              resources: this.memoizedResources(l, f),
              selectable: this.props.selectable,
              accessors: f,
              getters: d,
              components: u,
              scrollRef: this.scrollRef,
              isOverflowing: this.state.isOverflowing,
              longPressThreshold: g,
              onSelectSlot: this.handleSelectAllDaySlot,
              onSelectEvent: this.handleSelectAlldayEvent,
              onDoubleClickEvent: this.props.onDoubleClickEvent,
              onKeyPressEvent: this.props.onKeyPressEvent,
              onDrillDown: this.props.onDrillDown,
              getDrilldownView: this.props.getDrilldownView,
              resizable: y,
            }),
            r.createElement(
              'div',
              {
                ref: this.contentRef,
                className: 'rbc-time-content',
                onScroll: this.handleScroll,
              },
              r.createElement(Es, {
                date: b,
                ref: this.gutterRef,
                localizer: p,
                min: be(b, v),
                max: be(b, h),
                step: this.props.step,
                getNow: this.props.getNow,
                timeslots: this.props.timeslots,
                components: u,
                className: 'rbc-time-gutter',
                getters: d,
              }),
              this.renderEvents(o, _, O, c())
            )
          )
        )
      }),
      (o.clearSelection = function() {
        clearTimeout(this._selectTimer), (this._pendingSelection = [])
      }),
      (o.measureGutter = function() {
        var e = this
        this.measureGutterAnimationFrameRequest &&
          window.cancelAnimationFrame(this.measureGutterAnimationFrameRequest),
          (this.measureGutterAnimationFrameRequest = window.requestAnimationFrame(
            function() {
              var t = _s(e.gutter)
              t && e.state.gutterWidth !== t && e.setState({ gutterWidth: t })
            }
          ))
      }),
      (o.applyScroll = function() {
        if (null != this._scrollRatio) {
          var e = this.contentRef.current
          ;(e.scrollTop = e.scrollHeight * this._scrollRatio),
            (this._scrollRatio = null)
        }
      }),
      (o.calculateScroll = function(e) {
        void 0 === e && (e = this.props)
        var t = e,
          n = t.min,
          r = t.max,
          o = t.scrollToTime,
          a = o - X(o, 'day'),
          i = Ee(r, n)
        this._scrollRatio = a / i
      }),
      t
    )
  })(t.Component)
  ;(xs.propTypes = {}),
    (xs.defaultProps = {
      step: 30,
      timeslots: 2,
      min: X(new Date(), 'day'),
      max: q(new Date(), 'day'),
      scrollToTime: X(new Date(), 'day'),
    })
  var js = (function(e) {
    function t() {
      return e.apply(this, arguments) || this
    }
    return (
      c(t, e),
      (t.prototype.render = function() {
        var e = this.props,
          n = e.date,
          o = s(e, ['date']),
          a = t.range(n)
        return r.createElement(xs, i({}, o, { range: a, eventOffset: 10 }))
      }),
      t
    )
  })(r.Component)
  ;(js.propTypes = {}),
    (js.range = function(e) {
      return [X(e, 'day')]
    }),
    (js.navigate = function(e, t) {
      switch (t) {
        case x.PREVIOUS:
          return Y(e, -1, 'day')
        case x.NEXT:
          return Y(e, 1, 'day')
        default:
          return e
      }
    }),
    (js.title = function(e, t) {
      return t.localizer.format(e, 'dayHeaderFormat')
    })
  var Ts = (function(e) {
    function t() {
      return e.apply(this, arguments) || this
    }
    return (
      c(t, e),
      (t.prototype.render = function() {
        var e = this.props,
          n = e.date,
          o = s(e, ['date']),
          a = t.range(n, this.props)
        return r.createElement(xs, i({}, o, { range: a, eventOffset: 15 }))
      }),
      t
    )
  })(r.Component)
  function ks(e, t) {
    return Ts.range(e, t).filter(function(e) {
      return -1 === [6, 0].indexOf(e.getDay())
    })
  }
  ;(Ts.propTypes = {}),
    (Ts.defaultProps = xs.defaultProps),
    (Ts.navigate = function(e, t) {
      switch (t) {
        case x.PREVIOUS:
          return Y(e, -1, 'week')
        case x.NEXT:
          return Y(e, 1, 'week')
        default:
          return e
      }
    }),
    (Ts.range = function(e, t) {
      var n = t.localizer.startOfWeek()
      return ye(X(e, 'week', n), q(e, 'week', n))
    }),
    (Ts.title = function(e, t) {
      var n = t.localizer,
        r = Ts.range(e, { localizer: n }),
        o = r[0],
        a = r.slice(1)
      return n.format({ start: o, end: a.pop() }, 'dayRangeHeaderFormat')
    })
  var Ps,
    Ns = (function(e) {
      function t() {
        return e.apply(this, arguments) || this
      }
      return (
        c(t, e),
        (t.prototype.render = function() {
          var e = this.props,
            t = e.date,
            n = s(e, ['date']),
            o = ks(t, this.props)
          return r.createElement(xs, i({}, n, { range: o, eventOffset: 15 }))
        }),
        t
      )
    })(r.Component)
  function As(e, t) {
    return e
      .replace(new RegExp('(^|\\s)' + t + '(?:\\s|$)', 'g'), '$1')
      .replace(/\s+/g, ' ')
      .replace(/^\s*|\s*$/g, '')
  }
  function Rs(e) {
    var n = e.selected,
      o = e.getters,
      a = e.accessors,
      i = e.localizer,
      s = e.components,
      c = e.length,
      l = e.date,
      u = e.events,
      f = t.useRef(null),
      d = t.useRef(null),
      p = t.useRef(null),
      v = t.useRef(null),
      h = t.useRef(null)
    t.useEffect(function() {
      g()
    })
    var m = function(e, t) {
        var n = '',
          o = s.time,
          c = i.messages.allDay,
          l = a.end(t),
          u = a.start(t)
        return (
          a.allDay(t) ||
            (G(u, l)
              ? (c = i.format(u, 'agendaTimeFormat'))
              : G(u, l, 'day')
              ? (c = i.format({ start: u, end: l }, 'agendaTimeRangeFormat'))
              : G(e, u, 'day')
              ? (c = i.format(u, 'agendaTimeFormat'))
              : G(e, l, 'day') && (c = i.format(l, 'agendaTimeFormat'))),
          Z(e, u, 'day') && (n = 'rbc-continues-prior'),
          Q(e, l, 'day') && (n += ' rbc-continues-after'),
          r.createElement(
            'span',
            { className: n.trim() },
            o ? r.createElement(o, { event: t, day: e, label: c }) : c
          )
        )
      },
      g = function() {
        if (h.current) {
          var e = f.current,
            t = h.current.firstChild
          if (t) {
            var n,
              r,
              o = v.current.scrollHeight > v.current.clientHeight,
              a = [],
              i = a
            ;(a = [_s(t.children[0]), _s(t.children[1])]),
              (i[0] === a[0] && i[1] === a[1]) ||
                ((d.current.style.width = a[0] + 'px'),
                (p.current.style.width = a[1] + 'px')),
              o
                ? ((r = 'rbc-header-overflowing'),
                  (n = e).classList
                    ? n.classList.add(r)
                    : (function(e, t) {
                        return e.classList
                          ? !!t && e.classList.contains(t)
                          : -1 !==
                              (
                                ' ' +
                                (e.className.baseVal || e.className) +
                                ' '
                              ).indexOf(' ' + t + ' ')
                      })(n, r) ||
                      ('string' == typeof n.className
                        ? (n.className = n.className + ' ' + r)
                        : n.setAttribute(
                            'class',
                            ((n.className && n.className.baseVal) || '') +
                              ' ' +
                              r
                          )),
                  (e.style.marginRight = Os() + 'px'))
                : (function(e, t) {
                    e.classList
                      ? e.classList.remove(t)
                      : 'string' == typeof e.className
                      ? (e.className = As(e.className, t))
                      : e.setAttribute(
                          'class',
                          As((e.className && e.className.baseVal) || '', t)
                        )
                  })(e, 'rbc-header-overflowing')
          }
        }
      },
      y = i.messages,
      b = Y(l, c, 'day'),
      w = ye(l, b, 'day')
    return (
      (u = u.filter(function(e) {
        return Oi(e, l, b, a)
      })).sort(function(e, t) {
        return +a.start(e) - +a.start(t)
      }),
      r.createElement(
        'div',
        { className: 'rbc-agenda-view' },
        0 !== u.length
          ? r.createElement(
              r.Fragment,
              null,
              r.createElement(
                'table',
                { ref: f, className: 'rbc-agenda-table' },
                r.createElement(
                  'thead',
                  null,
                  r.createElement(
                    'tr',
                    null,
                    r.createElement(
                      'th',
                      { className: 'rbc-header', ref: d },
                      y.date
                    ),
                    r.createElement(
                      'th',
                      { className: 'rbc-header', ref: p },
                      y.time
                    ),
                    r.createElement('th', { className: 'rbc-header' }, y.event)
                  )
                )
              ),
              r.createElement(
                'div',
                { className: 'rbc-agenda-content', ref: v },
                r.createElement(
                  'table',
                  { className: 'rbc-agenda-table' },
                  r.createElement(
                    'tbody',
                    { ref: h },
                    w.map(function(e, t) {
                      return (function(e, t, c) {
                        var l = s.event,
                          u = s.date
                        return (t = t.filter(function(t) {
                          return Oi(t, X(e, 'day'), q(e, 'day'), a)
                        })).map(function(s, f) {
                          var d = a.title(s),
                            p = a.end(s),
                            v = a.start(s),
                            h = o.eventProp(s, v, p, Lt(s, n)),
                            g = 0 === f && i.format(e, 'agendaDateFormat'),
                            y =
                              0 === f &&
                              r.createElement(
                                'td',
                                {
                                  rowSpan: t.length,
                                  className: 'rbc-agenda-date-cell',
                                },
                                u ? r.createElement(u, { day: e, label: g }) : g
                              )
                          return r.createElement(
                            'tr',
                            {
                              key: c + '_' + f,
                              className: h.className,
                              style: h.style,
                            },
                            y,
                            r.createElement(
                              'td',
                              { className: 'rbc-agenda-time-cell' },
                              m(e, s)
                            ),
                            r.createElement(
                              'td',
                              { className: 'rbc-agenda-event-cell' },
                              l ? r.createElement(l, { event: s, title: d }) : d
                            )
                          )
                        }, [])
                      })(e, u, t)
                    })
                  )
                )
              )
            )
          : r.createElement(
              'span',
              { className: 'rbc-agenda-empty' },
              y.noEventsInRange
            )
      )
    )
  }
  ;(Ns.propTypes = {}),
    (Ns.defaultProps = xs.defaultProps),
    (Ns.range = ks),
    (Ns.navigate = Ts.navigate),
    (Ns.title = function(e, t) {
      var n = t.localizer,
        r = ks(e, { localizer: n }),
        o = r[0],
        a = r.slice(1)
      return n.format({ start: o, end: a.pop() }, 'dayRangeHeaderFormat')
    }),
    (Rs.propTypes = {}),
    (Rs.defaultProps = { length: 30 }),
    (Rs.range = function(e, t) {
      var n = t.length
      return {
        start: e,
        end: Y(e, void 0 === n ? Rs.defaultProps.length : n, 'day'),
      }
    }),
    (Rs.navigate = function(e, t, n) {
      var r = n.length,
        o = void 0 === r ? Rs.defaultProps.length : r
      switch (t) {
        case x.PREVIOUS:
          return Y(e, -o, 'day')
        case x.NEXT:
          return Y(e, o, 'day')
        default:
          return e
      }
    }),
    (Rs.title = function(e, t) {
      var n = t.length,
        r = void 0 === n ? Rs.defaultProps.length : n,
        o = t.localizer,
        a = Y(e, r, 'day')
      return o.format({ start: e, end: a }, 'agendaHeaderFormat')
    })
  var Cs = (((Ps = {})[j.MONTH] = Vi),
  (Ps[j.WEEK] = Ts),
  (Ps[j.WORK_WEEK] = Ns),
  (Ps[j.DAY] = js),
  (Ps[j.AGENDA] = Rs),
  Ps)
  function Ls(e, t) {
    var n = t.action,
      r = t.date,
      o = t.today,
      a = s(t, ['action', 'date', 'today'])
    switch (((e = 'string' == typeof e ? Cs[e] : e), n)) {
      case x.TODAY:
        r = o || new Date()
        break
      case x.DATE:
        break
      default:
        ;(e && 'function' == typeof e.navigate) || E(!1),
          (r = e.navigate(r, n, a))
    }
    return r
  }
  var Fs = (function(e) {
    function t() {
      for (var t, n = arguments.length, r = new Array(n), o = 0; o < n; o++)
        r[o] = arguments[o]
      return (
        ((t = e.call.apply(e, [this].concat(r)) || this).navigate = function(
          e
        ) {
          t.props.onNavigate(e)
        }),
        (t.view = function(e) {
          t.props.onView(e)
        }),
        t
      )
    }
    c(t, e)
    var n = t.prototype
    return (
      (n.render = function() {
        var e = this.props,
          t = e.localizer.messages,
          n = e.label
        return r.createElement(
          'div',
          { className: 'rbc-toolbar' },
          r.createElement(
            'span',
            { className: 'rbc-btn-group' },
            r.createElement(
              'button',
              { type: 'button', onClick: this.navigate.bind(null, x.TODAY) },
              t.today
            ),
            r.createElement(
              'button',
              { type: 'button', onClick: this.navigate.bind(null, x.PREVIOUS) },
              t.previous
            ),
            r.createElement(
              'button',
              { type: 'button', onClick: this.navigate.bind(null, x.NEXT) },
              t.next
            )
          ),
          r.createElement('span', { className: 'rbc-toolbar-label' }, n),
          r.createElement(
            'span',
            { className: 'rbc-btn-group' },
            this.viewNamesGroup(t)
          )
        )
      }),
      (n.viewNamesGroup = function(e) {
        var t = this,
          n = this.props.views,
          o = this.props.view
        if (n.length > 1)
          return n.map(function(n) {
            return r.createElement(
              'button',
              {
                type: 'button',
                key: n,
                className: D({ 'rbc-active': o === n }),
                onClick: t.view.bind(null, n),
              },
              e[n]
            )
          })
      }),
      t
    )
  })(r.Component)
  function zs(e, t) {
    for (
      var n = -1, r = null == e ? 0 : e.length;
      ++n < r && !1 !== t(e[n], n, e);

    );
    return e
  }
  function Is(e, t, n) {
    '__proto__' == t && rs
      ? rs(e, t, { configurable: !0, enumerable: !0, value: n, writable: !0 })
      : (e[t] = n)
  }
  Fs.propTypes = {}
  var Ws = Object.prototype.hasOwnProperty
  function Hs(e, t, n) {
    var r = e[t]
    ;(Ws.call(e, t) && Oe(r, n) && (void 0 !== n || t in e)) || Is(e, t, n)
  }
  function Vs(e, t, n, r) {
    var o = !n
    n || (n = {})
    for (var a = -1, i = t.length; ++a < i; ) {
      var s = t[a],
        c = r ? r(n[s], e[s], s, n, e) : void 0
      void 0 === c && (c = e[s]), o ? Is(n, s, c) : Hs(n, s, c)
    }
    return n
  }
  var Us = Object.prototype.hasOwnProperty
  function Ks(e) {
    if (!Fe(e))
      return (function(e) {
        var t = []
        if (null != e) for (var n in Object(e)) t.push(n)
        return t
      })(e)
    var t = pa(e),
      n = []
    for (var r in e) ('constructor' != r || (!t && Us.call(e, r))) && n.push(r)
    return n
  }
  function Bs(e) {
    return Be(e) ? fa(e, !0) : Ks(e)
  }
  var Ys = 'object' == typeof e && e && !e.nodeType && e,
    $s =
      Ys && 'object' == typeof module && module && !module.nodeType && module,
    Xs = $s && $s.exports === Ys ? De.Buffer : void 0,
    qs = Xs ? Xs.allocUnsafe : void 0
  var Gs = va(Object.getPrototypeOf, Object),
    Zs = Object.getOwnPropertySymbols
      ? function(e) {
          for (var t = []; e; ) Io(t, Bo(e)), (e = Gs(e))
          return t
        }
      : Vo
  function Js(e) {
    return Ho(e, Bs, Zs)
  }
  var Qs = Object.prototype.hasOwnProperty
  function ec(e) {
    var t = new e.constructor(e.byteLength)
    return new Eo(t).set(new Eo(e)), t
  }
  var tc = /\w*$/
  var nc = xe ? xe.prototype : void 0,
    rc = nc ? nc.valueOf : void 0
  var oc = '[object Boolean]',
    ac = '[object Date]',
    ic = '[object Map]',
    sc = '[object Number]',
    cc = '[object RegExp]',
    lc = '[object Set]',
    uc = '[object String]',
    fc = '[object Symbol]',
    dc = '[object ArrayBuffer]',
    pc = '[object DataView]',
    vc = '[object Float32Array]',
    hc = '[object Float64Array]',
    mc = '[object Int8Array]',
    gc = '[object Int16Array]',
    yc = '[object Int32Array]',
    bc = '[object Uint8Array]',
    wc = '[object Uint8ClampedArray]',
    Ec = '[object Uint16Array]',
    _c = '[object Uint32Array]'
  function Oc(e, t, n) {
    var r,
      o,
      a,
      i = e.constructor
    switch (t) {
      case dc:
        return ec(e)
      case oc:
      case ac:
        return new i(+e)
      case pc:
        return (function(e, t) {
          var n = t ? ec(e.buffer) : e.buffer
          return new e.constructor(n, e.byteOffset, e.byteLength)
        })(e, n)
      case vc:
      case hc:
      case mc:
      case gc:
      case yc:
      case bc:
      case wc:
      case Ec:
      case _c:
        return (function(e, t) {
          var n = t ? ec(e.buffer) : e.buffer
          return new e.constructor(n, e.byteOffset, e.length)
        })(e, n)
      case ic:
        return new i()
      case sc:
      case uc:
        return new i(e)
      case cc:
        return (
          ((a = new (o = e).constructor(o.source, tc.exec(o))).lastIndex =
            o.lastIndex),
          a
        )
      case lc:
        return new i()
      case fc:
        return (r = e), rc ? Object(rc.call(r)) : {}
    }
  }
  var Mc = Object.create,
    Sc = (function() {
      function e() {}
      return function(t) {
        if (!Fe(t)) return {}
        if (Mc) return Mc(t)
        e.prototype = t
        var n = new e()
        return (e.prototype = void 0), n
      }
    })()
  var Dc = '[object Map]'
  var xc = sa && sa.isMap,
    jc = xc
      ? ra(xc)
      : function(e) {
          return Ge(e) && Pa(e) == Dc
        },
    Tc = '[object Set]'
  var kc = sa && sa.isSet,
    Pc = kc
      ? ra(kc)
      : function(e) {
          return Ge(e) && Pa(e) == Tc
        },
    Nc = 1,
    Ac = 2,
    Rc = 4,
    Cc = '[object Arguments]',
    Lc = '[object Function]',
    Fc = '[object GeneratorFunction]',
    zc = '[object Object]',
    Ic = {}
  function Wc(e, t, n, r, o, a) {
    var i,
      s = t & Nc,
      c = t & Ac,
      l = t & Rc
    if ((n && (i = o ? n(e, r, o, a) : n(e)), void 0 !== i)) return i
    if (!Fe(e)) return e
    var u = Wo(e)
    if (u) {
      if (
        ((i = (function(e) {
          var t = e.length,
            n = new e.constructor(t)
          return (
            t &&
              'string' == typeof e[0] &&
              Qs.call(e, 'index') &&
              ((n.index = e.index), (n.input = e.input)),
            n
          )
        })(e)),
        !s)
      )
        return (function(e, t) {
          var n = -1,
            r = e.length
          for (t || (t = Array(r)); ++n < r; ) t[n] = e[n]
          return t
        })(e, i)
    } else {
      var f = Pa(e),
        d = f == Lc || f == Fc
      if (ta(e))
        return (function(e, t) {
          if (t) return e.slice()
          var n = e.length,
            r = qs ? qs(n) : new e.constructor(n)
          return e.copy(r), r
        })(e, s)
      if (f == zc || f == Cc || (d && !o)) {
        if (
          ((i =
            c || d
              ? {}
              : (function(e) {
                  return 'function' != typeof e.constructor || pa(e)
                    ? {}
                    : Sc(Gs(e))
                })(e)),
          !s)
        )
          return c
            ? (function(e, t) {
                return Vs(e, Zs(e), t)
              })(
                e,
                (function(e, t) {
                  return e && Vs(t, Bs(t), e)
                })(i, e)
              )
            : (function(e, t) {
                return Vs(e, Bo(e), t)
              })(
                e,
                (function(e, t) {
                  return e && Vs(t, ga(t), e)
                })(i, e)
              )
      } else {
        if (!Ic[f]) return o ? e : {}
        i = Oc(e, f, s)
      }
    }
    a || (a = new vo())
    var p = a.get(e)
    if (p) return p
    if ((a.set(e, i), Pc(e)))
      return (
        e.forEach(function(r) {
          i.add(Wc(r, t, n, r, e, a))
        }),
        i
      )
    if (jc(e))
      return (
        e.forEach(function(r, o) {
          i.set(o, Wc(r, t, n, o, e, a))
        }),
        i
      )
    var v = l ? (c ? Js : ya) : c ? keysIn : ga,
      h = u ? void 0 : v(e)
    return (
      zs(h || e, function(r, o) {
        h && (r = e[(o = r)]), Hs(i, o, Wc(r, t, n, o, e, a))
      }),
      i
    )
  }
  function Hc(e, t) {
    return (
      null ==
        (e = (function(e, t) {
          return t.length < 2 ? e : ui(e, _e(t, 0, -1))
        })(e, (t = si(t, e)))) ||
      delete e[
        li(((n = t), (r = null == n ? 0 : n.length), r ? n[r - 1] : void 0))
      ]
    )
    var n, r
  }
  ;(Ic[Cc] = Ic['[object Array]'] = Ic['[object ArrayBuffer]'] = Ic[
    '[object DataView]'
  ] = Ic['[object Boolean]'] = Ic['[object Date]'] = Ic[
    '[object Float32Array]'
  ] = Ic['[object Float64Array]'] = Ic['[object Int8Array]'] = Ic[
    '[object Int16Array]'
  ] = Ic['[object Int32Array]'] = Ic['[object Map]'] = Ic[
    '[object Number]'
  ] = Ic[zc] = Ic['[object RegExp]'] = Ic['[object Set]'] = Ic[
    '[object String]'
  ] = Ic['[object Symbol]'] = Ic['[object Uint8Array]'] = Ic[
    '[object Uint8ClampedArray]'
  ] = Ic['[object Uint16Array]'] = Ic['[object Uint32Array]'] = !0),
    (Ic['[object Error]'] = Ic[Lc] = Ic['[object WeakMap]'] = !1)
  var Vc = '[object Object]',
    Uc = Function.prototype,
    Kc = Object.prototype,
    Bc = Uc.toString,
    Yc = Kc.hasOwnProperty,
    $c = Bc.call(Object)
  function Xc(e) {
    return (function(e) {
      if (!Ge(e) || Le(e) != Vc) return !1
      var t = Gs(e)
      if (null === t) return !0
      var n = Yc.call(t, 'constructor') && t.constructor
      return 'function' == typeof n && n instanceof n && Bc.call(n) == $c
    })(e)
      ? void 0
      : e
  }
  function qc(e) {
    return (null == e ? 0 : e.length) ? qi(e, 1) : []
  }
  var Gc = (function(e) {
      return cs(ns(e, void 0, qc), e + '')
    })(function(e, t) {
      var n = {}
      if (null == e) return n
      var r = !1
      ;(t = ni(t, function(t) {
        return (t = si(t, e)), r || (r = t.length > 1), t
      })),
        Vs(e, Js(e), n),
        r && (n = Wc(n, 7, Xc))
      for (var o = t.length; o--; ) Hc(n, t[o])
      return n
    }),
    Zc = Object.prototype,
    Jc = Zc.hasOwnProperty,
    Qc = ls(function(e, t) {
      e = Object(e)
      var n = -1,
        r = t.length,
        o = r > 2 ? t[2] : void 0
      for (o && qe(t[0], t[1], o) && (r = 1); ++n < r; )
        for (var a = t[n], i = Bs(a), s = -1, c = i.length; ++s < c; ) {
          var l = i[s],
            u = e[l]
          ;(void 0 === u || (Oe(u, Zc[l]) && !Jc.call(e, l))) && (e[l] = a[l])
        }
      return e
    })
  var el = function(e) {
    return function(t) {
      return (function(e, t) {
        var n = null
        return (
          'function' == typeof t
            ? (n = t(e))
            : 'string' == typeof t &&
              'object' == typeof e &&
              null != e &&
              t in e &&
              (n = e[t]),
          n
        )
      })(t, e)
    }
  }
  function tl(e) {
    return Array.isArray(e) ? e : Object.keys(e)
  }
  var nl = (function(e) {
    function t() {
      for (var t, n = arguments.length, r = new Array(n), o = 0; o < n; o++)
        r[o] = arguments[o]
      return (
        ((t = e.call.apply(e, [this].concat(r)) || this).getViews = function() {
          var e,
            n,
            r,
            o = t.props.views
          return Array.isArray(o)
            ? (function(e, t, n) {
                var r = Wo(e),
                  o = r || ta(e) || la(e)
                if (((t = yi(t)), null == n)) {
                  var a = e && e.constructor
                  n = o ? (r ? new a() : []) : Fe(e) && Ve(a) ? Sc(Gs(e)) : {}
                }
                return (
                  (o ? zs : Zi)(e, function(e, r, o) {
                    return t(n, e, r, o)
                  }),
                  n
                )
              })(
                o,
                function(e, t) {
                  return (e[t] = Cs[t])
                },
                {}
              )
            : 'object' == typeof o
            ? ((e = o),
              (r = {}),
              (n = yi(
                (n = function(e, t) {
                  return !0 === e ? Cs[t] : e
                })
              )),
              Zi(e, function(e, t, o) {
                Is(r, t, n(e, t, o))
              }),
              r)
            : Cs
        }),
        (t.getView = function() {
          return t.getViews()[t.props.view]
        }),
        (t.getDrilldownView = function(e) {
          var n = t.props,
            r = n.view,
            o = n.drilldownView,
            a = n.getDrilldownView
          return a ? a(e, r, Object.keys(t.getViews())) : o
        }),
        (t.handleRangeChange = function(e, n, r) {
          var o = t.props,
            a = o.onRangeChange,
            i = o.localizer
          a && n.range && a(n.range(e, { localizer: i }), r)
        }),
        (t.handleNavigate = function(e, n) {
          var r = t.props,
            o = r.view,
            a = r.date,
            c = r.getNow,
            l = r.onNavigate,
            u = s(r, ['view', 'date', 'getNow', 'onNavigate']),
            f = t.getView(),
            d = c()
          l(
            (a = Ls(f, i({}, u, { action: e, date: n || a || d, today: d }))),
            o,
            e
          ),
            t.handleRangeChange(a, f)
        }),
        (t.handleViewChange = function(e) {
          e !== t.props.view &&
            (function(e, t) {
              return -1 !== tl(t.views).indexOf(e)
            })(e, t.props) &&
            t.props.onView(e)
          var n = t.getViews()
          t.handleRangeChange(t.props.date || t.props.getNow(), n[e], e)
        }),
        (t.handleSelectEvent = function() {
          for (var e = arguments.length, n = new Array(e), r = 0; r < e; r++)
            n[r] = arguments[r]
          k(t.props.onSelectEvent, n)
        }),
        (t.handleDoubleClickEvent = function() {
          for (var e = arguments.length, n = new Array(e), r = 0; r < e; r++)
            n[r] = arguments[r]
          k(t.props.onDoubleClickEvent, n)
        }),
        (t.handleKeyPressEvent = function() {
          for (var e = arguments.length, n = new Array(e), r = 0; r < e; r++)
            n[r] = arguments[r]
          k(t.props.onKeyPressEvent, n)
        }),
        (t.handleSelectSlot = function(e) {
          k(t.props.onSelectSlot, e)
        }),
        (t.handleDrillDown = function(e, n) {
          var r = t.props.onDrillDown
          r
            ? r(e, n, t.drilldownView)
            : (n && t.handleViewChange(n), t.handleNavigate(x.DATE, e))
        }),
        (t.state = { context: t.getContext(t.props) }),
        t
      )
    }
    c(t, e)
    var n = t.prototype
    return (
      (n.UNSAFE_componentWillReceiveProps = function(e) {
        this.setState({ context: this.getContext(e) })
      }),
      (n.getContext = function(e) {
        var t = e.startAccessor,
          n = e.endAccessor,
          r = e.allDayAccessor,
          o = e.tooltipAccessor,
          s = e.titleAccessor,
          c = e.resourceAccessor,
          l = e.resourceIdAccessor,
          u = e.resourceTitleAccessor,
          f = e.eventPropGetter,
          d = e.backgroundEventPropGetter,
          p = e.slotPropGetter,
          v = e.slotGroupPropGetter,
          h = e.dayPropGetter,
          m = e.view,
          g = e.views,
          y = e.localizer,
          b = e.culture,
          w = e.messages,
          E = void 0 === w ? {} : w,
          _ = e.components,
          O = void 0 === _ ? {} : _,
          M = e.formats,
          S = void 0 === M ? {} : M,
          D = tl(g)
        return {
          viewNames: D,
          localizer: A(
            y,
            b,
            S,
            (function(e) {
              return i({}, R, e)
            })(E)
          ),
          getters: {
            eventProp: function() {
              return (f && f.apply(void 0, arguments)) || {}
            },
            backgroundEventProp: function() {
              return (d && d.apply(void 0, arguments)) || {}
            },
            slotProp: function() {
              return (p && p.apply(void 0, arguments)) || {}
            },
            slotGroupProp: function() {
              return (v && v.apply(void 0, arguments)) || {}
            },
            dayProp: function() {
              return (h && h.apply(void 0, arguments)) || {}
            },
          },
          components: Qc(O[m] || {}, Gc(O, D), {
            eventWrapper: a,
            backgroundEventWrapper: a,
            eventContainerWrapper: a,
            dateCellWrapper: a,
            weekWrapper: a,
            timeSlotWrapper: a,
          }),
          accessors: {
            start: el(t),
            end: el(n),
            allDay: el(r),
            tooltip: el(o),
            title: el(s),
            resource: el(c),
            resourceId: el(l),
            resourceTitle: el(u),
          },
        }
      }),
      (n.render = function() {
        var e = this.props,
          t = e.view,
          n = e.toolbar,
          o = e.events,
          a = e.backgroundEvents,
          c = void 0 === a ? [] : a,
          l = e.style,
          u = e.className,
          f = e.elementProps,
          d = e.date,
          p = e.getNow,
          v = e.length,
          h = e.showMultiDayTimes,
          m = e.onShowMore,
          g = (e.components,
          e.formats,
          e.messages,
          e.culture,
          s(e, [
            'view',
            'toolbar',
            'events',
            'backgroundEvents',
            'style',
            'className',
            'elementProps',
            'date',
            'getNow',
            'length',
            'showMultiDayTimes',
            'onShowMore',
            'components',
            'formats',
            'messages',
            'culture',
          ]))
        d = d || p()
        var y = this.getView(),
          b = this.state.context,
          w = b.accessors,
          E = b.components,
          _ = b.getters,
          O = b.localizer,
          M = b.viewNames,
          S = E.toolbar || Fs,
          x = y.title(d, { localizer: O, length: v })
        return r.createElement(
          'div',
          i({}, f, {
            className: D(u, 'rbc-calendar', g.rtl && 'rbc-rtl'),
            style: l,
          }),
          n &&
            r.createElement(S, {
              date: d,
              view: t,
              views: M,
              label: x,
              onView: this.handleViewChange,
              onNavigate: this.handleNavigate,
              localizer: O,
            }),
          r.createElement(
            y,
            i({}, g, {
              events: o,
              backgroundEvents: c,
              date: d,
              getNow: p,
              length: v,
              localizer: O,
              getters: _,
              components: E,
              accessors: w,
              showMultiDayTimes: h,
              getDrilldownView: this.getDrilldownView,
              onNavigate: this.handleNavigate,
              onDrillDown: this.handleDrillDown,
              onSelectEvent: this.handleSelectEvent,
              onDoubleClickEvent: this.handleDoubleClickEvent,
              onKeyPressEvent: this.handleKeyPressEvent,
              onSelectSlot: this.handleSelectSlot,
              onShowMore: m,
            })
          )
        )
      }),
      t
    )
  })(r.Component)
  ;(nl.defaultProps = {
    elementProps: {},
    popup: !1,
    toolbar: !0,
    view: j.MONTH,
    views: [j.MONTH, j.WEEK, j.DAY, j.AGENDA],
    step: 30,
    length: 30,
    drilldownView: j.DAY,
    titleAccessor: 'title',
    tooltipAccessor: 'title',
    allDayAccessor: 'allDay',
    startAccessor: 'start',
    endAccessor: 'end',
    resourceAccessor: 'resourceId',
    resourceIdAccessor: 'id',
    resourceTitleAccessor: 'title',
    longPressThreshold: 250,
    getNow: function() {
      return new Date()
    },
    dayLayoutAlgorithm: 'overlap',
  }),
    (nl.propTypes = {})
  var rl = (function e(t, n, o) {
      void 0 === o && (o = [])
      var a,
        i = t.displayName || t.name || 'Component',
        s =
          !!(a = t) &&
          ('function' != typeof a ||
            (a.prototype && a.prototype.isReactComponent)),
        c = Object.keys(n),
        l = c.map(M)
      !s && o.length && E(!1)
      var u = (function(e) {
        var a, i
        function s() {
          for (var t, r = arguments.length, a = new Array(r), i = 0; i < r; i++)
            a[i] = arguments[i]
          return (
            ((t =
              e.call.apply(e, [this].concat(a)) ||
              this).handlers = Object.create(null)),
            c.forEach(function(e) {
              var r = n[e]
              t.handlers[r] = function(n) {
                if (t.props[r]) {
                  var o
                  t._notifying = !0
                  for (
                    var a = arguments.length,
                      i = new Array(a > 1 ? a - 1 : 0),
                      s = 1;
                    s < a;
                    s++
                  )
                    i[s - 1] = arguments[s]
                  ;(o = t.props)[r].apply(o, [n].concat(i)), (t._notifying = !1)
                }
                ;(t._values[e] = n), t.unmounted || t.forceUpdate()
              }
            }),
            o.length &&
              (t.attachRef = function(e) {
                t.inner = e
              }),
            t
          )
        }
        ;(i = e),
          ((a = s).prototype = Object.create(i.prototype)),
          (a.prototype.constructor = a),
          (a.__proto__ = i)
        var u = s.prototype
        return (
          (u.shouldComponentUpdate = function() {
            return !this._notifying
          }),
          (u.componentWillMount = function() {
            var e = this,
              t = this.props
            ;(this._values = Object.create(null)),
              c.forEach(function(n) {
                e._values[n] = t[M(n)]
              })
          }),
          (u.componentWillReceiveProps = function(e) {
            var t = this,
              n = this.props
            c.forEach(function(r) {
              !O(e, r) && O(n, r) && (t._values[r] = e[M(r)])
            })
          }),
          (u.componentWillUnmount = function() {
            this.unmounted = !0
          }),
          (u.render = function() {
            var e = this,
              n = this.props,
              o = n.innerRef,
              a = (function(e, t) {
                if (null == e) return {}
                var n,
                  r,
                  o = {},
                  a = Object.keys(e)
                for (r = 0; r < a.length; r++)
                  (n = a[r]), t.indexOf(n) >= 0 || (o[n] = e[n])
                return o
              })(n, ['innerRef'])
            l.forEach(function(e) {
              delete a[e]
            })
            var i = {}
            return (
              c.forEach(function(t) {
                var n = e.props[t]
                i[t] = void 0 !== n ? n : e._values[t]
              }),
              r.createElement(
                t,
                w({}, a, i, this.handlers, { ref: o || this.attachRef })
              )
            )
          }),
          s
        )
      })(r.Component)
      ;(u.displayName = 'Uncontrolled(' + i + ')'),
        (u.propTypes = w(
          { innerRef: function() {} },
          (function(e, t) {
            var n = {}
            return (
              Object.keys(e).forEach(function(e) {
                n[M(e)] = _
              }),
              n
            )
          })(n)
        )),
        o.forEach(function(e) {
          u.prototype[e] = function() {
            var t
            return (t = this.inner)[e].apply(t, arguments)
          }
        })
      var f = u
      return (
        r.forwardRef &&
          ((f = r.forwardRef(function(e, t) {
            return r.createElement(u, w({}, e, { innerRef: t }))
          })).propTypes = u.propTypes),
        (f.ControlledComponent = t),
        (f.deferControlTo = function(t, r, o) {
          return void 0 === r && (r = {}), e(t, w({}, n, r), o)
        }),
        f
      )
    })(nl, { view: 'onView', date: 'onNavigate', selected: 'onSelectEvent' }),
    ol = function(e, t, n) {
      var r = e.start,
        o = e.end
      return n.format(r, 'LT', t) + ' – ' + n.format(o, 'LT', t)
    },
    al = {
      dateFormat: 'DD',
      dayFormat: 'DD ddd',
      weekdayFormat: 'ddd',
      selectRangeFormat: ol,
      eventTimeRangeFormat: ol,
      eventTimeRangeStartFormat: function(e, t, n) {
        var r = e.start
        return n.format(r, 'LT', t) + ' – '
      },
      eventTimeRangeEndFormat: function(e, t, n) {
        var r = e.end
        return ' – ' + n.format(r, 'LT', t)
      },
      timeGutterFormat: 'LT',
      monthHeaderFormat: 'MMMM YYYY',
      dayHeaderFormat: 'dddd MMM DD',
      dayRangeHeaderFormat: function(e, t, n) {
        var r = e.start,
          o = e.end
        return (
          n.format(r, 'MMMM DD', t) +
          ' – ' +
          n.format(o, G(r, o, 'month') ? 'DD' : 'MMMM DD', t)
        )
      },
      agendaHeaderFormat: function(e, t, n) {
        var r = e.start,
          o = e.end
        return n.format(r, 'L', t) + ' – ' + n.format(o, 'L', t)
      },
      agendaDateFormat: 'ddd MMM DD',
      agendaTimeFormat: 'LT',
      agendaTimeRangeFormat: ol,
    }
  var il = function(e, t, n) {
      var r = e.start,
        o = e.end
      return n.format(r, 't', t) + ' – ' + n.format(o, 't', t)
    },
    sl = {
      dateFormat: 'dd',
      dayFormat: 'ddd dd/MM',
      weekdayFormat: 'ddd',
      selectRangeFormat: il,
      eventTimeRangeFormat: il,
      eventTimeRangeStartFormat: function(e, t, n) {
        var r = e.start
        return n.format(r, 't', t) + ' – '
      },
      eventTimeRangeEndFormat: function(e, t, n) {
        var r = e.end
        return ' – ' + n.format(r, 't', t)
      },
      timeGutterFormat: 't',
      monthHeaderFormat: 'Y',
      dayHeaderFormat: 'dddd MMM dd',
      dayRangeHeaderFormat: function(e, t, n) {
        var r = e.start,
          o = e.end
        return (
          n.format(r, 'MMM dd', t) +
          ' – ' +
          n.format(o, G(r, o, 'month') ? 'dd' : 'MMM dd', t)
        )
      },
      agendaHeaderFormat: function(e, t, n) {
        var r = e.start,
          o = e.end
        return n.format(r, 'd', t) + ' – ' + n.format(o, 'd', t)
      },
      agendaDateFormat: 'ddd MMM dd',
      agendaTimeFormat: 't',
      agendaTimeRangeFormat: il,
    }
  function cl(e) {
    return new N({
      firstOfWeek: function(t) {
        return (
          ((t = (function(t) {
            return t ? e.findClosestCulture(t) : e.culture()
          })(t)) &&
            t.calendar.firstDay) ||
          0
        )
      },
      formats: sl,
      format: function(t, n, r) {
        return e.format(t, n, r)
      },
    })
  }
  var ll = function(e, t, n) {
      var r = e.start,
        o = e.end
      return (
        n.format(r, { time: 'short' }, t) +
        ' – ' +
        n.format(o, { time: 'short' }, t)
      )
    },
    ul = {
      dateFormat: 'dd',
      dayFormat: 'eee dd/MM',
      weekdayFormat: 'eee',
      selectRangeFormat: ll,
      eventTimeRangeFormat: ll,
      eventTimeRangeStartFormat: function(e, t, n) {
        var r = e.start
        return n.format(r, { time: 'short' }, t) + ' – '
      },
      eventTimeRangeEndFormat: function(e, t, n) {
        var r = e.end
        return ' – ' + n.format(r, { time: 'short' }, t)
      },
      timeGutterFormat: { time: 'short' },
      monthHeaderFormat: 'MMMM yyyy',
      dayHeaderFormat: 'eeee MMM dd',
      dayRangeHeaderFormat: function(e, t, n) {
        var r = e.start,
          o = e.end
        return (
          n.format(r, 'MMM dd', t) +
          ' – ' +
          n.format(o, G(r, o, 'month') ? 'dd' : 'MMM dd', t)
        )
      },
      agendaHeaderFormat: function(e, t, n) {
        var r = e.start,
          o = e.end
        return (
          n.format(r, { date: 'short' }, t) +
          ' – ' +
          n.format(o, { date: 'short' }, t)
        )
      },
      agendaDateFormat: 'eee MMM dd',
      agendaTimeFormat: { time: 'short' },
      agendaTimeRangeFormat: ll,
    }
  var fl = function(e, t, n) {
      var r = e.start,
        o = e.end
      return n.format(r, 'p', t) + ' – ' + n.format(o, 'p', t)
    },
    dl = {
      dateFormat: 'dd',
      dayFormat: 'dd eee',
      weekdayFormat: 'cccc',
      selectRangeFormat: fl,
      eventTimeRangeFormat: fl,
      eventTimeRangeStartFormat: function(e, t, n) {
        var r = e.start
        return n.format(r, 'h:mma', t) + ' – '
      },
      eventTimeRangeEndFormat: function(e, t, n) {
        var r = e.end
        return ' – ' + n.format(r, 'h:mma', t)
      },
      timeGutterFormat: 'p',
      monthHeaderFormat: 'MMMM yyyy',
      dayHeaderFormat: 'cccc MMM dd',
      dayRangeHeaderFormat: function(e, t, n) {
        var r = e.start,
          o = e.end
        return (
          n.format(r, 'MMMM dd', t) +
          ' – ' +
          n.format(o, G(r, o, 'month') ? 'dd' : 'MMMM dd', t)
        )
      },
      agendaHeaderFormat: function(e, t, n) {
        var r = e.start,
          o = e.end
        return n.format(r, 'P', t) + ' – ' + n.format(o, 'P', t)
      },
      agendaDateFormat: 'ccc MMM dd',
      agendaTimeFormat: 'p',
      agendaTimeRangeFormat: fl,
    },
    pl = { eventWrapper: a, timeSlotWrapper: a, dateCellWrapper: a }
  ;(e.Calendar = rl),
    (e.DateLocalizer = N),
    (e.Navigate = x),
    (e.Views = j),
    (e.components = pl),
    (e.dateFnsLocalizer = function(e) {
      var t = e.startOfWeek,
        n = e.getDay,
        r = e.format,
        o = e.locales
      return new N({
        formats: dl,
        firstOfWeek: function(e) {
          return n(t(new Date(), { locale: o[e] }))
        },
        format: function(e, t, n) {
          return r(new Date(e), t, { locale: o[n] })
        },
      })
    }),
    (e.globalizeLocalizer = function(e) {
      var t = function(t) {
        return t ? e(t) : e
      }
      return e.load
        ? new N({
            firstOfWeek: function(e) {
              try {
                var n = t(e).cldr,
                  r = n.attributes.territory,
                  o = n.get('supplemental').weekData.firstDay[r || '001']
                return [
                  'sun',
                  'mon',
                  'tue',
                  'wed',
                  'thu',
                  'fri',
                  'sat',
                ].indexOf(o)
              } catch (n) {
                var a = new Date(),
                  i = Math.max(
                    parseInt(t(e).formatDate(a, { raw: 'e' }), 10) - 1,
                    0
                  )
                return Math.abs(a.getDay() - i)
              }
            },
            formats: ul,
            format: function(e, n, r) {
              return (
                (n = 'string' == typeof n ? { raw: n } : n),
                t(r).formatDate(e, n)
              )
            },
          })
        : cl(e)
    }),
    (e.momentLocalizer = function(e) {
      return new N({
        formats: al,
        firstOfWeek: function(t) {
          var n = t ? e.localeData(t) : e.localeData()
          return n ? n.firstDayOfWeek() : 0
        },
        format: function(t, n, r) {
          return ((o = e(t)), (a = r), a ? o.locale(a) : o).format(n)
          var o, a
        },
      })
    }),
    (e.move = Ls),
    Object.defineProperty(e, '__esModule', { value: !0 })
})
